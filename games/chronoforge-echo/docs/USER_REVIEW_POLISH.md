...
- **Seven layered keyboard checks pass** in [modal-escape/results.json](../evidence/modal-escape/results.json). F → rest → Esc returns to the world. From an atlas over a service, Esc closes the atlas, then the service. Confirmations and key-binding capture dismiss first. Required story choices and ending callbacks still require explicit action. Battle pause restores the exact action.
