...
export function mountWorldView(game) {
  ...
  chrome.innerHTML =
    '<header><div><strong></strong><small>World view · paused</small></div><button type="button">Return to play <kbd>R / Esc</kbd></button></header><p aria-live="polite"></p>';
  root.append(chrome);
  ...
  function draw(progress) {
    const { scene, start, image, state } = session,
      camera = worldViewCamera(scene, start, progress);
