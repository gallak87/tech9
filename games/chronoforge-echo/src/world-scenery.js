...
const measuredAspect = (biome, part) => {
  ...
  return f
    ? f.w / f.h
    : REGIONAL_PROP_ASPECTS[biome]?.[part] ||
        { rock: 1.07, cypress: 0.56, arch: 0.85, tree: 0.96 }[part] ||
        1;
};

export function sceneryBounds(o, biome) {
  if (o.type === 'cave') return caveEntranceBounds(o, biome);
  const detail = worldDetailBounds(o, biome);
  ...
  if (o.type === 'camp')
    return frameBounds(
      WORLD_PROP_ART.frames[WORLD_PROP_ART.restFrames[0]],
      o.x,
      o.y,
      88,
    );
  if (o.type === 'sign')
    return frameBounds(
      ROAD_SIGN_ART.frames[0],
      o.x,
      o.y,
      ROAD_SIGN_ART.frames[0].nativeHeight,
    );
  let part, height;
  if (o.type === 'tree') {
    ...
    part = 'arch';
    height = 164;
  } else if (o.style === 'biome_prop') {
    part = o.part;
    height = o.drawHeight;
  } else if (
    o.type === 'landmark' &&
    ['ring', 'arch', 'bell'].includes(o.style)
  ) {
    part = o.style === 'arch' ? 'arch' : 'ring';
    height = 320 * (o.style === 'bell' ? 0.7 : o.size || 1);
  }
  if (!part) return null;
  const width = height * measuredAspect(biome, part);
  return { left: o.x - width * 0.5, top: o.y - height * 0.97, width, height };
}
...
export function sceneryOcclusionBounds(scene, o) {
  ...
  if (
    o.style === 'interior_supply' ||
    (o.fieldRecord && o.id.endsWith('_record'))
  ) {
    const f = caveArtFrame(scene.biome, 'station');
    return f ? frameBounds(f, o.x, o.y, 100) : null;
  }
  if (o.fieldRecord) {
    const width = 70 * (REGIONAL_PROP_ASPECTS[scene.biome]?.cypress || 0.56);
    return { left: o.x - width * 0.5, top: o.y - 70 * 0.97, width, height: 70 };
  }
  return null;
}
...
export function sceneryFootprints(o, biome) {
  const authored = authoredLandmarkFootprints(o);
  if (authored) return authored;
  const size = o.size || 1,
    b = sceneryBounds(o, biome),
    w = b?.width || 0;
  if (o.type === 'cave')
    return [
      rect(-w * 0.46, -41, w * 0.29, 45),
      rect(w * 0.18, -41, w * 0.28, 45),
      rect(-w * 0.18, -91, w * 0.36, 35),
    ];
  if (o.type === 'tree') {
    const fraction =
      o.variant === 3
        ? { coast: 0.57, desert: 0.47, snow: 0.66, ice: 0.58 }[biome] || 0.52
        : {
            coast: 0.32,
            desert: 0.39,
            forest: 0.32,
    ...
    const width = Math.max(27, w * fraction),
      height = (o.variant === 3 ? 27 : 22) * size;
    return [rect(-width / 2, -height, width, height + 3)];
  }
  if (o.type === 'rock')
    return [
      rect(
        -measuredAspect(biome, 'rock') * 26,
        -24,
        measuredAspect(biome, 'rock') * 52,
        28,
      ),
    ];
  ...
  if (o.type === 'ruin' || o.style === 'arch') {
    ...
    return [
      rect(-w * 0.44, -depth, w * 0.24, depth + 4),
      rect(w * 0.2, -depth, w * 0.24, depth + 4),
    ];
  }
  if (o.style === 'biome_prop') {
    if (o.part === 'cypress') return [rect(-w * 0.29, -17, w * 0.58, 20)];
    if (o.part === 'rock') return [rect(-w * 0.4, -11, w * 0.8, 14)];
  }
  if (o.type !== 'landmark') return null;
  switch (o.style) {
    case 'ring':
    case 'bell':
      return [rect(-w * 0.34, -30 * size, w * 0.68, 34 * size)];
    case 'greattree':
      return [rect(-112, -65, 225, 70)];
    case 'furnace':
      return [rect(-185, -105, 370, 112)];
    case 'elevator':
      return [rect(-91, -57, 182, 63)];
    case 'ice':
      return [rect(-121, -49, 242, 55)];
    case 'hand':
      return [rect(-147, -57, 294, 65)];
    case 'palace':
      return [rect(-184, -76, 370, 85)];
    case 'bones':
      return [
        rect(-237, -84, 163, 123),
        rect(36, -115, 155, 114),
    ...
    case 'masts':
      return [-79, 0, 79].map((x, i) => {
        const width = (190 + i * 26) * measuredAspect(biome, 'cypress') * 0.48;
        return rect(x - width * 0.5, -28 - (i % 2) * 15, width, 33);
      });
    default:
      return null;
  }
...
export function footprintBlocks(o, x, y, padding = 6) {
  return (
    o.footprints?.some(
      (r) =>
        x > o.x + r.x - padding &&
        x < o.x + r.x + r.w + padding &&
        y > o.y + r.y - padding &&
        y < o.y + r.y + r.h + padding,
    ) || false
  );
}
...
export function configureWorldScenery(regions) {
  for (const scene of Object.values(regions)) {
    for (const [id, [x, y]] of Object.entries(placements[scene.id] || {})) {
      ...
      if (object) Object.assign(object, { x, y });
    }
    ...
    for (const portal of scene.portals) {
      ...
      const marker = {
        x: portal.x + (horizontal ? 0 : -73),
        y: portal.y + (horizontal ? -78 : 0),
      };
      const stone = {
        x: portal.x + (horizontal ? 0 : 72),
        y: portal.y + (horizontal ? 69 : 0),
      };
      scene.objects.push(
        {
          id: portal.id + '_marker',
          type: 'landmark',
          style: 'biome_prop',
          part: 'cypress',
          drawHeight: 110,
    ...
    const targets = scene.objects.filter(
      (o) =>
        ['town', 'house', 'cave', 'console'].includes(o.type) ||
        (o.type === 'landmark' &&
          !o.portalScenery &&
          !o.building &&
          o.style !== 'biome_prop'),
    ...
    scene.objects = scene.objects.filter((o) => {
      ...
      const b = sceneryBounds(o, scene.biome);
      if (!b) return true;
      return !targets.some((t) => {
        const landmark = worldDetailBounds(t, scene.biome),
          focusY = t.y - (landmark ? Math.min(120, landmark.height * 0.3) : 35);
        return (
          o.y > t.y - 12 &&
          focusY > b.top + 20 &&
          focusY < o.y &&
          t.x > b.left + 18 &&
          t.x < b.left + b.width - 18
        );
      });
    });
  }
...
export function configureCaveScenery(scene) {
  ...
  for (const o of scene.objects) {
    if (o.style === 'interior_supply' || o.fieldRecord) {
      o.solid = true;
      o.footprints =
        o.style === 'interior_supply' || o.id.endsWith('_record')
          ? [rect(-37, -26, 74, 29)]
          : [rect(-19, -16, 38, 20)];
    } else if (o.style === 'relic') {
      o.solid = true;
      o.footprints = [rect(-28, -16, 56, 19)];
    }
  }
}
