...
import {
  TOWN_INTERIOR_ASSETS,
  townInteriorFrame,
} from '../src/town-interior-art.js';
import { havenInteriorBounds } from '../src/haventide-interior-renderer.js';
import {
  upgradePreviewStates,
  upgradeTourCameras,
  UpgradeTour,
...
test('all four towns have four required production kits with valid measured crops', () => {
  ...
  for (const region of regions)
    for (const level of [1, 2, 3, 4]) {
      ...
      assert.equal(asset.metadata.sourceWidth, png.readUInt32BE(16));
      assert.equal(asset.metadata.sourceHeight, png.readUInt32BE(20));
      assert.deepEqual(
        asset.metadata.frames.map((f) => f.part),
        parts,
      );
      for (const part of parts) {
        ...
        assert.ok(
          frame.x + frame.w <= asset.metadata.sourceWidth &&
            frame.y + frame.h <= asset.metadata.sourceHeight,
          asset.id + ' ' + part,
        );
        for (const [x, y] of frame.backgroundSeeds || [])
          assert.ok(x >= 0 && y >= 0 && x < frame.w && y < frame.h);
      }
      assert.equal(
        spec.frame.key,
        false,
        'Floor extraction must remain opaque',
      );
    }
});
...
for (const region of [...regions.slice(1), ...Object.keys(ROAD_TOWNS)])
  test(
    ...
    () => {
      const hall = getScene(region + '_town'),
        state = createState();
      state.tier = 4;
      assert.equal(hall.restorationInterior, true);
      const origin = hall.spawn,
        step = 16,
        points = [origin],
        seen = new Set(['0,0']),
        nodes = [[0, 0]];
      ...
      for (const object of [...hall.objects, ...hall.portals].filter(
        (o) => o.service || o.type === 'npc' || o.type === 'portal',
      )) {
        assert.ok(
        ...
        if (object.service) {
          ...
          assert.equal(
            isWalkable(
              hall,
              object.x - object.artWidth * 0.5 - 24,
              object.y + 5,
            ),
            true,
            'Staff approaches',
      ...
      for (let x = 83; x <= 1517; x += 32)
        for (let y = 145; y <= 1055; y += 32) {
...
test('selected regional art and all upgrade steps are detached previews at Haventide', () => {
  ...
  state.tier = 2;
  state.buildings.town_center = 3;
  state.fog.haventide = { '4,2': true };
  state.visited.haventide_town = true;
  const baseline = JSON.stringify(state),
    hall = getScene(state.region),
    geometry = JSON.stringify(hall);
  const preview = new ArtPreview();
  preview.setOpen(true);
  for (const region of regions) {
    preview.selectTown(region);
    preview.selectTownCenter(4);
    const visual = preview.visualState(state);
    assert.equal(
      townInteriorFrame('board', visual, hall.townId).asset.region,
      region,
    );
    for (const actual of regions.slice(1))
      assert.equal(
        townInteriorFrame('board', visual, actual).asset.region,
        actual,
        'Art substitution is confined to Haventide',
      );
    ...
    for (const level of [1, 2, 3]) {
      ...
      assert.equal(plan.previewLocation, 'haventide');
      for (const snapshot of [plan.before, plan.after]) {
        assert.equal(snapshot.region, state.region);
        assert.equal(snapshot.tier, 2);
        assert.deepEqual(snapshot.fog, state.fog);
        assert.deepEqual(snapshot.visited, state.visited);
        assert.deepEqual(snapshot.resources, state.resources);
        const spec = townInteriorFrame('board', snapshot, hall.townId);
        assert.equal(spec.asset.region, region);
        assert.ok(
          havenInteriorBounds(
            hall.objects.find((o) => o.service === 'construction'),
            snapshot,
          ),
        );
      }
      assert.notEqual(
        townInteriorFrame('board', plan.before).asset.source,
        townInteriorFrame('board', plan.after).asset.source,
      );
      plan.after.resources.ore = 0;
...
test('real regional upgrades share the approved fixed indoor comparison and half-second restoration', () => {
  for (const region of regions) {
    ...
    assert.equal(plan.region, region);
    assert.equal(plan.previewLocation, undefined);
    const view = {
      camera: { x: 440, y: 450 },
      actors: [{ id: 'kaida', x: 900, y: 975, facing: 'up' }],
    };
    ...
    const tour = new UpgradeTour({
      interiorReveal: inside.restorationInterior,
    });
    tour.play();
    let observed = false;
    while (tour.phase === 'playing') {
      if (tour.frame.id === 'interiorUpgrade') {
        observed = true;
        assert.equal(tour.frame.duration, 0.5);
        assert.equal(tour.frame.from, 'deskBefore');
        assert.equal(tour.frame.to, 'deskAfter');
      }
      tour.advance(0.025);
    }
