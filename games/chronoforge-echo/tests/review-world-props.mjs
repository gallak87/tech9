...
const context=await browser.newContext({viewport:{width:1440,height:810},recordVideo:{dir:'evidence/world-prop-motion',size:{width:1440,height:810}}});
...
async function stage(scene,id,name,time=0){
 ...
 await page.waitForTimeout(150);await page.screenshot({path:`evidence/polish-${name}.png`});rows.push({name,...info});return info;
}
...
for(const [i,t]of [[1,.4],[2,.75]]){await page.evaluate(t=>__ECHO__.game.visualTime=t,t);await page.waitForTimeout(100);await page.screenshot({path:`evidence/polish-rest-lantern-${i}.png`});}
...
const video=page.video();await context.close();await video.saveAs('evidence/world-props-lantern-motion.webm');await browser.close();
await fs.writeFile('evidence/world-props-review.json',JSON.stringify({rows,collected,errors,diagnostics},null,2));
if(errors.length||diagnostics.snapshot.errors.length)throw Error('Runtime errors');console.log(JSON.stringify({captures:rows.length+3,collected,errors,metrics:diagnostics.snapshot.artMetrics,video:'evidence/world-props-lantern-motion.webm'},null,2));
