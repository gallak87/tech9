...
try{
 ...
 await page.evaluate(()=>{window.__ECHO__.preset('party');window.__ECHO__.game.ui.toggleMenu();});
 ...
 await page.evaluate(()=>{const g=window.__ECHO__.game;g.state.inventory.glacial_claw=1;g.state.heroes[0].hp=10;g.state.heroes[0].skillPoints=5;g.state.heroes[0].skills=['rift_cleave','salt_mend'];g.ui.hero=0;});
 ...
 await page.keyboard.press('6');await click('save:1');await page.evaluate(()=>window.__ECHO__.game.state.heroes[0].hp=12);await click('load:1');await page.keyboard.press('Escape');assert.equal((await state()).heroes[0].hp,12);await click('load:1');await click('confirm-yes');assert.equal((await state()).heroes[0].hp,90);
 ...
 await page.keyboard.press('1');const before=await page.evaluate(()=>({x:window.__ECHO__.game.ui.map.panX,z:window.__ECHO__.game.ui.map.zoom}));await page.keyboard.press('ArrowRight');await page.keyboard.press('+');assert.ok(await page.evaluate(b=>window.__ECHO__.game.ui.map.panX<b.x&&window.__ECHO__.game.ui.map.zoom>b.z,before));await page.keyboard.press('r');assert.equal(await page.evaluate(()=>window.__ECHO__.game.ui.map.panX),0);
 await page.evaluate(()=>{const g=window.__ECHO__.game;g.state.flags.haventide_liberated=true;g.ui.render();});await click('travel:haventide');await page.waitForTimeout(700);assert.equal((await state()).region,'haventide_town');
 await page.evaluate(()=>{const g=window.__ECHO__.game;g.ui.showVendor({id:'review-rest',name:'Wayfarer’s rest',service:'rest'});});await page.screenshot({path:new URL('rest.png',out).pathname});assert.equal(await page.locator('[data-do="open-atlas"]').count(),0);assert.equal(await page.evaluate(()=>document.activeElement?.dataset.do),'rest');await page.keyboard.press('Enter');assert.equal(await page.locator('.expedition').count(),0);assert.match(await page.locator('.notice').innerText(),/crew rests|welcome the crew freely/i);await page.keyboard.press('Escape');assert.equal(await page.locator('.atlas').count(),0);
 await page.evaluate(()=>{const g=window.__ECHO__.game;g.ui.showDialogue([{speaker:'Kaida',text:'The salt road is quiet again. Let’s see who still keeps a lamp in the window.'}]);});await page.screenshot({path:new URL('dialogue.png',out).pathname});await page.keyboard.press('Enter');
