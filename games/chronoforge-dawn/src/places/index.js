...
// places — LANE STUB.
//
// Cities, buildings, doors, interior lighting, the transition both ways, NPCs.
//
// Phase 0 ships the seam, not the feature. `main.js` already installs this and
// hands it the shared `ctx`; the places lane fills it in without touching a
// single shared-core file. Do not widen this signature — add fields to the
// returned API instead.
//
// Owned by the `places` lane. See CONTRACT.md §1.
// ─────────────────────────────────────────────────────────────────────────────

export function installPlaces(ctx) {
  return {
     ...
     *  `?showcase=places` boots straight into it and the critic reviews the
