import { encounterBadgeLabel,encounterDanger,ENEMY_DANGER_STYLES } from './enemy-levels.js';
...
export function drawEncounterRings(ctx,scene,camera,state,{contactReady=true}={}){
  ...
  for(const encounter of scene.objects){
    ...
    const x=encounter.x-Math.round(camera.x||0),y=encounter.y-Math.round(camera.y||0);
    if(x+radiusX<0||x-radiusX>VIEW_WIDTH||y+radiusY<0||y-radiusY>VIEW_HEIGHT)continue;
    ctx.beginPath();ctx.ellipse(x,y,radiusX,radiusY,0,0,Math.PI*2);
    ctx.fillStyle='#d657500c';ctx.fill();
    // A quiet dark under-stroke keeps the same red legible on snow and lava.
    ctx.lineWidth=2;ctx.strokeStyle='#15191c40';ctx.stroke();
    ctx.lineWidth=1;ctx.strokeStyle='#d657506b';ctx.stroke();
  }
  ctx.restore();
}

function skull(ctx,x,y,style){
  ctx.fillStyle=style.text;ctx.beginPath();ctx.ellipse(x,y-1,3.5,3.3,0,0,Math.PI*2);ctx.fill();
  ctx.fillRect(x-2,y+1,4,3.5);
  ctx.fillStyle=style.background;
  ctx.fillRect(x-2.5,y-1.5,1.5,1.5);ctx.fillRect(x+1,y-1.5,1.5,1.5);ctx.fillRect(x-.5,y+1,1,1.5);
}
...
export function drawEncounterLevels(ctx, scene, camera, state) {
  ...
  for (const encounter of scene.objects) {
    ...
    const danger=cleared?'lower':encounterDanger(encounter,state),style=ENEMY_DANGER_STYLES[danger],warning=danger==='severe';
    const width = Math.ceil(ctx.measureText(label).width) + 14 + (warning?12:0);
    if (x + width / 2 < 0 || x - width / 2 > VIEW_WIDTH || footY < 0 || footY > VIEW_HEIGHT) continue;
    const left = Math.max(4,Math.min(VIEW_WIDTH-width-4,Math.round(x-width/2)));
    const y = Math.min(VIEW_HEIGHT-21,footY+(hasContactBoundary(encounter,state)?ENCOUNTER_RING.radiusY+5:10));
