...
export function localDevPreviewRequested(location) {
  return localDevHost(location)&&new URLSearchParams(location.search).get('dev')==='1';
}

export function devPreviewReady(game) {
  return game.mode==='world'&&!game.ui.blocked&&!game.transition&&!game.state.recruitmentWalk;
}
