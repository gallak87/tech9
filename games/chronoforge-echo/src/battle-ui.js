...
const combatantName = (actor) =>
  actor?.side === 'enemy' ? enemyNameWithLevel(actor) : actor?.name;
const pct = (value) => `${Math.max(0, Math.min(100, value))}%`;
const commandNotes = {
  Attack:
    'One precise strike. Catch the timing window to raise critical chance.',
  Tech: 'Learned techniques, healing and coordinated attacks. Each participant must be ready.',
  Defend:
    'Reduce damage by 65% until your next action. Catch the timing window for a 75% critical guard.',
...
const portrait = (id) =>
  `<canvas width="96" height="96" data-battle-portrait="${esc(id)}" aria-hidden="true"></canvas>`;
const status = (h) =>
  h.hp <= 0
    ? 'DOWN'
    : h.guarding
      ...
      : h.shield
        ...
        : h.slowTurns
          ? 'SLOWED'
          : '';
const button = (kind, body, attrs = '', classes = '') =>
  `<button type="button" class="${classes}" data-battle-intent="${kind}" ${attrs}>${body}</button>`;
...
export class BattleUI {
  ...
  render() {
    ...
    if (g.mode === 'battle' && b?.itemConfirmation && !g.ui.blocked) {
      ...
      g.ui.confirmItemUse(
        pending.use,
        () => {
          battleIntent(b, g.state, { kind: 'item-confirm' });
          g.ui.render();
        },
        { onCancel: () => battleIntent(b, g.state, { kind: 'item-cancel' }) },
      );
    }
    ...
    const stage =
      heroAction || incoming
        ...
        : v.mode === 'target'
          ? 2
          : ['command', 'tech', 'item'].includes(v.mode)
            ? 1
            : 0;
    const actor =
      v.heroes.find(
    ...
    const signature = JSON.stringify([
      ...
      v.action?.id,
      v.action?.resolved,
      v.action?.timingAttempted,
      v.action?.timingSuccess,
      v.action?.critical,
      v.result,
      v.heroes.map((h) => [
        h.id,
      ...
      v.enemies.map((e) => [e.uid, e.hp, e.shield, e.charging, e.bossPhase]),
      v.techs,
      v.items,
      b.noticeTime > 0 ? b.notice : '',
      g.state.settings.reducedMotion,
    ]);
    this.root.dataset.stage = stage;
    this.root.dataset.timingSide = incoming
      ? 'defense'
      : heroAction
        ? defending
          ? 'defense'
    ...
    this.root.classList.toggle(
      'cb-reduced',
      Boolean(g.state.settings.reducedMotion),
    );
    ...
    if (signature !== this.signature || this.battle !== b) {
      ...
      const selectedAction = incoming
        ? 'Defend'
        : v.pending?.name ||
          (heroAction ? v.action.name : v.commands[v.cursor]?.name || 'Action');
      const targets =
        heroAction || incoming
          ? v.action.targets
              .map(
                (id) =>
                  v.heroes.find((h) => h.id === id) ||
                  v.enemies.find((e) => e.uid === id),
              )
              .filter(Boolean)
          : v.targets;
      const targetLabel = heroAction
        ? targets.length > 1
          ? `${targets.length} targets`
          : combatantName(targets[0]) || 'Self'
        : v.pending?.target?.startsWith('all')
          ? 'Group'
          : combatantName(v.targets[v.target]) || 'Target';
      const attacker = incoming
        ? combatantName(
            v.enemies.find((e) => e.uid === v.action.participants[0]),
          ) || 'Enemy'
        : '';
      const labels = [
        incoming && targets.length > 1 ? 'The crew' : actor?.name || 'Crew',
        selectedAction,
        incoming ? attacker : targetLabel,
        'Timing',
      ];
      const nodeNames = ['Crew', 'Action', 'Target', 'Timing'];
      const title = b.enemies.some((e) => e.id === 'void_architect')
        ? 'The shape of tomorrow'
        : b.encounter.name ||
          (b.encounter.boss ? 'A signal in the dark' : 'Hold the line');
      const charging = v.enemies.filter((e) => e.hp > 0 && e.charging);
      const waitingStatus =
        v.action?.side === 'enemy'
          ? `${v.action.name} · enemy action`
          : v.readyQueue.length
            ? `${v.readyQueue.length} ready`
            : 'Gauges charging';
      ...
      for (const canvas of this.root.querySelectorAll('[data-battle-portrait]'))
        drawPortrait(
          ...
          canvas.dataset.battlePortrait,
          0,
          0,
          96,
        );
      this.root
        .querySelector('.cb-list .cb-selected')
        ?.scrollIntoView({ block: 'nearest' });
      if (focused)
        (
          this.root.querySelector('.cb-open .cb-selected:not(:disabled)') ||
          this.root.querySelector('.cb-open button:not(:disabled)')
        )?.focus({ preventScroll: true });
    }
    for (const h of v.heroes) {
      this.root.querySelectorAll(`[data-atb="${h.id}"]`).forEach((el) => {
        el.style.width = pct(h.atb);
      });
      this.root.querySelectorAll(`[data-charge="${h.id}"]`).forEach((el) => {
        el.textContent =
          h.hp <= 0 ? 'DOWN' : h.atb >= 100 ? 'READY' : `${Math.floor(h.atb)}%`;
      });
      const rail = this.root.querySelector(`[data-rail-state="${h.id}"]`);
      if (rail)
        rail.textContent =
          status(h) || (h.atb >= 100 ? 'READY' : `${Math.floor(h.atb)}%`);
    }
    const action = v.action;
    const timing = this.root.querySelector('.cb-timing');
    if (timing && action) {
      const inWindow =
        action.timingEligible &&
        !action.timingAttempted &&
        !action.resolved &&
        action.elapsed >= action.windowStart &&
        action.elapsed <= action.windowEnd;
      this.root.classList.toggle('cb-critical-window', inWindow);
      const progress = timing.querySelector('.cb-timing-marker');
      if (progress)
        progress.style.left = pct((action.elapsed / action.contact) * 100);
      const elapsed = timing.querySelector('.cb-action-progress i');
      if (elapsed)
        elapsed.style.width = pct((action.elapsed / action.duration) * 100);
      const cue = this.root.querySelector('.cb-timing-cue');
      if (cue)
        cue.textContent = action.timingAttempted
          ? action.timingSuccess
            ? defending
              ? 'CRITICAL GUARD'
              : 'TIMING CAUGHT'
            : incoming
  ...
  actions(v) {
    const list =
      v.mode === 'tech' ? v.techs : v.mode === 'item' ? v.items : v.commands;
    const current = list[v.cursor];
    const sub = v.mode !== 'command';
    return `<div class="cb-pane-head"><h3>${v.mode === 'tech' ? 'Techniques' : v.mode === 'item' ? 'Field supplies' : 'Choose an action'}</h3><span>${sub ? `${v.cursor + 1} / ${list.length}` : 'COMMANDS'}</span></div><div class="cb-action-layout"><div class="cb-list ${sub ? 'cb-sublist' : ''}">${list
      .map((item, i) =>
        button(
          sub ? 'list' : 'command',
          `${!sub ? `<span class="cb-command-sign">${signs[item.name]}</span>` : ''}<span class="cb-row-title">${esc(item.name)}${sub && item.participants?.length > 1 ? `<small>${esc(item.participants.join(' + '))}</small>` : ''}</span><span class="cb-command-cost">${
            v.mode === 'tech'
              ? typeof item.mp === 'number'
                ? `${item.mp} MP`
                : Object.entries(item.mp)
                    .map(([id, n]) => `${id} ${n}`)
                    .join(' / ')
              : v.mode === 'item'
                ? `×${item.count}`
          ...
          `data-index="${i}" aria-label="${esc(item.name)}${item.unavailable ? ', ' + esc(item.unavailable) : ''}" aria-disabled="${Boolean(item.unavailable)}"`,
          `cb-choice ${i === v.cursor ? 'cb-selected' : ''} ${item.unavailable ? 'cb-unavailable' : ''}`,
        ),
      )
      .join(
        '',
      )}</div><aside class="cb-choice-detail"><strong>${esc(current?.name || 'No supplies')}</strong><p>${esc(current?.unavailable || current?.description || commandNotes[current?.name] || 'No field supplies remain.')}</p>${current?.participants?.length > 1 ? `<small>${current.participants.length} ready companions required</small>` : ''}</aside></div>`;
  }
  ...
  timing(v) {
    const a = v.action;
    if (!a) return '';
    const incoming = a.side === 'enemy',
      defending = incoming || a.kind === 'defend',
      verb = defending ? 'Guard' : 'Strike';
    const inputNotch = Number.isFinite(a.timingPressedAt)
      ? `<i class="cb-input-notch ${a.timingSuccess ? 'cb-input-caught' : ''}" style="left:${pct((a.timingPressedAt / a.contact) * 100)}" role="img" aria-label="Your input at ${Math.round((a.timingPressedAt / a.contact) * 100)}% of the timing track" title="Your input"></i>`
      : '';
    const result = incoming
      ? a.timingSuccess
        ? 'Critical guard: 75% less damage from this attack.'
        : a.timingAttempted || a.resolved
          ? 'Existing guards and wards still apply.'
          : 'Incoming attack. Catch the orange window for 75% less damage.'
      : a.timingSuccess
        ? defending
          ? 'Critical guard: 75% less damage until your next action.'
          : 'Timing caught. Critical chance raised.'
        : a.timingAttempted
          ? defending
            ? 'Normal guard: 65% less damage until your next action.'
            : 'Normal strike. Your action continues.'
          : defending
            ? 'Catch the orange window for critical guard. A miss still raises normal guard.'
            : a.timingEligible
              ? 'Press as the marker crosses the orange window.'
              : 'The action is in motion.';
