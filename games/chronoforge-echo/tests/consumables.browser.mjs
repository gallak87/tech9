...
try{
 ...
 await p.evaluate(async()=>{const g=window.__ECHO__.game,P=await import('/src/progression.js'),{ITEMS}=await import('/src/content.js');window.__ECHO__.preset('party');g.ui.hero=1;const h=g.state.heroes[1];h.hp=P.stats(h,g.state).maxHp-1;for(const id of Object.keys(ITEMS))g.state.inventory[id]=3;g.ui.menu=true;g.ui.tab=2;g.ui.item='field_tonic';g.ui.render();});
 const before=await p.evaluate(()=>{const g=window.__ECHO__.game,pack=document.querySelector('.exp-pack-items');pack.scrollTop=60;return {stock:g.state.inventory.field_tonic,hp:g.state.heroes[1].hp,pack:pack.scrollTop};});
 ...
 const savedScroll=await p.evaluate(()=>window.__ECHO__.game.ui.panel.returnScroll.pack);
 await p.screenshot({path:reviewRoot + 'echo-consumable-confirm.png'});
 await p.keyboard.press('Escape');
 const canceled=await p.evaluate(()=>{const g=window.__ECHO__.game;return {stock:g.state.inventory.field_tonic,hp:g.state.heroes[1].hp,hero:g.ui.hero,item:g.ui.item,focus:document.activeElement.dataset.do,pack:document.querySelector('.exp-pack-items').scrollTop};});
 ...
 const accepted=await p.evaluate(()=>{const g=window.__ECHO__.game;return {stock:g.state.inventory.field_tonic,hp:g.state.heroes[1].hp,hero:g.ui.hero,item:g.ui.item};});
 ...
 await p.evaluate(async()=>{window.__ECHO__.preset('battle');const g=window.__ECHO__.game,b=g.battle,h=b.heroes[0];h.hp=h.maxHp-1;h.atb=100;b.readyQueue=[h.id];b.selectedHero=h.id;const C=await import('/src/combat.js');C.battleIntent(b,g.state,{kind:'command',index:3});C.battleIntent(b,g.state,{kind:'list',index:C.battleView(b,g.state).items.findIndex(i=>i.id==='field_tonic')});});
 await p.locator('[data-battle-intent="execute"]').click();await p.locator('.purchase-confirm').waitFor();
 await p.keyboard.press('Escape');assert.equal(await p.evaluate(()=>window.__ECHO__.game.battle.mode),'target');
 assert.equal(await p.evaluate(()=>window.__ECHO__.game.state.inventory.field_tonic),5);
 await p.keyboard.press('Enter');await p.locator('.purchase-confirm').waitFor();await p.keyboard.press('Enter');
 await p.waitForFunction(()=>window.__ECHO__.game.state.inventory.field_tonic===4);
 assert.equal(await p.evaluate(()=>{const h=window.__ECHO__.game.battle.heroes[0];return h.hp===h.maxHp;}),true);
