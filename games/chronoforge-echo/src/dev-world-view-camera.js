...
export function worldViewCamera(scene,start,progress) {
  ...
  const centerY=(start.y+H/2)*(1-ease)+(scene.height/2-13/fit)*ease;
...
export function worldViewSurfaceSize(scene) {
  ...
  const scale=Math.min(1,Math.sqrt(16_777_216/(scene.width*scene.height)));
  return {width:Math.max(1,Math.floor(scene.width*scale)),height:Math.max(1,Math.floor(scene.height*scale))};
}

export function* worldViewTiles(scene) {
  for(let y=0;y<scene.height;y+=H)for(let x=0;x<scene.width;x+=W)
    yield {x,y,width:Math.min(W,scene.width-x),height:Math.min(H,scene.height-y)};
}
