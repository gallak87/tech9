...
const encounter = (id, x, y, enemies, extra = {}) =>
  obj(id, 'encounter', x, y, { enemies, ...extra });
const consoleAt = (id, name, x, y, extra = {}) =>
  obj(id, 'console', x, y, { name, ...extra });
const pickup = (id, item, x, y, amount = 1) =>
  obj(id, 'pickup', x, y, { item, amount, name: 'Recovered supplies' });
const landmark = (id, name, x, y, style = 'ring', extra = {}) =>
  obj(id, 'landmark', x, y, { name, style, ...extra });
const sign = (id, name, x, y, dialogue) =>
...
const regions = [
  {
    ...
    objects: [
      ...
      encounter('hav_east_sentries', 4090, 990, ['drone_sentinel', 'slag_rat']),
  ...
  },
  {
    ...
    objects: [
      ...
      encounter('ember_signal', 1360, 475, ['neon_cultist', 'drone_sentinel'], {
        boss: true,
  ...
  },
  {
    ...
    objects: [
      landmark('crater_furnace', 'The second sunrise', 2410, 1040, 'furnace', {
        size: 2,
      }),
  ...
  },
  {
    ...
    objects: [
      ...
      encounter('orbital_east', 3430, 1020, ['frost_revenant', 'glacier_wolf']),
  ...
  },
  {
    ...
    objects: [
      ...
      landmark('frost_rescue', 'The silent rescue camp', 2850, 1560, 'caravan'),
      sign(
        'frost_warning',
...
for (const r of Object.values(REGIONS)) {
  if (r.town) {
    ...
    ['provisions', 'smith', 'inn', 'archivist', 'artificer', 'trainer'].forEach(
      (service, i) => {
    ...
    if (r.id === 'emberline')
      s.objects.push(obj('vex', 'npc', 410, 705, { name: 'Vex', hero: 'vex' }));
    if (r.id === 'orbital_reach')
      s.objects.push(
        obj('rune', 'npc', 410, 705, { name: 'Rune', hero: 'rune' }),
      );
    if (r.id === 'last_crown')
      s.objects.push(consoleAt('rune_oath', 'The sentinel’s oath', 1110, 725));
    interiors[id] = s;
  }
}
const dwellings = {
...
REGIONS.orbital_reach.objects.push(
  ...
  obj('orbital_entry_stone', 'rock', 380, 1290, { solid: true, w: 57, h: 26 }),
);
...
REGIONS.haventide.objects.find((o) => o.id === 'haventide_entrance').building =
  'town_center';
...
for (const [ri, r] of Object.values(REGIONS).entries()) {
  ...
  ruins.forEach(([x, y], i) => {
    if (distanceToRoad(r, x, y) < 100 || terrainAt(r, x, y) === 'water') return;
    r.objects.push(
  ...
  boulders.forEach(([x, y], i) => {
    if (distanceToRoad(r, x, y) < 65 || terrainAt(r, x, y) === 'water') return;
    r.objects.push(
...
export function getScene(id) {
  const scene = REGIONS[id] || interiors[id];
...
for (const [region, [, spoken, title, letter]] of Object.entries(
  ...
  caveScene.objects.find((o) => o.id === caveId + '_record').dialogue = letter;
}
const cavePlans = {
  hav_cave: [
    [70, 600, 410, 250],
