...
export function encounterLeader(encounter) {
  return encounter.enemies?.reduce(
    (leader, enemy) =>
      leader === undefined ||
      (enemyLevel(enemy) ?? 0) > (enemyLevel(leader) ?? 0)
        ? enemy
        : leader,
    undefined,
  );
...
export function encounterBadgeLabel(encounter) {
  const leader = encounterLeader(encounter),
    actor = typeof leader === 'string' ? ENEMIES[leader] : leader;
...
export function encounterDanger(encounter, state) {
  const kaida = state.heroes?.find((hero) => hero.id === 'kaida'),
    level = enemyLevel(encounterLeader(encounter));
...
export function encounterInteractionLabel(encounter, cleared = false) {
  ...
  const leader = encounterLeader(encounter),
    name = typeof leader === 'string' ? ENEMIES[leader]?.name : leader?.name;
