  ...
  const lethal=await page.evaluate(()=>{
   ...
   while(turns++<150){
    ...
    const b=g.battle;if(!b.action){
     const key=k=>B.battleKey(b,g.state,k),choose=i=>{let n=0;while(b.cursor!==i&&n++<30)key('ArrowDown');key('Enter');};
     const available=B.battleView(b,g.state).techs.filter(t=>!t.unavailable).map(t=>C.TECHS[t.id]);
     let tech=b.heroes.some(h=>h.hp>0&&h.hp/h.maxHp<.55)?available.find(t=>t.effect==='heal'&&t.target==='allAllies')||available.find(t=>t.effect==='heal'):null;
     if(!tech)tech=available.filter(t=>['damage','drain','slow'].includes(t.effect)).sort((a,z)=>z.power*z.heroes.length-a.power*a.heroes.length)[0];
     if(b.mode==='waiting')key('Enter');if(tech){choose(1);choose(B.battleView(b,g.state).techs.findIndex(t=>t.id===tech.id));}else choose(0);if(b.mode==='target')key('Enter');
    }
    const a=b.action;if(!a)throw Error('Legal action was not available');
    const before=B.battleView(b,g.state);window.deathUpdate.call(g,Math.max(.001,a.contact-a.elapsed));
    if(b.enemies.every(e=>e.hp===0))return {turns,enemy:b.enemies[0].id,killingAction:a.command.name,before,clock:b.clock,after:B.battleView(b,g.state)};
    if(b.heroes.every(h=>h.hp===0))throw Error('Fixture party defeated before the requested death frame');
    window.deathUpdate.call(g,a.duration-a.elapsed+.001);
