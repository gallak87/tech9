...
export function drawWorld(ctx,g){
  ...
  party.forEach((h,i)=>{
    ...
    const locomotionPhase=pos.locomotionPhase??g.locomotionPhase??0;
    draws.push({y:ty+.5,draw:()=>{steppedEllipse(ctx,'#25263855',tx,ty+2,12,5);drawHero(ctx,h.id,heroState,tx,ty,.75,pos.facing||face,time+i*.12,{reducedMotion:!!s.settings?.reducedMotion,...(moving?{locomotionPhase}:{})});}});
  });
