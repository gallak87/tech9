...
const earned=JSON.parse(await fs.readFile(base+'evidence/earned-campaign-saves.json','utf8')).saves['battle-void_architect'];
...
try{
 ...
 await page.waitForTimeout(40);await page.screenshot({path:base+'evidence/hp-hud-earned-crown-idle.png'});
 ...
 await page.waitForTimeout(40);await page.screenshot({path:base+'evidence/hp-hud-earned-crown-contact.png'});assert.deepEqual(report.errors,[]);report.result='pass';
}catch(e){report.result='fail';report.failure=String(e);process.exitCode=1;}finally{await fs.writeFile(base+'evidence/hp-hud-earned-crown.json',JSON.stringify(report,null,2));await browser.close();console.log(JSON.stringify({result:report.result,failure:report.failure,errors:report.errors,gutters:report.gutters}));}
