...
export function hasContactBoundary(encounter,state){
  return encounter.type==='encounter'&&!state.cleared?.[encounter.id]
    &&meetsWorldRequirement(state,encounter.requires);
}

function contactTime(encounter,from,to){
  const rx=ENCOUNTER_RING.radiusX+ENCOUNTER_CONTACT_PADDING,ry=ENCOUNTER_RING.radiusY+ENCOUNTER_CONTACT_PADDING;
  // Normalize the padded ellipse into a unit circle for the swept crossing.
  const x=(from.x-encounter.x)/rx,y=(from.y-encounter.y)/ry;
  const distance=x*x+y*y-1;
  if(distance<=0)return 0;
  const dx=(to.x-from.x)/rx,dy=(to.y-from.y)/ry,length=dx*dx+dy*dy;
  if(!length)return null;
  const dot=x*dx+y*dy,discriminant=dot*dot-length*distance;
...
export function contactEncounter(scene,state,from=state){
  let result=null,first=Infinity;
  for(const encounter of scene.objects){
    ...
    if(time!==null&&time<first){first=time;result=encounter;}
  }
  return result;
}
