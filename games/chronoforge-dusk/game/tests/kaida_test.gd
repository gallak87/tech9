...
func run(root: DuskFoundation) -> void:
	...
	check(walk_distance > 2.7 and walk_distance < 3.2, "Kaida responds to mapped walking input")
	...
	key_up(KEY_W)
	key_up(KEY_D)
	check(game.actor.position.length() < 3.25, "Diagonal input has no speed boost")
	game.actor.place(Vector3.ZERO)
	key_down(KEY_SHIFT)
	key_down(KEY_W)
	await seconds(1.0)
	check(game.actor.position.length() > 5.8 and absf(game.actor.visual.player.speed_scale - game.actor.run_speed / game.actor.run_stride_speed) < 0.03, "Run displacement and imported playback agree")
	key_up(KEY_W)
	key_up(KEY_SHIFT)
	var release: Vector3 = game.actor.position
	await seconds(0.25)
	check(game.actor.position.distance_to(release) < 0.65 and game.actor.visual.active_role == "idle", "Run stops within 65 cm without residual skating")
