...
func run(root: DuskFoundation) -> void:
	...
	check(walking > 2.7 and walking < 3.2 and game.actor.position.z < -1.7, "Camera-relative walk uses real input mapping")
	...
	check(running > walking * 1.5 and running < 6.9, "Shift runs faster than walking")
