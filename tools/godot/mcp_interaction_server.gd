...
# Port is overridable so this game and void-fracture can run side by side for
# A/B comparison. Precedence: --mcp-port=N on the command line, then the
# GODOT_MCP_PORT env var, then DEFAULT_PORT (what the MCP client expects).
...
func _resolve_port() -> int:
	...
	if OS.has_environment("GODOT_MCP_PORT"):
		return int(OS.get_environment("GODOT_MCP_PORT"))
	return DEFAULT_PORT
