...
function fixture() {
  const game = {
    ...
    battle: null,
    transition: null,
    ui: { blocked: false, panel: null },
    upgradeTour: { open: false },
    travel(id, spawn) {
      this.transition = {
        to: id,
        spawn: safeArrival(REGIONS[id], spawn.x, spawn.y),
      };
    },
  };
...
test('world preview isolates gameplay without revealing or changing the original survey', () => {
  ...
  assert.equal(
    preview.enable(),
    false,
    'Repeated activation must not replace the real expedition',
  );
  ...
  assert.deepEqual(
    original,
    baseline,
    'Gameplay in the preview must remain isolated',
  );
...
test('all eight region jumps use safe arrivals while story gates remain untouched', () => {
  const { game, preview } = fixture(),
    flags = structuredClone(game.state.flags);
  ...
  for (const region of Object.values(REGIONS)) {
    ...
    assert.equal(
      preview.active,
      true,
      'The first jump starts the detached expedition automatically',
    );
    assert.equal(game.transition.to, region.id);
    assert.equal(
      isWalkable(region, game.transition.spawn.x, game.transition.spawn.y),
      true,
    );
    assert.equal(
      preview.jump('haventide'),
      false,
      'Cannot jump during an existing transition',
    );
...
test('map reveal is independent of temporary travel and never writes survey or save data', () => {
  ...
  for (const region of Object.values(REGIONS)) {
    ...
    assert.equal(
      mapPointVisible(game, region.id, region.width - 1, region.height - 1),
      true,
    );
  }
  assert.equal(
    preview.active,
    false,
    'Revealing the menu map does not start a trip',
  );
  assert.equal(preview.saveSource().state, original);
  assert.deepEqual(original, baseline);
  game.devTools.mapExplored = false;
  assert.equal(
    preview.jump('last_crown'),
    true,
    'Quick jump needs no map reveal',
  );
  game.transition = null;
  game.state.region = 'last_crown';
  game.state.x = 4000;
  game.state.y = 400;
  const detached = game.state,
    before = structuredClone(detached);
  game.devTools.mapExplored = true;
  ...
  assert.deepEqual(
    game.state,
    before,
    'Toggling cannot move the party or change its survey',
  );
  assert.equal(preview.saveSource().state, original);
  assert.deepEqual(original, baseline);
  assert.equal(
    preview.jump('mire_bog'),
    true,
    'Quick jumps remain available after turning reveal off',
  );
  assert.equal(preview.restore(), true);
  assert.equal(game.state, original);
});
...
test('autosave, manual save and export use the original expedition even during a preview battle', () => {
  const { game, preview } = fixture(),
    baseline = structuredClone(game.state);
  const slots = new Map(),
    storage = {
  ...
  game.state.x = 4000;
  game.state.y = 400;
  game.state.flags.final_ready = true;
  game.state.resources.ore = 9999;
  game.state.heroes[0].hp = 1;
  game.state.playTime = 9000;
  game.battle = { id: 'preview-only-battle' };
  game.mode = 'battle';
  ...
  for (const slot of ['checkpoint', 1]) {
    ...
    assert.equal(
      JSON.parse(exportSave(slot, storage)).state.flags.final_ready,
      undefined,
    );
  }
  preview.restore();
...
test('loading or starting another expedition discards preview references without restoring the old state', () => {
  ...
  preview.restore();
  assert.equal(game.state.resources.ore, 42);
});
...
test('activation waits for exploration and jumps cannot interrupt encounters, dialogue or recruitment', () => {
  ...
  for (const patch of [
    ...
    const { game, preview } = fixture();
    preview.enable();
    if (patch.state) Object.assign(game.state, patch.state);
    else Object.assign(game, patch);
