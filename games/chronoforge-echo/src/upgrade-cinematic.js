...
const shots = [
  ...
  { id: 'upgrade', duration: 0.85, from: 'outsideBefore', to: 'outsideAfter' },
  { id: 'exterior', duration: 0.95, from: 'outsideAfter' },
...
const restorationShots = shots.flatMap((shot) =>
  shot.id === 'inside'
    ? [
        { ...shot, to: 'deskBefore' },
        { id: 'interiorBefore', duration: 0.24, from: 'deskBefore' },
        {
          id: 'interiorUpgrade',
          duration: 0.5,
          from: 'deskBefore',
...
export function upgradeShotScale(shot, frame, reducedMotion = false) {
  ...
  if (frame.startsWith('outside'))
    return shot.id === 'before'
      ? 1 + 0.02 * smooth((shot.progress - 0.2) / 0.8)
      : shot.id === 'outside'
        ? 1
        : 1.02;
  return 1;
...
export function upgradeSparkles(elapsed) {
  ...
  for (let i = 0; i < 36; i++) {
    ...
    particles.push({
      ...
      alpha: Math.min(1, age / 0.045) * Math.min(1, (life - age) / 0.2),
      size: 1.1 + (i % 3) * 0.55,
      trail: 5 + (i % 4) * 2,
      color: i % 3 ? '#f5dab0' : '#b4f5ee',
    });
  }
  return particles;
}
...
export function interiorUpgradeSparkles(elapsed, targets) {
  ...
  for (let i = 0; i < 16; i++) {
    ...
    particles.push({
      x:
        target.x +
        target.width * (0.5 + Math.sin(i * 7.13) * 0.34) +
        Math.sin(i * 3.71) * age * 10,
      y:
        target.y +
        target.height * (0.6 + Math.sin(i * 2.39) * 0.24) -
        age * (80 + (i % 3) * 18),
      alpha: 0.65 * Math.min(1, age / 0.035) * Math.min(1, (life - age) / 0.12),
      size: 0.8 + (i % 3) * 0.3,
      trail: 2 + (i % 2),
      color: i % 3 ? '#f5dab0' : '#b4f5ee',
    });
...
export const UPGRADE_TOUR_DURATION = shots.reduce(
...
export class UpgradeTour {
  ...
  constructor({ interiorReveal = false } = {}) {
    ...
    this.duration = (interiorReveal ? restorationShots : shots).reduce(
      (sum, shot) => sum + shot.duration,
      0,
    );
  }
  ...
  advance(seconds) {
    if (this.phase !== 'playing' || !Number.isFinite(seconds) || seconds < 0)
      return;
  ...
  get frame() {
    return this.phase === 'ready'
      ? { id: 'ready', from: 'deskBefore', progress: 0, blend: 0 }
      : upgradeTourFrame(this.elapsed, this.interiorReveal);
  }
}
...
export function upgradeTourCameras(
  ...
  const bounds = [
    townCenterBounds(entrance, plan.before),
    townCenterBounds(entrance, plan.after),
  ];
