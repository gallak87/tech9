// ─────────────────────────────────────────────────────────────────────────────
// progression — LANE STUB.
//
// XP, levels, three gear slots per hero, skill trees, quests, localStorage save,
// and the seven-tab pause overlay ported from the prototype.
//
// Phase 0 ships the seam, not the feature. `main.js` already installs this and
// hands it the shared `ctx`; the progression lane fills it in without touching a
// single shared-core file. Do not widen this signature — add fields to the
// returned API instead.
//
// Owned by the `progression` lane. See CONTRACT.md §1.
// ─────────────────────────────────────────────────────────────────────────────

export function installProgression(ctx) {
  return {
     ...
     *  `?showcase=progression` boots straight into it and the critic reviews the
