...
test('all three indoor upgrade rehearsals preserve civilization, expedition, and real saves', () => {
  ...
  for (const level of [1, 2, 3]) {
    ...
    for (const frame of [plan.before, plan.after]) {
      assert.equal(
        frame.tier,
        2,
        'An art rehearsal does not advance civilization or unlock vendors',
      );
      ...
      assert.equal(frame.x, state.x);
      assert.equal(frame.y, state.y);
  ...
  for (const invalid of [0, 4, 2.5, '2', NaN])
    assert.throws(() => upgradePreviewStates(state, invalid));
});

test('upgrade tour starts at the indoor desk, visits before/after outside, and returns inside', () => {
  const tour = new UpgradeTour();
  assert.equal(tour.phase, 'ready');
  ...
  while (tour.phase === 'playing') {
    ...
    if (frame.id === 'inside')
      assert.equal(
        frame.to,
        'deskAfter',
        'Return directly to the opening indoor composition',
      );
    if (frame.id === 'interior')
      assert.equal(
        frame.from,
        'deskAfter',
        'Hold that same composition, with no unrelated overview',
      );
    tour.advance(0.025);
  }
  ...
  assert.equal(tour.frame.from, 'deskAfter');
...
test('skip and replay work from every reveal segment and invalid time cannot poison the tour', () => {
  for (const interiorReveal of [false, true])
    for (const elapsed of [0, 0.7, 1.3, 2, 3.2, 4.2, 4.5, 4.8, 5, 6.4]) {
      const tour = new UpgradeTour({ interiorReveal });
      tour.skip();
      assert.equal(tour.phase, 'ready');
      tour.play();
      tour.advance(elapsed);
      tour.skip();
      assert.equal(tour.phase, 'complete');
      assert.equal(tour.frame.from, 'deskAfter');
      tour.play();
      assert.equal(tour.phase, 'playing');
      assert.equal(tour.elapsed, 0);
...
test('restoration returns to the old hall, holds for comparison, and dissolves once into the new hall', () => {
  const tour = new UpgradeTour({ interiorReveal: true });
  tour.play();
  const seen = [],
    dissolve = [];
  while (tour.phase === 'playing') {
    ...
    if (frame.id === 'inside')
      assert.equal(
        frame.to,
        'deskBefore',
        'The return fade must not reveal the renovated hall early',
      );
    if (frame.id === 'interiorBefore') assert.equal(frame.from, 'deskBefore');
    if (frame.id === 'interiorUpgrade') {
      assert.equal(frame.from, 'deskBefore');
      assert.equal(frame.to, 'deskAfter');
      assert.equal(frame.duration, 0.5);
      assert.equal(
        frame.transition,
        undefined,
        'Compare the room through a dissolve',
      );
  ...
  assert.ok(
    dissolve.length > 1 && dissolve[0] < 0.05 && dissolve.at(-1) > 0.95,
  );
  assert.ok(dissolve.every((value, i) => i === 0 || value > dissolve[i - 1]));
  assert.equal(tour.frame.from, 'deskAfter');
  assert.equal(tour.elapsed, tour.duration);
});

test('paired exterior shots fit one camera and the staged crew fits the indoor desk shot', () => {
  const outside = getScene('haventide'),
    inside = getScene('haventide_town');
  const state = createState();
  recruit(state, 'vex');
  recruit(state, 'rune');
  ...
  for (const fromLevel of [1, 2, 3]) {
    const plan = upgradePreviewStates(state, fromLevel),
      camera = upgradeTourCameras(outside, inside, plan);
    for (const visual of [plan.before, plan.after]) {
      ...
      assert.ok(
        bounds.top + bounds.height - camera.outside.y < 517,
        'Foundation clears the letterbox',
      );
    }
    for (const actor of camera.actors) {
      assert.ok(
        isWalkable(inside, actor.x, actor.y),
        'Staged party stands on reachable floor',
      );
