...
export const ORBITAL_REACH_TOWN_CENTER = {
  id: 'orbital_reach_town_center',
  region: 'orbital_reach',
  name: 'Orbital Reach',
...
export function townCenterBounds(object, state) {
  ...
  if (
    object.id !== actualRegion + '_entrance' ||
    !centersByRegion.has(actualRegion)
  )
    return null;
  ...
  return {
    ...
    frameIndex: level - 1,
    scale,
    left: object.x - frame.anchorX * scale,
    top: object.y - frame.anchorY * scale,
...
export function townCenterPreviewBounds(entrance) {
  const bounds = TOWN_CENTERS.flatMap((center) =>
    center.metadata.frames.map((_, i) =>
      townCenterBounds(entrance, {
        buildings: { town_center: i + 1 },
        townCenterArtRegion: center.region,
      }),
    ),
  );
  ...
  return {
    left,
    top,
    width: Math.max(...bounds.map((b) => b.left + b.width)) - left,
    height: Math.max(...bounds.map((b) => b.top + b.height)) - top,
  };
}
