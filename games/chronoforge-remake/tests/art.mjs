...
const animation=await page.evaluate(async()=>{
 ...
 const {HERO_IDLE_SHEETS}=await import('./src/hero-idle.js');
 for(const id of ['kaida','vex','rune'])for(const state of ['idle','walk','run','attack','cast','hurt']){
  const idle=state==='idle'?HERO_IDLE_SHEETS[id].layout:null,fps=state==='run'?13:state==='attack'?10:8;
  const times=idle?idle.sequence.map((_,i)=>(i+.1)*idle.beatDuration):Array.from({length:6},(_,i)=>(i+.1)/fps);
  const hashes=[];for(const time of times){ctx.clearRect(0,0,128,128);drawHero(ctx,id,state,64,116,1.4,'right',time);const px=ctx.getImageData(0,0,128,128).data;let hash=2166136261;for(const n of px)hash=Math.imul(hash^n,16777619);hashes.push(hash>>>0);}
  records.push({id,state,distinctFrames:new Set(hashes).size,requiredFrames:idle?new Set(idle.sequence).size:4});
 }return records;
});
assert(animation.every(r=>r.distinctFrames>=r.requiredFrames),JSON.stringify(animation));
