...
export function balanceMaterialLighting(data,width,height) {
 ...
 for(let y=0;y<height;y++)for(let x=0;x<width;x++){
  const i=(y*width+x)*4,luma=data[i]*.2126+data[i+1]*.7152+data[i+2]*.0722;
  columns[x]+=luma/height;rows[y]+=luma/width;total+=luma;
 }
 const mean=total/(width*height);if(mean<1)return;
 const soften=values=>Array.from(values,(_,i)=>{
  let sum=0;for(let j=-8;j<=8;j++)sum+=values[Math.max(0,Math.min(values.length-1,i+j))];
  return sum/17;
 });
 const xs=soften(columns),ys=soften(rows);
 for(let y=0;y<height;y++)for(let x=0;x<width;x++){
  const gain=Math.max(.75,Math.min(1.33,mean*mean/Math.max(1,xs[x]*ys[y])));
  const i=(y*width+x)*4;
...
export function forestShoreMix(distance,x,y,water) {
 const irregular=Math.sin(x*.071+Math.cos(y*.053))*3+Math.cos(y*.091-x*.029)*2;
 const d=Math.max(0,distance+irregular);
 return water?{
  shallows:1-smooth((d-10)/34),
  roots:1-smooth(d/15),
  shade:(1-smooth(d/20))*.2,
 }:{shallows:0,roots:(1-smooth(d/25))*.74,shade:(1-smooth(d/22))*.17};
}
