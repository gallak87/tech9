...
try{
 ...
 await page.evaluate(()=>{window.__ECHO__.preset('party');const g=window.__ECHO__.game;g._reviewUpdate=g.update.bind(g);g.update=()=>{};g.ui.toggleMenu();});
 for(const [width,height]of [[1440,810],[1024,640]]){
  await page.setViewportSize({width,height});
  for(let i=0;i<7;i++){
   await page.keyboard.press(String(i+1));await page.waitForTimeout(150);
   report.menus.push(await page.evaluate(({width,i})=>{const body=document.querySelector('.atlas-body'),root=document.querySelector('#game');return {width,tab:i,actual:window.__ECHO__.game.ui.tab,bodyWidth:body.clientWidth,bodyScroll:body.scrollWidth,menuFont:getComputedStyle(body).fontFamily,screenWidth:root.clientWidth,documentWidth:document.documentElement.clientWidth,documentScroll:document.documentElement.scrollWidth};},{width,i}));
 ...
 await page.evaluate(()=>{const g=window.__ECHO__.game;g.state.settings.minimap=false;g.ui.updateHUD();});await shot('hud-minimap-off');
 assert.equal(await page.locator('#minimap').count(),0);assert.equal(await page.locator('.location-plaque').count(),1);
 await page.evaluate(()=>{const g=window.__ECHO__.game;g.state.settings.minimap=true;g.update=g._reviewUpdate;g.ui.updateHUD();});
 ...
 await page.evaluate(()=>{window.__ECHO__.goto('haventide');window.__ECHO__.interact('hav_camp');});await shot('rest-selection');
 ...
 await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue([{speaker:'Kaida',text:'One road. One pair of boots.'},{speaker:'Vex',text:'And a few questions worth carrying.'}]));await shot('dialogue');
 ...
 await page.evaluate(()=>{window.__ECHO__.preset('battle');const g=window.__ECHO__.game;g.update=()=>{};for(let i=0;i<2400&&!g.battle.selectedHero;i++)g._reviewUpdate(1/60);});await shot('battle-crew');
 ...
 report.battle=await page.evaluate(()=>({mode:window.__ECHO__.game.battle.mode,stage:document.querySelector('#battle-interface').dataset.stage,font:getComputedStyle(document.querySelector('#battle-interface')).fontFamily}));
