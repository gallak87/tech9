...
const enemies = [
  ...Object.values(EnemyArt),
  ...Object.values(MidEnemyArt),
  ...Object.values(BossArt),
].filter((a) => a?.id && a?.source && a?.frames);
...
export const ASSET_MANIFEST = [
  ...
  atlas('kaida_showcase', 'kaida-showcase-source.png', 6, 2, 'kaida', {
    key: 'neutral-exterior',
  }),
  atlas('coast_props', 'coast-props-source.png', 4, 2, 'environment', {
    biome: 'coast',
  }),
  atlas('coast_ground', 'coast-ground-source.png', 3, 2, 'ground', {
    biome: 'coast',
  }),
  atlas('emberline_props', 'emberline-props-source.png', 4, 2, 'environment', {
    biome: 'desert',
    key: 'neutral-exterior',
    backgroundSeeds: [
      [258, 78],
  ...
  atlas('emberline_ground', 'emberline-ground-source.png', 3, 2, 'ground', {
    biome: 'desert',
  }),
  atlas(
    'interior_ground',
    'interior-ground-source.png',
    3,
    2,
    'interiorGround',
  ),
  ...
  atlas('civic_culture', 'civic-culture-source.png', 4, 4, 'building', {
    group: 'culture',
    key: 'neutral-exterior',
    backgroundSeeds: [
      [1098, 1082],
  ...
  atlas(
    'haventide_civilians',
    'haventide-civilians-source.png',
    6,
    2,
    'civilian',
    { key: 'neutral-exterior' },
  ),
  atlas('rust_scrapper', 'rust-scrapper-source.png', 3, 2, 'enemy', {
    metadata: { width: 128, height: 112, anchorX: 0.5, anchorY: 0.86 },
  }),
  ...ENVIRONMENT_ASSETS.map((entry) => ({
    ...entry,
    url: entry.source,
    required: true,
  })),
  ...WORLD_PROP_ASSETS.map((entry) => ({
    ...entry,
    url: entry.source,
    required: true,
  })),
  ...
  ...heroes.map((metadata) => ({
    id: metadata.id,
    url: metadata.source || `assets/${metadata.id}-source.png`,
    columns: metadata.columns || 6,
    rows: metadata.rows || 2,
    kind: 'hero',
    required: true,
    key: metadata.preserveSourceAlpha
      ? undefined
      : metadata.key || 'neutral-exterior',
    keyMin: metadata.keyMin,
    backgroundSeeds: metadata.backgroundSeeds,
    metadata,
  })),
  ...enemies.map((metadata) => ({
    id: metadata.id,
    url: metadata.source,
    columns: metadata.columns || 3,
    rows: metadata.rows || 2,
    kind: 'enemy',
    required: true,
    key: metadata.key,
...
export function keyNeutralExterior(
  ...
  width,
  height,
  { keyMin = 175, backgroundSeeds = [], keyZones = [] } = {},
) {
...
export async function loadPixelAtlas(entry) {
  ...
  if (entry.metadata?.frames) {
    ...
    if (
      (m.sourceWidth && m.sourceWidth !== image.width) ||
      (m.sourceHeight && m.sourceHeight !== image.height)
    )
      throw Error(`Source dimensions changed: ${entry.id}`);
