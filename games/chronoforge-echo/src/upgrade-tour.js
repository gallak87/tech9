...
export function mountUpgradeTour(
  game,
  ...
  chrome.className = 'upgrade-chrome';
  chrome.innerHTML =
    previewMarkup ??
    `
  ...
  function refreshChrome(visibleFrame, fade = 0) {
    const shot = tour.frame,
      key = tour.phase + ':' + shot.id + ':' + visibleFrame;
    root.querySelector('.upgrade-caption').style.opacity = String(1 - fade);
    if (shown === key) return;
    shown = key;
    for (const pane of root.querySelectorAll('[data-tour-phase]'))
      pane.hidden = pane.dataset.tourPhase !== tour.phase;
    const exterior = visibleFrame.startsWith('outside'),
      returned = [
        'inside',
        'interiorBefore',
        'interiorUpgrade',
    ...
    root.querySelector('.upgrade-caption span').textContent = exterior
      ? 'THE TOWN WE ARE BUILDING'
      : returned
        ? 'A STRONGER HOME'
        : 'A PLACE TO COME HOME TO';
    root.querySelector('.upgrade-caption strong').textContent = exterior
      ? `Town Center · Level ${visibleFrame === 'outsideAfter' ? plan.toLevel : plan.fromLevel}`
      : `${townName} · Settlement hall`;
    root.dataset.phase = tour.phase;
    if (preview && tour.phase === 'complete')
      root.querySelector('[data-tour="done"]').focus({ preventScroll: true });
  }
  function draw() {
    const shot = tour.frame,
      reducedMotion = game.state.settings.reducedMotion;
    const isFade = shot.transition === 'fade';
    const visibleFrame =
      shot.to && (isFade ? shot.progress >= 0.5 : shot.blend >= 0.5)
        ? shot.to
        : shot.from;
    const fade = isFade ? 1 - Math.abs(shot.progress * 2 - 1) : 0;
    function layer(frame, alpha = 1) {
      const scale = upgradeShotScale(shot, frame, reducedMotion),
        pivot = frame.startsWith('outside') ? frames.anchor : frames.deskAnchor;
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.translate(pivot.x, pivot.y);
      ctx.scale(scale, scale);
      ctx.translate(-pivot.x, -pivot.y);
      ctx.drawImage(frames[frame], 0, 0, W, H);
      ctx.restore();
    }
    ctx.globalAlpha = 1;
    ...
    if (isFade) layer(visibleFrame);
      ...
      layer(shot.from);
      if (shot.to) layer(shot.to, shot.blend);
    }
    if (['upgrade', 'interiorUpgrade'].includes(shot.id) && !reducedMotion) {
      const exterior = shot.id === 'upgrade',
        anchor = exterior ? frames.anchor : { x: 0, y: 0 };
      const elapsed = shot.progress * shot.duration;
      const particles = exterior
        ? upgradeSparkles(elapsed)
        : interiorUpgradeSparkles(elapsed, frames.interiorTargets);
      ctx.save();
      for (const particle of particles) {
        const x = anchor.x + particle.x,
          y = anchor.y + particle.y;
        if (!exterior && (x < 12 || x > W - 12 || y < 44 || y > H - 28))
          continue;
    ...
    ctx.fillStyle = '#0b141ac9';
    ctx.fillRect(0, 0, W, 39);
    ctx.fillRect(0, H - 23, W, 23);
    refreshChrome(visibleFrame, fade);
  }
  function tick(now) {
    if (!tour) return;
    const delta = previousTime ? Math.min(0.1, (now - previousTime) / 1000) : 0;
    previousTime = now;
    ...
    if (
      !levelSound &&
      tour.phase !== 'ready' &&
      ((shot.id === 'upgrade' && shot.progress >= 0.72) ||
        [
          'exterior',
          'inside',
          'interiorBefore',
  ...
  function open(nextPlan, done = null) {
    ...
    const interiorTargets = inside.objects
      .filter((o) => o.service && o.havenPart)
      .map((o) => {
        const b = havenInteriorBounds(o, plan.after);
        const x = Math.max(16, b.left - cameras.desk.x),
          y = Math.max(52, b.top - cameras.desk.y);
        return {
          x,
          y,
          width: Math.min(W - 16, b.left + b.width - cameras.desk.x) - x,
          height: Math.min(H - 30, b.top + b.height - cameras.desk.y) - y,
        };
      })
    ...
    tour = new UpgradeTour({ interiorReveal });
    root.hidden = false;
    shown = '';
    if (preview) {
      root.querySelector('[data-tour-phase="ready"] h2').textContent =
        `Town Center · ${plan.fromLevel} → ${plan.toLevel}`;
      draw();
      root.querySelector('[data-tour="play"]').focus({ preventScroll: true });
    } else {
      root.querySelector('[data-town-name]').textContent =
        townName + ' · Town Center upgrade';
      play();
    }
  }
