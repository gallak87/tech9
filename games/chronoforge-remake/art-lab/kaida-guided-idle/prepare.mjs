...
save('reference.png',cell,cell,reference);
const W=1536,H=1024,scale=4,offsetX=22,offsetY=156;
...
for(let col=0;col<2;col++)for(let y=0;y<cell*scale;y++)for(let x=0;x<cell*scale;x++){const src=(Math.floor(y/scale)*cell+Math.floor(x/scale))*4,dst=((y+offsetY)*W+col*768+x+offsetX)*4;reference.copy(board,dst,src,src+4);}
save('edit-target.png',W,H,board);
...
for(const [col,pose] of [rest,inhale].entries()){
...
line([768,40],[768,984],muted,2);save('pose-guides.png',W,H,guide);
writeFileSync(new URL('guide-landmarks.json',here),JSON.stringify({canvas:[W,H],cellWidth:768,sourceCell:[cell,cell],scale,offset:[offsetX,offsetY],rest,inhale,notes:'Visual reference only; not an editing mask or actual ControlNet conditioning. Gold marks are invariant anchors. Guide overlays must not appear in the generated art.'},null,2)+'\n');
console.log('Prepared original reference, duplicated edit target, and rest/inhale pose guides.');
