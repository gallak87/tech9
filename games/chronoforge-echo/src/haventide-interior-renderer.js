...
export function keyInteriorFrame(data,width,height,{keyMin=145,backgroundSeeds=[]}={}) {
  const seen=new Uint8Array(width*height),queue=new Int32Array(width*height);
  let head=0,tail=0;
  const seed=i=>{
    if(seen[i])return;
    seen[i]=1;const k=i*4,hi=Math.max(data[k],data[k+1],data[k+2]),lo=Math.min(data[k],data[k+1],data[k+2]);
    if(data[k+3]===0||(lo>=keyMin&&hi-lo<18)){queue[tail++]=i;data[k+3]=0;}
  };
  for(let x=0;x<width;x++){seed(x);seed((height-1)*width+x);}
  for(let y=0;y<height;y++){seed(y*width);seed(y*width+width-1);}
  for(const [x,y] of backgroundSeeds)if(x>=0&&y>=0&&x<width&&y<height)seed(Math.floor(y)*width+Math.floor(x));
  while(head<tail){const i=queue[head++],x=i%width;if(x)seed(i-1);if(x+1<width)seed(i+1);if(i>=width)seed(i-width);if(i+width<height*width)seed(i+width);}
}
...
export function installHaventideInterior(image,entry) {
  ...
  for(const frame of entry.metadata.frames){
    ...
    if(frame.key!==false){
      ...
      keyInteriorFrame(pixels.data,frame.w,frame.h,frame);context.putImageData(pixels,0,0);
    }
...
function selectedPart(object,state) {
  return object.unlockTier&&state.tier<object.unlockTier?'storage':object.havenPart;
}
...
export function drawHaventideFloor(context,scene,camera,state) {
  ...
  const runner=sheet.frames.get('runner'),width=HAVENTIDE_HALL.runner.width;
  const height=Math.min(660,width*runner.height/runner.width),x=HAVENTIDE_HALL.runner.x-width/2,y=HAVENTIDE_HALL.runner.y-height;
  c.drawImage(runner,x,y,width,height);
  c.restore();return true;
}
export function haventideInteriorMetrics() {
  return {sources:[...sheets.values()].map(s=>s.image),frames:[...sheets.values()].flatMap(s=>[...s.frames.values(),s.tile])};
}
