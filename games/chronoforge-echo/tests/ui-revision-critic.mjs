...
const base=new URL('../',import.meta.url).pathname,state=JSON.parse(await fs.readFile(reviewRoot + 'earned-campaign-saves.json','utf8')).saves['chapter-aftermath'].state;
...
try{
 ...
 for(const tab of ['2','5']){
  ...
  await page.screenshot({path:reviewRoot + 'critic-revised-keyboard-tab-'+tab+'.png'});
 }
 assert.equal(report.party.visible,true,'Rune equipment visible without scrolling');
 await page.keyboard.press('6');report.saveText=await page.locator('.save-slot').first().innerText();assert.match(report.saveText,/Haventide/);assert.doesNotMatch(report.saveText,/haventide_town/);await page.screenshot({path:reviewRoot + 'critic-ui-save-display.png'});await page.keyboard.press('Escape');
 ...
 await page.locator('[data-do="hero:0"]').focus();await page.keyboard.press('Enter');await page.keyboard.press('Home');await page.screenshot({path:reviewRoot + 'critic-ui-vendor-top.png'});const initial=await sample();await page.keyboard.press('End');const end=await sample();assert.ok(end.top>initial.top,'Service End scrolls');report.vendorScroll={initial,end};await page.screenshot({path:reviewRoot + 'critic-ui-vendor-comparisons.png'});
 assert.deepEqual(errors,[]);report.result='pass';
}catch(e){report.result='fail';report.failure=String(e);process.exitCode=1;}finally{await fs.writeFile(reviewRoot + 'ui-revision-critic.json',JSON.stringify(report,null,2));console.log(JSON.stringify({result:report.result,failure:report.failure,party:report.party,tabs:report.tabs,saveText:report.saveText,heroes:report.vendor?.map(v=>({hero:v.hero,lossRows:v.rows.filter(r=>r.expectedLostStrength).length})),errors}));await browser.close();}
