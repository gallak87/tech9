...
export function navigateExpedition(ui, key) {
  ...
  if (active?.closest('.exp-gear')) {
    const slots = buttons('.exp-gear-slot'),
      i = slots.indexOf(active);
    if (key === 'ArrowUp') i === 0 ? focus(hero()) : move(slots, -1);
    if (key === 'ArrowDown') move(slots, 1);
    if (key === 'ArrowRight')
      focus(
        root.querySelector('.exp-pack-row.selected .exp-pack-item') ||
          buttons('.exp-pack-item')[0],
      );
    return true;
  }
  ...
  if (row) {
    const list = buttons('.exp-pack-item'),
      i = list.findIndex((b) => b.closest('.exp-pack-row') === row);
    if (key === 'ArrowUp') i === 0 ? focus(hero()) : focus(list[i - 1]);
    if (key === 'ArrowDown') focus(list[Math.min(list.length - 1, i + 1)]);
    if (key === 'ArrowRight')
      focus(row.querySelector('.exp-pack-action:not(:disabled)'));
    if (key === 'ArrowLeft')
      focus(
        active.matches('.exp-pack-action')
          ? row.querySelector('.exp-pack-item')
          : buttons('.exp-gear-slot')[0] || hero(),
      );
    return true;
  }
  return false;
