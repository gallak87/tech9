...
const source=read(readFileSync(new URL('../../assets/vex.png',here))),cell=181;
if(source.w!==cell*6||source.h!==cell*8)throw Error('Vex atlas dimensions changed; update guide landmarks first.');
...
save('original-first-cell.png',cell,cell,reference);
...
const rest={head:[97,49],neck:[94,65],shoulderL:[77,70],shoulderR:[110,69],elbowL:[78,91],elbowR:[112,87],wristL:[73,112],wristR:[126,85],hipL:[84,105],hipR:[106,105],kneeL:[83,137],kneeR:[109,137],ankleL:[77,160],ankleR:[114,160],toeL:[83,166],toeR:[126,166],staffTop:[135,35],staffFoot:[121,165]};
...
for(const [col,pose] of [rest,halfway,inhale].entries()){
 ...
 line([col*768+50,820],[col*768+718,820],muted,2);
 for(const [a,b]of limbs)line(p(a),p(b),cyan,5);
 for(const name of Object.keys(pose).filter(name=>!['staffTop','staffFoot'].includes(name)))disc(...p(name),6,cyan);
 line(p('staffTop'),p('staffFoot'),pink,5);
 for(const name of ['toeL','toeR','wristL','wristR','hipL','hipR','staffTop','staffFoot']){const [x,y]=p(name);line([x-12,y],[x+12,y],gold,3);line([x,y-12],[x,y+12],gold,3);}
}
