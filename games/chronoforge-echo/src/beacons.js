...
export const BEACONS = {
  hav_beacon: {
    name: 'The listening beacon', restoredFlag: 'beacon_restored', gate: 'hav_guard',
  ...
  frost_beacon: {
    name: 'The midnight beacon', restoredFlag: 'frost_seal', gate: 'frost_colossus',
...
export function beaconStatus(state, id) {
  ...
  const inspected = lit || !!state.flags?.[id + '_inspected'] ||
    (id === 'hav_beacon' && !!state.flags?.opening_seen);
  return {
    ...definition, lit, ready, inspected,
    hint: !inspected ? '' : lit ? definition.restoredHint : ready ?
      'Path clear · Interact to light the beacon.' : definition.instruction,
  };
}
