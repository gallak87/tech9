...
function finish(battle, state) {
  for (let i = 0; battle.action && i < 200; i++)
    updateBattle(battle, state, 0.025);
...
test('encounters contain one to four distinct, independently identified enemies; invalid required content fails visibly', () => {
  ...
  for (let n = 1; n <= 4; n++) {
    ...
    assert.equal(
      new Set(battle.enemies.map((e) => `${e.home.x},${e.home.y}`)).size,
      n,
    );
  }
  ...
  assert.throws(
    () => setup(['unknown_enemy']),
    /Required enemy content missing/,
  );
});
...
test('readiness queue orders actual threshold arrivals; a later arrival cannot steal focus; Tab is deliberate', () => {
  ...
  b.heroes[0].atb = 99;
  b.heroes[1].atb = 99.99;
  b.heroes[2].atb = 99.9;
  updateBattle(b, s, 0.1);
  assert.deepEqual(b.readyQueue, ['vex', 'rune', 'kaida']);
  assert.equal(b.selectedHero, 'vex');
  updateBattle(b, s, 0.3);
  assert.equal(b.selectedHero, 'vex');
  battleKey(b, s, 'Tab');
  assert.equal(b.selectedHero, 'rune');
  b.heroes[2].hp = 0;
  updateBattle(b, s, 0.01);
...
test('command and target choices pause every gauge; Escape belongs exclusively to global pause', () => {
  ...
  updateBattle(b, s, 20);
  assert.deepEqual(
    [...b.heroes, ...b.enemies].map((a) => a.atb),
    gauges,
  );
...
test('held and repeated keydowns cannot skip choices or reuse the execute press as timing input', () => {
  ...
  ready(b, s);
  const down = (repeat) =>
    battleKey(b, s, { key: 'Enter', type: 'keydown', repeat });
...
test('timing offers one fresh attempt, boosts probability without guaranteeing critical, and resolves damage once', () => {
  ...
  contact(b, s);
  assert.ok(a.criticalChance > 0.45 && a.criticalChance < 1);
  const after = b.enemies[0].hp;
  assert.ok(after < before);
  updateBattle(b, s, 0.05);
  assert.equal(b.enemies[0].hp, after);
  assert.equal(a.timingAttempted, true);
  const missed = setup(['gravbot']);
  ready(missed.battle, missed.state);
  attack(missed.battle, missed.state);
  battleKey(missed.battle, missed.state, ' '); // too early: the single attempt is spent
  const ma = missed.battle.action;
  updateBattle(
    missed.battle,
    missed.state,
    (ma.windowStart + ma.windowEnd) / 2,
  );
  battleKey(missed.battle, missed.state, 'Enter');
  assert.equal(ma.timingSuccess, false);
  contact(missed.battle, missed.state);
  assert.ok(ma.criticalChance < 0.2);
});

test('timing assistance widens the opportunity; identical seeds and fresh presses produce identical outcomes', () => {
  const normal = setup(['gravbot']),
    assisted = setup(['gravbot']);
  assisted.state.settings.timingAssist = true;
  for (const setup of [normal, assisted]) {
    ready(setup.battle, setup.state);
    attack(setup.battle, setup.state);
  }
  assert.ok(
    assisted.battle.action.windowEnd - assisted.battle.action.windowStart >
      normal.battle.action.windowEnd - normal.battle.action.windowStart,
  );
  const twin = setup(['gravbot']);
  ready(twin.battle, twin.state);
  attack(twin.battle, twin.state);
  finish(twin.battle, twin.state);
  finish(normal.battle, normal.state);
...
test('the accepted timing position freezes through recovery and save/resume for attacks and both guards', () => {
  for (const kind of ['attack', 'defend', 'incoming'])
    for (const phase of ['early', 'window', 'late']) {
      const { battle: b, state: s } = setup(['rust_scrapper']);
      if (kind === 'incoming') {
        b.enemies[0].atb = 100;
        updateBattle(b, s, 0.001);
      } else {
        ready(b, s);
        if (kind === 'attack') attack(b, s);
        else battleIntent(b, s, { kind: 'command', index: 2 });
      }
      const a = b.action;
      ...
      updateBattle(resumed, s, 0.01);
      battleKey(resumed, s, 'Enter');
      assert.equal(
        resumed.action.timingPressedAt,
        pressedAt,
        'a second press cannot move the notch',
      );
      contact(resumed, s);
      updateBattle(resumed, s, 0.1);
      assert.ok(resumed.action.elapsed > pressedAt);
      assert.equal(
        battleView(resumed, s).action.timingPressedAt,
        pressedAt,
        'contact/recovery keep the accepted position',
      );
      const legacy = structuredClone(resumed);
      delete legacy.action.timingPressedAt;
      assert.equal(
        battleView(legacy, s).action.timingPressedAt,
        null,
        'older saves do not invent an input position',
      );
      finish(resumed, s);
      assert.equal(battleView(resumed, s).action, null);
    }
});

test('only successful timed hero critical damage is reduced; outside-window criticals and ordinary hits are unchanged', () => {
  for (const [seed, critical] of [
    [1, true],
    [123456789, false],
  ]) {
    const results = {};
    for (const phase of ['none', 'early', 'window', 'late']) {
      ...
      s.rng = seed;
      b.enemies[0].hp = b.enemies[0].maxHp = 10000;
      if (phase !== 'none') {
        const elapsed =
          phase === 'early'
            ? 0
            : phase === 'window'
              ? (b.action.windowStart + b.action.windowEnd) / 2
              : b.action.windowEnd + 0.02;
    ...
    if (critical) {
      ...
      assert.ok(
        Math.abs(results.window.damage - results.none.damage * 0.9) <= 1,
        'successful timed crit deals 90% of the prior crit, allowing integer rounding',
      );
    } else
      assert.deepEqual(
        results.window,
        results.none,
        'a successful timing input that does not crit keeps normal damage',
      );
  }
});
...
test('a paused timeline, contact, and timing window remain exact until updateBattle resumes', () => {
  ...
  ready(b, s);
  attack(b, s);
  updateBattle(b, s, b.action.windowStart + 0.01);
  ...
  assert.equal(b.action.timingSuccess, true);
  updateBattle(b, s, 0.01);
  assert.equal(b.action.elapsed, b.action.windowStart + 0.02);
});
...
test('single and group healing target only living allies and persist at the authoritative contact', () => {
  ...
  assert.equal(b.heroes[2].hp, 0);
  finish(b, s);
  ready(b, s, ['vex']);
  b.selectedHero = 'vex';
  tech(b, s, 'lantern_mend');
  battleKey(b, s, 'Enter');
  assert.deepEqual(b.action.targets, ['kaida', 'vex']);
  contact(b, s);
  assert.ok(b.heroes[0].hp > 40);
});
...
test('all pair techniques and the full-party technique enforce readiness and spend each participant once', () => {
  for (const id of [
    ...
    for (let i = 0; i < b.heroes.length; i++) {
      assert.equal(
        b.heroes[i].mp,
        mp[i] - (members.includes(b.heroes[i].id) ? TECHS[id].mp : 0),
      );
      if (members.includes(b.heroes[i].id)) assert.equal(b.heroes[i].atb, 0);
    }
    const targetHp = b.enemies.map((e) => e.hp);
    contact(b, s);
    if (TECHS[id].effect === 'damage')
      assert.ok(b.enemies.some((e, i) => e.hp < targetHp[i]));
  }
});
...
test('shield, drain, and slow have combat effects; defense never requests an enemy', () => {
  ...
  ready(b, s, ['rune']);
  b.selectedHero = 'rune';
  tech(b, s, 'aegis_field');
  battleKey(b, s, 'Enter');
  contact(b, s);
  assert.ok(b.heroes.every((h) => h.shield > 0));
  finish(b, s);
  const hp = b.heroes.reduce((n, h) => n + h.hp, 0),
    ward = b.heroes.reduce((n, h) => n + h.shield, 0);
  const unwarded = structuredClone(b),
    unwardedState = structuredClone(s);
  for (const h of unwarded.heroes) h.shield = 0;
  unwarded.enemies[0].atb = 100;
  updateBattle(unwarded, unwardedState, 0.001);
  contact(unwarded, unwardedState);
  b.enemies[0].atb = 100;
  updateBattle(b, s, 0.001);
  contact(b, s);
  assert.ok(b.heroes.reduce((n, h) => n + h.shield, 0) < ward);
  assert.ok(
    b.heroes.reduce((n, h) => n + h.hp, 0) >
      unwarded.heroes.reduce((n, h) => n + h.hp, 0),
  );
  assert.ok(b.heroes.reduce((n, h) => n + h.hp, 0) <= hp);
  finish(b, s);
  giveSkill(s, 'chrono_strike');
  ready(b, s, ['kaida']);
  b.selectedHero = 'kaida';
  tech(b, s, 'chrono_strike');
  battleKey(b, s, 'Enter');
  contact(b, s);
  assert.equal(b.enemies[0].slowTurns, 2);
  finish(b, s);
  b.enemies[0].hp = 1000;
  b.enemies[0].maxHp = 1000;
  giveSkill(s, 'entropy_surge');
  b.heroes[1].hp = 20;
  ready(b, s, ['vex']);
  b.selectedHero = 'vex';
  tech(b, s, 'entropy_surge');
  battleKey(b, s, 'Enter');
  contact(b, s);
  assert.ok(b.heroes[1].hp > 20);
  finish(b, s);
  ready(b, s, ['kaida']);
  b.selectedHero = 'kaida';
  battleKey(b, s, 'Enter');
  battleKey(b, s, 'ArrowDown');
  battleKey(b, s, 'ArrowDown');
  battleKey(b, s, 'Enter');
  assert.equal(b.action.command.kind, 'defend');
  assert.deepEqual(b.action.targets, ['kaida']);
});
...
test('revival uses a consumed field supply and can only target a fallen companion', () => {
  ...
  battleKey(b, s, 'Enter');
  assert.deepEqual(
    battleView(b, s).targets.map((t) => t.id),
    ['vex'],
  );
  battleKey(b, s, 'Enter');
  assert.equal(s.inventory.dawn_seed, 1, 'Supply is held until contact');
  contact(b, s);
  assert.equal(s.inventory.dawn_seed, 0);
  assert.equal(b.heroes[1].hp, Math.round(b.heroes[1].maxHp * 0.35));
  finish(b, s);
  assert.equal(s.inventory.dawn_seed, 0, 'Supply is spent exactly once');
});
...
test('elite telegraphs allow a response; the Void Architect changes phases before its real defeat', () => {
  ...
  b.enemies[0].atb = 100;
  updateBattle(b, s, 0.001);
  assert.equal(b.action.command.effect, 'telegraph');
  contact(b, s);
  assert.equal(b.enemies[0].charging, true);
  finish(b, s);
  assert.equal(b.result, null);
  b.enemies[0].hp = Math.floor(b.enemies[0].maxHp * 0.66);
  ready(b, s);
  attack(b, s);
  contact(b, s);
  assert.equal(b.enemies[0].bossPhase, 2);
  assert.ok(b.enemies[0].shield > 0);
  finish(b, s);
  b.enemies[0].shield = 0;
  b.enemies[0].hp = Math.floor(b.enemies[0].maxHp * 0.32);
  ready(b, s);
  attack(b, s);
  contact(b, s);
  assert.equal(b.enemies[0].bossPhase, 3);
  finish(b, s);
  b.enemies[0].hp = 1;
  b.enemies[0].shield = 0;
  ready(b, s);
  ...
  assert.equal(
    b.result,
    null,
    'The final contact and recovery remain visible.',
  );
  finish(b, s);
  ...
  assert.equal(
    s.campaignComplete,
    false,
    'Narrative ending and reward ownership belong to the integrator.',
  );
});

test('mouse uses the same deliberate targeting and execution rules in native 960×540 coordinates', () => {
  const { battle: b, state: s } = setup(['gravbot', 'mire_hulk']);
  ready(b, s);
  ...
  battleClick(b, s, 300 * 1.25, 335 * 1.25);
  assert.equal(b.mode, 'target');
  b.hitAreas = [
    { x: 580, y: 190, w: 100, h: 100, kind: 'actor', id: 'enemy_1' },
  ];
  battleClick(b, s, 600 * 1.25, 220 * 1.25);
...
test('mouse can page techniques, select an ally through the party panel, cancel, and request global pause', () => {
  ...
  battleClick(b, s, 300 * 1.25, 350 * 1.25);
  assert.equal(b.mode, 'tech');
  b.hitAreas = [{ x: 616, y: 389, w: 17, h: 24, kind: 'scroll', direction: 1 }];
  battleClick(b, s, 620 * 1.25, 399 * 1.25);
  ...
  battleClick(b, s, 460 * 1.25, 335 * 1.25);
  assert.equal(b.mode, 'target');
  ...
  battleClick(b, s, 590 * 1.25, 35 * 1.25);
  assert.equal(b.mode, 'tech');
  const before = JSON.stringify(battleView(b, s));
...
test('defeated enemies hold a visible down pose before fading; result updates advance presentation only', () => {
  ...
  updateBattle(battle, state, 0.5);
  assert.equal(battleView(battle, state).enemies[0].visual.opacity, 0);
  assert.deepEqual(
    battle.heroes.map((h) => ({ hp: h.hp, mp: h.mp, atb: h.atb })),
    resources,
  );
...
test('accordion returns to a muted crew selection after recovery, then auto-selects the next arrival', () => {
  ...
  assert.equal(
    waiting.focusHero,
    'kaida',
    'the last companion retains a faint selection anchor',
  );
...
test('accordion directional keys and breadcrumbs share the real target/cost state', () => {
  ...
  battleKey(b, s, 'ArrowRight');
  assert.equal(b.mode, 'tech');
  battleKey(b, s, 'ArrowLeft');
  assert.equal(b.mode, 'command');
  assert.equal(b.cursor, 1);
  battleKey(b, s, 'ArrowUp');
  battleKey(b, s, 'ArrowRight');
  assert.equal(b.mode, 'target');
  battleKey(b, s, 'ArrowDown');
  assert.equal(b.target, 1);
...
test('Defend shares attack timing, requires a fresh press, and a miss preserves normal guard', () => {
  const a = setup(['gravbot']),
    d = setup(['gravbot']);
  ready(a.battle, a.state);
  attack(a.battle, a.state);
  ready(d.battle, d.state);
  battleKey(d.battle, d.state, 'Enter');
  battleKey(d.battle, d.state, 'ArrowDown');
  battleKey(d.battle, d.state, 'ArrowDown');
  ...
  for (const field of [
    ...
    assert.equal(action[field], a.battle.action[field]);
  ...
  battleKey(d.battle, d.state, { key: 'Enter', type: 'keydown' });
  assert.equal(action.timingSuccess, true);
  contact(d.battle, d.state);
  ...
  updateBattle(missed.battle, missed.state, 0.6);
  battleKey(missed.battle, missed.state, 'Enter');
  contact(missed.battle, missed.state);
...
test('critical guard reduces the next incoming hit by 75%, normal guard by 65%, and expires on next action', () => {
  ...
  const plain = structuredClone(b),
    plainState = structuredClone(s),
    normal = structuredClone(b),
    normalState = structuredClone(s);
  plain.heroes[0].guarding = false;
  normal.heroes[0].criticalGuard = false;
  const hp = b.heroes[0].hp;
  for (const [battle, state] of [
    [b, s],
    [plain, plainState],
    [normal, normalState],
  ]) {
    battle.heroes[0].atb = 0;
    battle.enemies[0].atb = 100;
    updateBattle(battle, state, 0.001);
    contact(battle, state);
  }
  const unguarded = hp - plain.heroes[0].hp;
  assert.equal(
    hp - normal.heroes[0].hp,
    Math.max(1, Math.round(unguarded * 0.35)),
  );
  assert.equal(hp - b.heroes[0].hp, Math.max(1, Math.round(unguarded * 0.25)));
  finish(b, s);
  ready(b, s);
  attack(b, s);
  assert.equal(b.heroes[0].guarding, false);
  assert.equal(b.heroes[0].criticalGuard, false);
});
...
test('incoming attacks offer one fresh critical guard, reduce only that hit, and restore the selected companion', () => {
  ...
  const plain = structuredClone(b),
    plainState = structuredClone(s);
  const hp = b.heroes.map((h) => h.hp),
    gauges = b.heroes.map((h) => h.atb);
  updateBattle(b, s, (a.windowStart + a.windowEnd) / 2);
  ...
  const resumed = JSON.parse(JSON.stringify(b));
  contact(resumed, s);
  contact(plain, plainState);
  assert.equal(
    resumed.action.criticalChance,
    plain.action.criticalChance,
    'guard timing must not boost enemy critical chance',
  );
  for (let i = 0; i < b.heroes.length; i++) {
    const damage = hp[i] - plain.heroes[i].hp;
    assert.equal(
      hp[i] - resumed.heroes[i].hp,
      damage ? Math.max(1, Math.round(damage * 0.25)) : 0,
    );
    assert.equal(
      resumed.heroes[i].guarding,
      false,
      'reactive guard must not become a persistent stance',
    );
  ...
  assert.deepEqual(
    {
      mode: resumed.mode,
      selectedHero: resumed.selectedHero,
  ...
  assert.deepEqual(
    resumed.heroes.map((h) => h.atb),
    gauges,
    'guard reaction does not spend a ready turn',
  );
});
...
test('incoming group guards protect every target without multiplying existing guard or bypassing wards', () => {
  ...
  b.enemies[0].charging = true;
  b.enemies[0].atb = 100;
  updateBattle(b, s, 0.001);
  assert.equal(b.action.targets.length, 3);
  const plain = structuredClone(b),
    plainState = structuredClone(s),
    hp = b.heroes.map((h) => h.hp);
  b.heroes[0].guarding = true;
  b.heroes[0].criticalGuard = true;
  ...
  contact(plain, plainState);
  for (let i = 0; i < b.heroes.length; i++) {
    const raw = hp[i] - plain.heroes[i].hp,
      guarded = Math.max(1, Math.round(raw * 0.25));
    assert.equal(
      hp[i] - b.heroes[i].hp,
      Math.max(0, guarded - (i === 1 ? 10 : 0)),
    );
  }
});
...
test('missing an incoming timing window preserves normal defenses; charge telegraphs cannot be guarded', () => {
  ...
  battleIntent(b, s, { kind: 'timing' });
  assert.equal(
    b.action.timingSuccess,
    false,
    'early input spends the only attempt',
  );
  contact(b, s);
  contact(plain, plainState);
  ...
  charge.battle.enemies[0].atb = 100;
  updateBattle(charge.battle, charge.state, 0.001);
