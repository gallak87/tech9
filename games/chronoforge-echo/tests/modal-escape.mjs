...
const state = () => page.evaluate(() => { const g = window.__ECHO__.game; return { mode: g.mode, panel: g.ui.panel?.type || null, menu: g.ui.menu, blocked: g.ui.blocked, tab: g.ui.tab, line: g.ui.panel?.index, binding: g.ui.bindCapture, scene: g.state.region }; });
const fixture = name => page.evaluate(name => { window.__ECHO__.preset(name); const g = window.__ECHO__.game; g.resetSession(); g.ui.render(); }, name);
async function choose(action) {
  const nav = await page.evaluate(action => { const ui = window.__ECHO__.game.ui, list = ui.focusables(); return { target: list.findIndex(el => el.dataset.do === action), current: list.indexOf(document.activeElement), count: list.length }; }, action);
...
async function service(kind, preset = 'settlement') {
  ...
  const object = await page.evaluate(kind => {
    const g = window.__ECHO__.game; g.state.flags.beacon_restored = true;
...
try {
  ...
  await check('vendor and build atlas layers return then dismiss', async () => {
    ...
    for (const kind of ['provisions', 'construction']) {
      ...
      await page.evaluate(() => { window.retainedPanel = window.__ECHO__.game.ui.panel; });
      ...
      assert.ok(await page.evaluate(() => window.retainedPanel === window.__ECHO__.game.ui.panel));
  ...
  await check('sign F reads actual text without portrait then Escape returns to field', async () => {
    ...
    const screenshot=await capture('southern-branch-reading');await press('Escape');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>JSON.stringify(window.__ECHO__.game.state.flags)),object.flags);
  ...
  await check('ordinary dialogue cancellation never invokes completion callback', async () => {
    await fixture('world');await page.evaluate(()=>{window.finishedDialogue=0;window.__ECHO__.game.ui.showDialogue([{speaker:'Kaida',text:'First line'},{speaker:'Kaida',text:'Last line'}],[],()=>window.finishedDialogue++);});
    assert.equal(await page.locator('.dialogue-portrait').count(),1);await press('Escape');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>window.finishedDialogue),0);
    await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue(['First line','Last line'],[],()=>window.finishedDialogue++));assert.equal(await page.locator('.dialogue-portrait').count(),0);await press('Enter');assert.equal((await state()).line,1);await press('Backspace');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>window.finishedDialogue),0);
    await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue(['First line','Last line'],[],()=>window.finishedDialogue++));await press('Enter');await press('Space');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>window.finishedDialogue),1);
    await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue(['Mouse can leave too.'],[],()=>window.finishedDialogue++));await page.locator('[data-do="dismiss-panel"]').click();assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>window.finishedDialogue),1);
    await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue(['Keyboard can focus Leave.'],[],()=>window.finishedDialogue++));await choose('dismiss-panel');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>window.finishedDialogue),1);
  ...
  await check('choice cancellation makes no selection and changes no quest flags', async () => {
    await fixture('world');await page.evaluate(()=>{window.finishedChoice=0;window.__ECHO__.game.ui.showDialogue(['Choose deliberately.'],[{text:'Keep the promise',flag:'test_modal_choice'}],()=>window.finishedChoice++);});const before=await page.evaluate(()=>JSON.stringify(window.__ECHO__.game.state.flags));await press('Escape');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>JSON.stringify(window.__ECHO__.game.state.flags)),before);assert.equal(await page.evaluate(()=>window.finishedChoice),0);
    await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue(['Choose deliberately.'],[{text:'Keep the promise',flag:'test_modal_choice'}],()=>window.finishedChoice++));await press('Backspace');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>window.finishedChoice),0);assert.equal(await page.evaluate(()=>!!window.__ECHO__.game.state.flags.test_modal_choice),false);
    await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue(['Choose deliberately.'],[{text:'Keep the promise',flag:'test_modal_choice'}],()=>window.finishedChoice++));await choose('choice:0');assert.equal(await page.evaluate(()=>window.finishedChoice),1);assert.equal(await page.evaluate(()=>!!window.__ECHO__.game.state.flags.test_modal_choice),true);
    return {explicitChoiceRequired:true};
  });
  await check('cancelled battle tutorial never starts the pending encounter', async()=>{
    await fixture('world');await page.evaluate(()=>{const g=window.__ECHO__.game;g.beginBattle(g.scene.objects.find(o=>o.id==='hav_guard'));});assert.equal((await state()).panel,'dialogue');await press('Escape');assert.equal((await state()).blocked,false);assert.equal((await state()).mode,'world');assert.equal(await page.evaluate(()=>window.__ECHO__.game.battle),null);return {encounterCallbackNotInvoked:true};
  });
  await check('ending resume never steals battle or transition input', async()=>{
    await fixture('battle');await page.evaluate(()=>{const g=window.__ECHO__.game;g.state.flags.pendingEnding=true;g.state.flags.architect_defeated=true;g.state.endingProgress={index:1,panel:'dialogue'};});await page.waitForFunction(()=>window.__ECHO__.snapshot().battle.selectedHero==='kaida');await press('Enter');assert.equal((await state()).mode,'battle');assert.equal((await state()).panel,null);assert.equal(await page.evaluate(()=>window.__ECHO__.snapshot().battle.mode),'command');await page.evaluate(()=>window.__ECHO__.game.presentEnding());assert.equal((await state()).panel,null);
    await fixture('world');await page.evaluate(()=>{const g=window.__ECHO__.game;g.state.flags.pendingEnding=true;g.transition={time:0,duration:999,swapped:false,to:'emberline',spawn:{x:130,y:1080}};});await press('Enter');assert.equal((await state()).panel,null);await page.evaluate(()=>{window.__ECHO__.game.presentEnding();});assert.equal((await state()).panel,null);await page.evaluate(()=>{window.__ECHO__.game.transition=null;});
  ...
  await check('ending cancellation preserves exact progress and explicit completion', async () => {
    await fixture('final');await page.evaluate(()=>{const g=window.__ECHO__.game;g.battle.encounter.id='void_architect';g.battle.result='victory';g.finishBattle();});await press('Enter');const line=(await state()).line;assert.equal(line,1);await press('Escape');assert.equal((await state()).blocked,false);assert.equal((await state()).panel,null);assert.ok(await page.locator('[data-resume-ending]').isVisible());assert.ok(await page.evaluate(()=>window.__ECHO__.game.state.flags.pendingEnding&&!window.__ECHO__.game.state.campaignComplete&&!window.__ECHO__.game.state.flags.ending_seen));assert.equal(await page.evaluate(()=>window.__ECHO__.game.state.endingProgress.index),line);
    await page.evaluate(()=>{const g=window.__ECHO__.game,o=g.scene.objects.find(o=>o.type==='sign');Object.assign(g.state,g.safePoint(g.scene,o.x,o.y+25));g.near=o;g.updateCamera(true);g.ui.updateHUD();});await press('f');assert.equal((await state()).panel,'reading');await press('Escape');assert.equal(await page.evaluate(()=>window.__ECHO__.game.state.endingProgress.index),line);
    // Unrelated field notes after cancelling must not overwrite ending progress.
    await page.evaluate(()=>window.__ECHO__.game.ui.showDialogue(['One note.','Another note.']));await press('Enter');await press('Enter');assert.equal(await page.evaluate(()=>window.__ECHO__.game.state.endingProgress.index),line);
    await press('Enter');assert.equal((await state()).line,line);const expected=await page.evaluate(()=>window.__ECHO__.game.ui.panel.lines[window.__ECHO__.game.ui.panel.index].text);assert.equal(await page.locator('.dialogue-text').innerText(),expected);await press('Backspace');assert.equal((await state()).blocked,false);
    // Cancellation writes a recoverable checkpoint without granting completion.
    await page.evaluate(()=>window.__ECHO__.game.load('checkpoint'));assert.equal((await state()).line,line);assert.equal(await page.locator('.dialogue-text').innerText(),expected);assert.equal(await page.evaluate(()=>window.__ECHO__.game.state.campaignComplete),false);
    for(let n=0;n<40&&(await state()).panel==='dialogue';n++)await press('Enter');assert.equal((await state()).panel,'ending');await press('Escape');assert.equal((await state()).blocked,false);assert.equal(await page.evaluate(()=>window.__ECHO__.game.state.campaignComplete),false);await press('Enter');assert.equal((await state()).panel,'ending');await press('Backspace');assert.equal((await state()).blocked,false);await page.evaluate(()=>window.__ECHO__.game.load('checkpoint'));assert.equal((await state()).panel,'ending');
    const screenshot=await capture('ending-explicit-return');await choose('ending-continue');await page.waitForFunction(()=>!window.__ECHO__.game.transition);assert.ok(await page.evaluate(()=>window.__ECHO__.game.state.campaignComplete&&window.__ECHO__.game.state.flags.ending_seen&&!window.__ECHO__.game.state.flags.pendingEnding));
  ...
  await check('battle atlas freezes all seven tabs and restores target action', async () => {
    ...
    await press('Enter'); await press('Enter'); assert.equal((await page.evaluate(() => window.__ECHO__.snapshot().battle)).mode, 'target');
    await press('Escape'); const frozen = await page.evaluate(() => window.__ECHO__.snapshot().battle);
    for (let tab = 1; tab <= 7; tab++) { await press(String(tab)); assert.equal((await state()).tab, tab - 1); assert.deepEqual(await page.evaluate(() => window.__ECHO__.snapshot().battle), frozen); }
    await page.waitForTimeout(180); assert.deepEqual(await page.evaluate(() => window.__ECHO__.snapshot().battle), frozen);
    await press('Escape'); assert.equal((await state()).blocked, false); assert.equal((await page.evaluate(() => window.__ECHO__.snapshot().battle)).mode, 'target');
    await press('Enter'); await page.waitForFunction(() => window.__ECHO__.snapshot().battle.action?.elapsed > .1);
    await press('Escape'); const action = await page.evaluate(() => window.__ECHO__.snapshot().battle);
    await page.waitForTimeout(200); assert.deepEqual(await page.evaluate(() => window.__ECHO__.snapshot().battle), action);
    await press('Escape'); await page.waitForFunction(() => window.__ECHO__.snapshot().battle.action?.resolved);
    const resumed = await page.evaluate(() => window.__ECHO__.snapshot().battle);
  ...
  report.runtimeErrors = await page.evaluate(() => window.__ECHO__.snapshot().errors);
