...
// ─────────────────────────────────────────────────────────────────────────────
// PHASE 0 PLACEHOLDER — the `world` lane owns this folder and replaces this
// file wholesale in Tier 1.
//
...
// layout. None of that is Phase 0's business. `heightAt()` and the `world:built`
// event are the contract the later lanes inherit; everything else here is
// scaffolding.
// ─────────────────────────────────────────────────────────────────────────────
...
export function installWorld(ctx) {
  const { engine, materials } = ctx;
  const root = new THREE.Group();
  root.name = 'world';
  engine.scene.add(root);

  root.add(buildTerrain(materials));

  const r = rng('world.phase0.props');
  ...
  const pr = rng('world.phase0.pebbles');
  for (let i = 0; i < 700; i++) {
  ...
  const focus = new THREE.Vector3(cx, heightAt(cx, cz), cz);
  const propFocus = new THREE.Vector3(cx, heightAt(cx, cz) + 1.4, cz + 1.5);

  bus.emit('world:built', { size: SIZE, seed: 'phase0' });

  return {
    root, focus, propFocus,
    size: SIZE,
    heightAt, normalAt,
    /** Phase 0 has nothing to animate. The seam is here so Tier 1 does not have
     *  to touch main.js to get a tick. */
    update(_dt) {},
