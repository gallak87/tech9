...
const snap=()=>page.evaluate(()=>({region:window.__ECHO__.game.state.region,heroes:window.__ECHO__.game.state.heroes.map(h=>h.id),transition:window.__ECHO__.game.transition,queue:window.__ECHO__.game.rewardQueue,keys:[...window.__ECHO__.game.keys],moving:window.__ECHO__.game.moving}));
async function pendingTravel(){await page.evaluate(()=>{const g=window.__ECHO__.game;g.travel('emberline');g.rewards([{id:'ore',amount:3,label:'Earlier expedition notice'}]);g.keys.add('ArrowRight');g.moving=true;});}
try{
  await page.goto('http://127.0.0.1:4321/?test=1');await page.waitForFunction(()=>window.__ECHO_READY__);await page.evaluate(()=>window.__ECHO__.preset('party'));
