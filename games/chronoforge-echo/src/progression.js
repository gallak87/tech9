...
export function stats(hero, state) {
  ...
  for (const key of [
    ...
    result[key] = Math.floor(def[key] + (hero.level - 1) * def.growth[key]);
  for (const itemId of Object.values(hero.equip || {}))
    if (itemId)
      for (const [key, value] of Object.entries(ITEMS[itemId]?.stats || {})) {
        const k = key === 'hp' ? 'maxHp' : key === 'mp' ? 'maxMp' : key;
        result[k] = (result[k] || 0) + value;
      }
  result.str += (state.buildings?.barracks || 0) * 2;
  result.tec += (state.buildings?.barracks || 0) * 2;
  result.def +=
    (state.buildings?.walls || 0) * 3 + (state.buildings?.forge || 0);
  ...
  for (const key of Object.keys(result))
    result[key] = Math.max(key === 'crit' ? 0 : 1, result[key]);
  return result;
}
function makeHero(id, level, state) {
  const def = HEROES[id],
    h = {
      ...
      equip: {
        weapon: def.weapon,
        armor: id === 'kaida' ? 'scrap_vest' : null,
        accessory: null,
      },
      skills: [...def.starters],
    };
...
export function createState() {
  ...
  state.heroes.push(makeHero('kaida', 1, state));
  return state;
}
export function awardXp(state, amount) {
  if (!Number.isFinite(amount) || amount < 0)
    return no('Experience must be a positive amount.');
  const rewards = [];
  for (const hero of state.heroes) {
    hero.xp += Math.floor(amount);
    let gained = 0;
    while (hero.level < 60 && hero.xp >= xpForLevel(hero.level)) {
    ...
    if (gained) {
      const s = stats(hero, state);
      hero.hp = s.maxHp;
      hero.mp = s.maxMp;
      rewards.push({
        id: hero.id,
        label: `${hero.name} · Level ${hero.level}`,
        amount: gained,
      });
    }
  }
  recomputeUnlocks(state);
  return ok(`The crew gained ${Math.floor(amount)} experience.`, [
    { id: 'xp', label: 'Experience', amount: Math.floor(amount) },
    ...rewards,
  ]);
}
export function recruit(state, id) {
  if (!HEROES[id]) return no('That traveler is unknown.');
  if (state.heroes.some((h) => h.id === id))
    return no(`${HEROES[id].name} is already with you.`);
  const level = Math.max(1, ...state.heroes.map((h) => h.level));
  state.heroes.push(makeHero(id, level, state));
  state.flags[`${id}_recruited`] = true;
  recomputeUnlocks(state);
  return ok(`${HEROES[id].name} joins the crew.`, [
    { id, label: `${HEROES[id].name} joined`, amount: 1 },
  ]);
}
export function applyRewards(state, reward = {}, ledgerId) {
  state.flags.rewardLedger ??= {};
  if (ledgerId && state.flags.rewardLedger[ledgerId])
    return no('These rewards have already been claimed.');
  if (ledgerId) state.flags.rewardLedger[ledgerId] = true;
  const rewards = [];
  for (const id of ['food', 'ore', 'energy', 'renown'])
    if (reward[id]) {
      state.resources[id] += reward[id];
      rewards.push({
        id,
  ...
  for (const [id, amount] of Object.entries(reward.items || {})) {
    if (!ITEMS[id]) throw new Error(`Unknown reward ${id}`);
    state.inventory[id] = (state.inventory[id] || 0) + amount;
    rewards.push({ id, label: ITEMS[id].name, amount });
  }
...
export function equip(state, heroId, itemId) {
  ...
  hero.equip[item.slot] = itemId;
  clampHero(hero, state);
  return ok(`${hero.name} equipped ${item.name}.`);
}
export function unequip(state, heroId, slot) {
  const hero = state.heroes.find((h) => h.id === heroId),
    id = hero?.equip[slot];
  if (!id) return no('That slot is empty.');
  state.inventory[id] = (state.inventory[id] || 0) + 1;
  hero.equip[slot] = null;
  clampHero(hero, state);
  return ok(`${ITEMS[id].name} returned to the pack.`);
}
function clampHero(hero, state) {
  const s = stats(hero, state);
  hero.hp = Math.min(hero.hp, s.maxHp);
  hero.mp = Math.min(hero.mp, s.maxMp);
}
export function learn(state, heroId, techId) {
  const hero = state.heroes.find((h) => h.id === heroId),
    tech = TECHS[techId];
  if (!hero || !tech || !tech.heroes.includes(heroId))
    return no('This technique belongs to another hero.');
  if (state.heroes.some((h) => h.skills.includes(techId)))
    return no('The crew already knows that technique.');
  if (tech.heroes.some((id) => !state.heroes.some((h) => h.id === id)))
    return no('Recruit every participant first.');
  if (tech.level && hero.level < tech.level)
    return no(`Requires hero level ${tech.level}.`);
  if (tech.tier && state.tier < tech.tier)
    return no(`Requires ${TIERS[tech.tier - 1]} civilization.`);
  if (tech.flag && !state.flags[tech.flag])
    return no('Complete the related personal story first.');
  const required = Array.isArray(tech.requires)
    ? tech.requires
    : tech.requires
      ? [tech.requires]
      : [];
  if (required.some((id) => !state.heroes.some((h) => h.skills.includes(id))))
    return no(
      `First learn ${required
        .filter((id) => !state.heroes.some((h) => h.skills.includes(id)))
        .map((id) => TECHS[id].name)
        .join(' and ')}.`,
    );
  if (hero.skillPoints < tech.cost)
    return no(`Requires ${tech.cost} skill points.`);
  const before = hero.skillPoints;
  hero.skillPoints -= tech.cost;
  hero.skills.push(techId);
  return ok(
    `${hero.name} learned ${tech.name}. ${tech.cost} SP spent · ${before} → ${hero.skillPoints} SP.`,
  );
}
export function buildingCost(state, id) {
  ...
  return Object.fromEntries(
    Object.entries(b.cost).map(([k, v]) => [
      k,
      Math.round(v * (1 + level * 0.65)),
    ]),
  );
}
const affordable = (state, cost) =>
  Object.entries(cost).every(([id, n]) => state.resources[id] >= n);
...
export function build(state, id) {
  ...
  return ok(`${b.name} is now level ${level + 1}.`, [
    { id, label: `${b.name} · ${level + 1}`, amount: 1 },
  ]);
}
export const TIER_REQUIREMENTS = {
  2: {
    buildings: { town_center: 2 },
    flags: ['beacon_restored'],
    cost: { food: 35, ore: 35, energy: 15, renown: 12 },
  },
  3: {
    buildings: { town_center: 3, research_lab: 1 },
    flags: ['rune_recruited'],
    cost: { food: 70, ore: 85, energy: 60, renown: 65 },
  },
  4: {
    buildings: { town_center: 4, research_lab: 2, forge: 2, walls: 2 },
    flags: ['forest_seal', 'mire_seal', 'crater_seal', 'frost_seal'],
    cost: { food: 130, ore: 150, energy: 130, renown: 180 },
  },
};
const TIER_STORY_REQUIREMENTS = {
  beacon_restored: {
    label: 'Haventide beacon restored',
    instruction: 'Restore the listening beacon west of Haventide',
  },
  rune_recruited: {
    label: 'Rune recruited',
    instruction:
      'Recruit Rune at Anchor Nine in Orbital Reach after defeating its entrance blockade',
  },
  forest_seal: {
    label: 'Forest Veil restored',
    instruction: 'Restore Forest Veil’s Heartwood Relay beyond its guardian',
  },
  mire_seal: {
    label: 'Mire Bog restored',
    instruction: 'Open Mire Bog’s submerged archive after defeating its keeper',
  },
  crater_seal: {
    label: 'Crater Ember restored',
    instruction:
      'Restore Crater Ember’s sun-forge after defeating its sovereign',
  },
  frost_seal: {
    label: 'Frost Canyon restored',
    instruction: 'Relight Frost Canyon’s midnight beacon beyond the Colossus',
  },
};
...
export function tierEligibility(state) {
  const nextTier = state.tier + 1,
    req = TIER_REQUIREMENTS[nextTier];
  if (!req)
    return {
      eligible: false,
      complete: true,
      ...
      requirements: [],
      missing: [],
      reason: 'The settlements are already Transcendent.',
    };
  const requirements = [
    ...Object.entries(req.buildings).map(([id, required]) => {
      const current = state.buildings[id] || 0;
      return {
        type: 'building',
        id,
        required,
        ...
        label: `${BUILDINGS[id].name} level ${required}`,
        instruction: `${current ? 'Upgrade' : 'Build'} ${BUILDINGS[id].name} to level ${required} (currently ${current})`,
      };
    }),
    ...req.flags.map((id) => ({
      type: 'story',
      id,
      met: !!state.flags[id],
      ...TIER_STORY_REQUIREMENTS[id],
    })),
    ...Object.entries(req.cost).map(([id, required]) => {
      ...
      return {
        ...
        met: current >= required,
        label: `${required} ${id}`,
        instruction: `Gather ${Math.ceil(required - current)} more ${id} (${Math.floor(current)} / ${required})`,
      };
    }),
  ];
  const missing = requirements.filter((r) => !r.met),
    name = TIERS[nextTier - 1];
  return {
    ...
    reason: missing.length
      ? `To unlock ${name}: ${missing.map((r) => r.instruction).join('; ')}.`
...
export function tierRequirements(state) {
  ...
  return status.complete
    ? 'Civilization has reached Transcendent.'
    : status.requirements
        .map((r) => `${r.label}${r.met ? ' ✓' : ''}`)
        .join(' · ');
}
export function advanceTier(state) {
  ...
  if (!status.eligible) return no(status.reason);
  pay(state, status.cost);
  state.tier = status.nextTier;
  recomputeUnlocks(state);
  return ok(`The settlements become ${TIERS[state.tier - 1]}.`, [
...
export function buy(state, itemId, quantity = 1) {
  ...
  return ok(`Bought ${quantity} ${item.name}.`, [
...
export function rest(state) {
  ...
  return ok(
    cost
      ? `The crew rests. Up to ${cost} food shared with the house.`
      : 'The lantern houses welcome the crew freely.',
  );
}
export function previewItemUse(state, itemId, heroId) {
  const h = state.heroes.find((h) => h.id === heroId);
  return assessItemUse(state, itemId, h, h ? stats(h, state) : null);
}
export function useItem(state, itemId, heroId, options = {}) {
  const h = state.heroes.find((h) => h.id === heroId);
  return consumeItem(state, itemId, h, h ? stats(h, state) : null, options);
}
export function train(state) {
  ...
  return awardXp(state, 300 + state.tier * 100);
}
export function research(state) {
  ...
  if (!affordable(state, { ore: 80, energy: 70 }))
    return no('Concord research requires 80 ore and 70 energy.');
  pay(state, { ore: 80, energy: 70 });
  state.flags.research_concord = true;
  return ok('Concord research: the crew gains +6 intelligence and technique.');
}
export function recomputeUnlocks(state) {
  state.flags.final_ready = Boolean(
    state.tier === 4 &&
    state.heroes.length === 3 &&
    [
      'forest_seal',
      'mire_seal',
      'crater_seal',
      'frost_seal',
      'crown_memory',
    ].every((f) => state.flags[f]),
  );
  return state.flags.final_ready;
}
export function futureEligibility(state) {
  return {
    eligible: Boolean(
      state.campaignComplete &&
      (state.heroes.find((h) => h.id === 'kaida')?.level || 0) >= 40,
    ),
    available: EXPANSION_CONTRACT.available,
    developmentAuthorized: EXPANSION_CONTRACT.developmentAuthorized,
    reason:
      'Time travel is a future, unapproved expansion. The base game remains in the present.',
  };
}
...
export function battleReward(encounter, firstClear = true, random = () => 1) {
  const enemies = encounter.enemies.map((id) => {
    if (!ENEMIES[id]) throw new Error(`Unknown reward enemy: ${id}`);
    return ENEMIES[id];
  });
  const reward = {
    xp: Math.floor(
      enemies.reduce((n, e) => n + e.xp, 0) * (firstClear ? 1 : 0.35),
    ),
    ore: Math.floor(
      enemies.reduce((n, e) => n + 8 * e.tier, 0) * (firstClear ? 1 : 0.5),
    ),
    food: 6 + enemies.length * 3,
    energy: enemies.reduce((n, e) => n + 3 * e.tier, 0),
    renown: enemies.reduce((n, e) => n + 3 * e.tier, 0),
    items: {},
  };
  for (const e of enemies)
    if (e.drop && ITEMS[e.drop] && (firstClear || random() < 0.3))
      reward.items[e.drop] = (reward.items[e.drop] || 0) + 1;
  return reward;
}
