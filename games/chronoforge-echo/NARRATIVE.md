...
## Measured balance audit (production modules, deterministic legal inputs)

The focused Node campaign test uses `createBattle`, `battleKey`, and `updateBattle`, not direct damage or victory assignment. Its route clears forty distinct encounters, follows actual regional portal prerequisites, gathers authored outdoor supplies, earns each story flag through its production interaction, builds all eight building types, advances all four civilization tiers, completes one valid outcome of each personal arc, defeats the Herald and then the separate Architect, and returns physically through the regional gateways for the homecoming. A separate branch test covers all six mutually exclusive outcomes.

The audit uses normal first-clear rewards through the shared `battleReward` helper. It learns affordable skills, transfers earned gear, uses available healing spells/supplies, and rests between fights. It grants no XP or currency outside gameplay rules, makes no repeated encounter clears, does no paid training, and receives no successful timing bonus. Twelve seconds of productive exploration between encounters and eight seconds between story interactions approximate ordinary traversal; this is a systems test, not evidence of camera movement or pathfinding quality.

Initial audit found excess experience: Kaida reached level 56 before the final boss and late bosses ended in four to six actions. Enemy and story XP awards were halved while retaining the linear level curve, and late-boss durability was raised to let the authored phases play. Under the final reward rule, three independent seeds (9127, 2048, 7) each completed all forty battles without a loss: Kaida was level 7 for the solo observatory fight, the full crew was level 38 before the Architect, and all three reached level 42 by the playable aftermath. The routes used three to four healing actions and no healing consumables. Simulated battle clocks totaled 506–520 seconds, excluding travel, command deliberation and dialogue. Required civic construction/advancement remained affordable without waiting for resources or farming. These are headless balance measurements; live visual and input review remains separate.

A second minimal-route audit deliberately skips every optional quest, road patrol and pickup: the first scrapper, Haventide’s sentry, the buoy, Emberline’s guard, and the observatory. Five seeds (9127, 2048, 7, 81, 19) all won the solo observatory at Kaida level 3 with one Salt Mend and no consumable or timing bonus; Vex joined with both heroes at level 5 after the earned reward. The Neon Cultist was tuned to 400 HP / 38 strength / 13 defense to make this clean path practical while preserving its threat-tier-3 specialist role and telegraphed group attack. Optional content improves preparation; it is not a recruitment prerequisite.
