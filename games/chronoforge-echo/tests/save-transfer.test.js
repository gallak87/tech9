...
function fixture(t) {
  ...
  const elements = [], feedback = [], loads = [], focus = [];
  const document = {
    body: {append(element) { element.attached = true; }},
    createElement(tag) {
      const element = {
        tag, events: {}, files: [],
        setAttribute() {},
  ...
  for (const [name, value] of Object.entries({document, localStorage: storage})) {
  ...
  const ui = {
    menu: true, tab: 5, panel: null, menuScroll: {5: 123},
    root: {querySelector(selector) {
      return selector === '.atlas-body' ? {scrollTop: 123} : {focus() { focus.push(selector); }};
    }},
    game: {audio: {sound() {}}, load(slot) { loads.push(slot); }},
    render() {},
    feedback(result) { feedback.push(result); },
    restoreShopRow(action, scroll) { focus.push({action, scroll}); },
    confirm(title, text, run, options) { this.panel = {type: 'confirm', title, text, run, ...options}; },
  };
  const state = createState();
  state.resources.ore = 777;
  const json = JSON.stringify(saveState(state, 1, storage));
  return {transfer: new SaveTransfer(ui), ui, state, json, storage, rows, elements, feedback, loads, focus};
}
...
test('imports wait for confirmation, default to cancel, and change only the selected slot', async t => {
  ...
  assert.deepEqual(rows, before, 'Reading and confirming must not write a save');
  ...
  assert.equal(rows.get('chronforge_echo_v1:1'), before.get('chronforge_echo_v1:1'));
...
test('leaving Save cancels pending file reads even if the menu opens again', async t => {
  ...
  transfer.requestImport('2');
  let resolve;
  transfer.input.files = [{size: json.length, text: () => new Promise(done => { resolve = done; })}];
...
test('invalid or oversized files never alter saved records', async t => {
  ...
  transfer.requestImport('2');
  transfer.input.files = [{size: MAX_SAVE_BYTES + 1, text() { assert.fail('Oversized file should not be read'); }}];
