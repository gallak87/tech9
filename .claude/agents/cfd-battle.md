---
name: cfd-battle
description: Chronoforge Dawn — battle lane. ATB, techs, dual/triple combos, time-freeze crits, battle camera choreography, elemental VFX, in-place staging at locked pitch. Tier 4, the crown jewel.
model: opus
effort: high
...
`node tools/duel.mjs` resolves 200 seeded battles with zero deadlocks, TTK inside band, and a loot distribution matching the declared drop tables with no item stuck at zero; `tools/stage.mjs` confirms every combo finisher frames its own actors and the cut lands on the hit. You fix defects 3 and 4. The prototype's `games/chronoforge/src/battle.js` is your authoritative design spec — port its math, restage its presentation.
