...
| The world graph topology itself. Inherited shape is a tree (8 outdoor regions, 7 edges, no cycles) with a T2-to-T4 jump from Emberline to Crater Ember. Open candidates: lateral cycles, settlement-tech-gated edges that make the base economy matter to exploration, one-way drops unlocked from the far side, and seeded time-of-day chrono-rifts. If rifts move, region.mjs asserts reachability in every phase rather than once. | level |
