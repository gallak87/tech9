...
import {worldViewCamera,worldViewSurfaceSize,worldViewTiles} from '../src/world-view-camera.js';
import {canOpenWorldView,mountWorldView,worldViewShortcut} from '../src/world-view.js';
...
function fixture(t) {
  const frames=new Map(),canvases=[],logs=[];
  let nextFrame=0,failDraw=false;
  const document={activeElement:null};
  function element(tag) {
    const node={tag,children:[],events:{},inert:false,isConnected:true,
      ...
      set innerHTML(value){
        if(value.includes('<header>')){
          const button=element('button');button.append(element('kbd'));
  ...
  const host=element('main'),hud=element('aside'),existingModal=element('aside'),dev=element('aside');
  dev.id='dev-tools';existingModal.inert=true;host.append(hud,existingModal,dev);
  const previousFocus=element('button');previousFocus.focus();
  document.createElement=element;document.querySelector=selector=>selector==='#game'?host:null;
  ...
  const game={mode:'world',battle:null,transition:null,visualTime:4,keys:new Set(['w']),moving:true,movePath:[{x:80,y:80}],
    camera:{x:0,y:0},followers:[],audio:{unlock(){}},
    scene:{id:'overview-test',name:'Test coast',biome:'coast',width:960,height:540,objects:[],portals:[]},
    state:{x:60,y:90,facing:'down',settings:{reducedMotion:true},fog:{haventide:[1]},playTime:25},
    get visualState(){return this.state;},log(type,data){logs.push({type,...data});},
  };
  game.ui={menu:false,panel:null,get blocked(){return this.menu||!!this.panel||!!game.worldView?.open;},positionInteraction(){}};
  game.worldView=mountWorldView(game);
  const root=host.children.at(-1),view=game.worldView;
  return {game,view,root,hud,existingModal,dev,frames,canvases,logs,previousFocus,document,
    failDraw(){failDraw=true;},
    frame(){const [id,callback]=frames.entries().next().value;frames.delete(id);callback(performance.now()+1000);},
  };
}

const key=(value,extra={})=>({key:value,preventDefault(){this.prevented=true;},...extra});

test('the production shortcut pauses exploration and returns without changing the expedition',t=>{
  const f=fixture(t),{game,view,root,hud,existingModal,dev,canvases,frames}=f;
  const before=JSON.stringify({state:game.state,camera:game.camera});
  assert.equal(view.handleKey(key('r')),true);
  assert.equal(view.open,true);assert.equal(game.ui.blocked,true);assert.equal(root.hidden,false);
...
test('world view leaves menus, battles, travel, typing and custom R bindings alone',t=>{
  ...
  for(const [owner,property,value] of [[game,'mode','title'],[game,'mode','battle'],[game,'battle',{}],[game,'transition',{}],[game.state,'recruitmentWalk',{}],[game.ui,'menu',true],[game.ui,'panel',{type:'dialogue'}]]){
    const previous=owner[property];owner[property]=value;
    assert.equal(canOpenWorldView(game),false,property);
    assert.equal(view.openView(),false);assert.equal(view.handleKey(key('r')),false);
    owner[property]=previous;
  }
  for(const extra of [{ctrlKey:true},{metaKey:true},{altKey:true},{isComposing:true},{target:{closest:()=>({})}}])assert.equal(view.handleKey(key('r',extra)),false);
  view.handleKey(key('r',{repeat:true}));assert.equal(view.open,false);
  for(const action of ['up','left','down','right','interact']){
    game.state.settings.keys={[action]:'R'};
    assert.equal(worldViewShortcut(game.state.settings),'');assert.equal(view.handleKey(key('r')),false);
  }
  assert.equal(view.openView(),true,'The button still opens when R has another binding');
  assert.equal(view.handleKey(key('Escape')),true);assert.equal(view.open,false);
});

test('closing during preparation cancels rendering; errors and disposal release the modal',t=>{
  const f=fixture(t),{view,root,frames,hud}=f;
  view.openView();assert.ok(frames.size>0);
  view.handleKey(key('Escape'));assert.equal(frames.size,0);assert.equal(hud.inert,false);
  view.openView();f.failDraw();f.frame();
  assert.equal(view.open,false);assert.equal(root.hidden,true);assert.equal(hud.inert,false);
  assert.equal(frames.size,0);assert.equal(f.logs[0].type,'world_view_error');
  view.dispose();assert.equal(root.isConnected,false);assert.equal(f.canvases[0].width,0);
});
