...
const report = {
  startedAt: new Date().toISOString(), section, filter,
  environment: { platform: process.platform, arch: process.arch, release: os.release(), cpu: os.cpus()[0]?.model, browser: await browser.version(), viewport: [1920, 1080], nativeCanvas: [960, 540] },
  methodology: 'New isolated browser context with HMR disconnected to keep the loaded build stable. UX cases use actual keyboard or mouse after explicit fixture setup. This is subsystem acceptance, not a claim of a clean-save manual campaign completion.',
  checks: [], consoleErrors: [], pageErrors: [], missingAssets: [], failedRequests: [], documentLoads: [], performance: {}, sequences: {},
};
page.on('framenavigated', frame => { if (frame === page.mainFrame()) report.documentLoads.push({ url: frame.url(), time: new Date().toISOString() }); });
...
async function check(name, method, fn) {
  ...
  try { const details = await fn(); report.checks.push({ name, method, pass: true, milliseconds: Math.round(performance.now() - start), details }); console.log(`PASS ${name}`); }
  catch (error) {
    const image = await capture(`failure-${name.replace(/[^a-z0-9]+/gi, '-').toLowerCase()}`).catch(() => null);
    report.checks.push({ name, method, pass: false, milliseconds: Math.round(performance.now() - start), error: error.message, screenshot: image });
...
async function keyboardTo(selector) {
  ...
  assert.ok(nav.index >= 0, `Keyboard target is visible and enabled: ${selector}`);
  const count = (nav.index - nav.current + nav.length) % nav.length;
  for (let i = 0; i < count; i++) await press('Tab');
  assert.ok(await page.evaluate(selector => document.activeElement === document.querySelector(selector), selector), `Keyboard reached ${selector}`);
}
const choose = async action => { await keyboardTo(`[data-do="${action}"]`); await press('Enter'); };
...
async function perf(name, duration = 1500) {
  ...
  report.performance[name] = { samples: intervals.length, meanMs: mean, medianMs: intervals[Math.floor(intervals.length / 2)], p95Ms: intervals[Math.min(intervals.length - 1, Math.floor(intervals.length * .95))], maxMs: intervals.at(-1), meanFps: 1000 / mean, targetP95Ms: 20, meetsTarget: intervals[Math.min(intervals.length - 1, Math.floor(intervals.length * .95))] <= 20 };
...
try {
  ...
  if (section === 'all' || section === 'ux') {
    await check('clean opening and all seven atlas tabs', 'Actual keyboard from title; no progress fixture.', async () => {
      await press('Enter');
      assert.equal((await snapshot()).state.heroes.length, 1);
      for (let i = 0; i < 10 && (await snapshot()).panel === 'dialogue'; i++) await press('Enter');
      assert.equal((await snapshot()).mode, 'world');
      const opening = await capture('opening');
      await press('Escape');
      const titles = ['Map', 'Party', 'Inventory', 'Skills', 'Quests', 'Save', 'Settings'];
      const screens = [];
      for (let i = 0; i < titles.length; i++) {
        await press(String(i + 1));
        assert.ok((await page.locator('.tabs .active').innerText()).includes(titles[i]));
        screens.push(await capture(`atlas-${i + 1}-${titles[i].toLowerCase()}`));
      }
      await press('q'); assert.ok((await page.locator('.tabs .active').innerText()).includes('Save'));
      await press('e'); assert.ok((await page.locator('.tabs .active').innerText()).includes('Settings'));
      return { opening, screens, tabs: titles };
    });

    await check('settings volume toggles and key binding', 'Actual Tab/arrow/Enter navigation.', async () => {
      await ensureMenu(7);
      const before = (await snapshot()).state.settings.music;
      await keyboardTo('[data-setting="music"]'); await press('ArrowRight');
      assert.equal((await snapshot()).state.settings.music, Math.min(1, before + .1));
      const oldAssist = (await snapshot()).state.settings.timingAssist;
      await choose('setting:timingAssist'); assert.equal((await snapshot()).state.settings.timingAssist, !oldAssist);
      const oldMap = (await snapshot()).state.settings.minimap;
      await choose('setting:minimap'); assert.equal((await snapshot()).state.settings.minimap, !oldMap);
      await choose('setting:minimap');
      await choose('rebind:up'); await press('i'); assert.equal((await snapshot()).state.settings.keys.up, 'i');
      await choose('rebind:up'); await press('w'); assert.equal((await snapshot()).state.settings.keys.up, 'w');
    });
    ...
    await check('map keyboard pan zoom recenter and pickup secrecy', 'Actual map keys. A temporary fixture relocates pickups onto revealed ground; production map pixels must remain identical.', async () => {
      ...
      const compared = await page.evaluate(async () => {
        ...
        const mini = document.createElement('canvas'); drawMinimap(mini, g.scene, g.state); const miniBefore = mini.toDataURL();
        for (const o of pickups) { o.x = g.state.x; o.y = g.state.y; }
        map.draw(); drawMinimap(mini, g.scene, g.state);
        const result = { atlasUnchanged: before === canvas.toDataURL(), minimapUnchanged: miniBefore === mini.toDataURL(), pickupCount: pickups.length, zoom: map.zoom, panX: map.panX, panY: map.panY };
        for (const { o, x, y } of originals) { o.x = x; o.y = y; } map.draw();
        return result;
      });
      assert.ok(compared.pickupCount > 0); assert.ok(compared.atlasUnchanged && compared.minimapUnchanged);
      assert.equal(compared.zoom, 1); assert.equal(compared.panX, 0); assert.equal(compared.panY, 0);
      return compared;
    });
    ...
    await check('inventory equipment comparisons use unequip and skill learning', 'Fixture adds one weapon, wounds Kaida, grants 5 skill points; all actions use keyboard.', async () => {
      ...
      let s = (await snapshot()).state; assert.equal(s.heroes[0].equip.weapon, 'glacial_claw'); assert.equal(s.inventory.iron_blade, 1);
      await choose('unequip:weapon'); assert.equal((await snapshot()).state.inventory.glacial_claw, 1);
      await choose('item:glacial_claw'); await choose('equip:glacial_claw');
      const beforeTonic = (await snapshot()).state.inventory.field_tonic;
      await choose('item:field_tonic'); await choose('use:field_tonic');
      s = (await snapshot()).state; assert.equal(s.heroes[0].hp, 90); assert.equal(s.inventory.field_tonic, beforeTonic - 1);
      await press('4'); await choose('learn:chrono_strike');
      s = (await snapshot()).state; assert.ok(s.heroes[0].skills.includes('chrono_strike')); assert.equal(s.heroes[0].skillPoints, 4);
      await choose('learn:chrono_strike'); assert.equal((await snapshot()).state.heroes[0].skillPoints, 4);
    });
    ...
    await check('save load delete confirmation and corrupt record recovery', 'Actual keyboard save/load/delete; fixtures change unsaved HP and create one malformed record.', async () => {
      ...
      await ensureMenu(6); await choose('delete:1'); await choose('confirm-no');
      assert.ok(await page.evaluate(() => localStorage.getItem('chronforge_echo_v1:1')));
      await choose('delete:1'); await choose('confirm-yes');
      assert.equal(await page.evaluate(() => localStorage.getItem('chronforge_echo_v1:1')), null);
      await choose('save:2');
      await page.evaluate(() => localStorage.setItem('chronforge_echo_v1:3', '{broken'));
      await press('5'); await press('6');
      assert.match(await page.locator('.atlas-body').innerText(), /Unreadable record/);
      await choose('load:2'); await choose('confirm-yes'); assert.equal((await snapshot()).state.heroes[0].hp, hp);
    ...
    await check('ending dialogue and final panel survive save load', 'A victory fixture resolves the real final-boss reward event. Actual keyboard advances dialogue, saves and loads at both stages, and chooses Return; this does not claim a manually fought final boss.', async () => {
      ...
      await ensureMenu(6); await choose('load:3'); await choose('confirm-yes');
      ...
      current = await snapshot();
      assert.equal(current.panel, 'ending'); assert.equal(current.state.endingProgress.panel, 'ending');
      assert.ok(current.state.flags.pendingEnding); assert.ok(!current.state.campaignComplete);
      await press('Backspace'); assert.equal((await snapshot()).panel, 'ending');
      await ensureMenu(6); await choose('save:2');
      await fixture('world');
      await ensureMenu(6); await choose('load:2'); await choose('confirm-yes');
      current = await snapshot();
      assert.equal(current.panel, 'ending'); assert.equal(current.state.endingProgress.panel, 'ending');
      ...
      const checkpoint = await page.evaluate(async () => (await import('/src/persistence.js')).loadState('checkpoint'));
      assert.ok(checkpoint.campaignComplete); assert.ok(checkpoint.flags.ending_seen); assert.equal(checkpoint.flags.pendingEnding, false);
      return { dialogueIndexRestored: dialogue.index, dialogueLine: dialogue.line, completionDeferredUntilReturn: true, completedScene: current.scene, screenshots: [midway, finalPanel, await capture('ending-return-world')] };
    });
    ...
    await check('vendor quantity buying selling and affordability', 'Liberated-settlement fixture; open actual provisions NPC; trade actions use keyboard.', async () => {
      ...
      await choose('sell:field_tonic'); after = (await snapshot()).state;
      assert.equal(after.inventory.field_tonic, before.inventory.field_tonic - 3); assert.equal(after.resources.ore, before.resources.ore + 9);
    ...
    await check('construction spending production and civilization advancement', 'Fixture supplies and beacon prerequisite; actual construction NPC and keyboard build/tier actions.', async () => {
      ...
      await choose('build:town_center'); assert.equal((await snapshot()).state.buildings.town_center, 2);
      await choose('tier'); s = (await snapshot()).state; assert.equal(s.tier, 2);
      const screen = await capture('construction-reclaimer');
      const food = s.resources.food; await press('Backspace'); await page.waitForTimeout(150);
  ...
  if (section === 'all' || section === 'combat') {
    await check('battle keyboard targeting held-key protection and exact global pause', 'Solo battle fixture. Actual keys through readiness, commands, target, timing window, and Esc.', async () => {
      ...
      await press('Escape'); const frozen = (await snapshot()).battle;
      ...
      await press('Escape'); await press('Space');
    ...
    await check('four-enemy render and steady combat performance', 'Four-enemy fixture with actual production renderer. Waiting ATB runs normally.', async () => {
  ...
  if (section === 'all' || section === 'world') {
    await check('party traversal followers camera and collision', 'Party fixture; encounters marked cleared to isolate navigation. Actual held movement keys, no simulation teleport between frames.', async () => {
      ...
      await page.waitForTimeout(700); await frame('stride-2');
      await page.waitForTimeout(700); await page.keyboard.up('ArrowRight'); await page.keyboard.up('Shift'); await frame('end');
      const first = frames[0].state, last = frames.at(-1).state;
      assert.ok(last.x - first.x > 300); assert.ok(last.camera.x - first.camera.x > 200);
      assert.equal(last.followers.length, 2); assert.ok(last.followers.every(h => h.walkable));
      assert.ok(last.followers[0].x < last.x && last.followers[1].x < last.followers[0].x);
      assert.ok(last.world[0] >= 960 * 6 && last.world[1] >= 540 * 4);
      report.sequences.followers = frames;
      return { distance: last.x - first.x, cameraPan: last.camera.x - first.camera.x, world: last.world, frames: frames.map(f => f.screenshot) };
    });
  ...
  if (section === 'showcase') {
    await check('latest production attack choreography frame sequence', 'Deterministic frame inspection: root renderer remains live; the authoritative battle timeline is stepped to exact poses. This section pauses between samples, then a separate real-time loop follows in the video.', async () => {
      await fixture('battle');
      ...
      return { frames: frames.map(f => ({ elapsed: f.elapsed, screenshot: f.screenshot })), normalVsCritical: frames[5].state.action.critical };
    });
  ...
  if (section === 'route') {
    await check('ordinary Haventide road across multiple viewports', 'Only prerequisites/cleared encounters are seeded. Production walkTo and Game.update run at fixed 1/60-second steps, normal Shift speed 245 pixels/second. Wall time is accelerated between rendered samples; world coordinates are never teleported along the route.', async () => {
      ...
      const route = await page.evaluate(async () => {
        ...
        Object.assign(g.state, g.scene.spawn); g.resetFollowers(); g.updateCamera(true);
        window.routeUpdate = g.update; g.update = () => {}; g.keys.add('Shift');
        window.routeStats = { simulatedSeconds: 0, walkedPixels: 0, maximumSpeed: 0, steps: 0, collisions: [], transitions: [] };
        const gate = g.scene.portals.find(p => p.to === 'emberline');
        return { start: { x: g.state.x, y: g.state.y }, region: g.scene.id, dimensions: [g.scene.width, g.scene.height], viewport: [960, 540], waypoints: [...g.scene.roads[0].slice(1).map(p => ({ x: p.x, y: p.y })), { x: gate.x, y: gate.y }], gate: { ...gate } };
      });
      const frames = [];
      ...
      for (let segment = 0; segment < route.waypoints.length; segment++) {
        ...
        for (let batch = 0; batch < 12; batch++) {
          ...
          if (!result.remaining && !result.transition && result.distance < 45 && segment < route.waypoints.length - 1) break;
          if (!result.remaining && !result.transition && segment === route.waypoints.length - 1 && result.distance < 62) {
            assert.equal(result.near, route.gate.id, 'The final road walk reaches the real gateway interaction radius');
            await press('Enter');
            continue;
          }
          assert.equal(result.mode, 'world', `Navigation fixture unexpectedly entered ${result.mode}`);
          assert.equal(result.panel, undefined, `Navigation fixture blocked by ${result.panel}`);
          if (!result.remaining && !result.transition && result.distance >= 45) throw Error(`Path blocked on road leg ${segment + 1} at ${result.x},${result.y}; ${result.distance}px from waypoint.`);
        }
      }
      ...
      assert.equal(last.region, 'emberline'); assert.ok(last.walkedPixels >= 5000);
      assert.ok(last.maximumSpeed <= 245.01); assert.equal(last.collisions.length, 0);
      assert.ok(pan >= 960 * 4);
      return { steps: last.steps, simulatedSeconds: last.simulatedSeconds, walkedPixels: last.walkedPixels, maximumSpeed: last.maximumSpeed, cameraPanPixels: pan, cameraPanViewports: pan / 960, screenshotCount: frames.length, transitions: last.transitions, collisionViolations: last.collisions.length };
    });
  }
  ...
  await browser.close();
  console.log(JSON.stringify({ results: path.relative(root, output), passed: report.passed, failed: report.failed, consoleErrors: report.consoleErrors.length, pageErrors: report.pageErrors.length, missingAssets: report.missingAssets.length, fatal: Boolean(report.fatal) }));
  if (report.failed || report.fatal || report.pageErrors.length || report.missingAssets.length || report.consoleErrors.length) process.exitCode = 1;
}
