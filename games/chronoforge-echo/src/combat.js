...
const learned = (hero, id) =>
  Array.isArray(hero.skills)
    ? hero.skills.includes(id)
    : Boolean(hero.skills?.[id]);
const actorById = (b, id) =>
  b.heroes.find((h) => h.id === id) || b.enemies.find((e) => e.uid === id);
...
function sync(b, state) {
  for (const h of b.heroes) {
    ...
    if (saved) {
      saved.hp = clamp(h.hp, 0, h.maxHp);
      saved.mp = clamp(h.mp, 0, h.maxMp);
    }
  }
}
...
function cleanQueue(b) {
  ...
  if (!b.readyQueue.includes(b.selectedHero))
    b.selectedHero = b.readyQueue[0] || null;
...
export function createBattle(state, encounter) {
  if (
    ...
    encounter.enemies.length > 4
  ) {
    throw new Error(
      `Encounter ${encounter.id || '?'} must contain one to four enemies.`,
    );
  }
  const heroes = state.heroes.map((h, i) => ({
    ...
    side: 'hero',
    index: i,
    hp: clamp(h.hp, 0, stats(h, state).maxHp),
    mp: clamp(h.mp, 0, stats(h, state).maxMp),
    atb: 58 - i * 9,
    shield: 0,
    guarding: false,
    criticalGuard: false,
  ...
  const enemies = encounter.enemies.map((id, i) => {
    ...
    return {
      ...
      maxMp: data.mp || 0,
      mp: data.mp || 0,
      int: data.int || data.str,
      tec: data.tec || data.str,
      crit: data.crit || 4,
      atb: 10 + i * 7,
      shield: 0,
      guarding: false,
      ...
      slowTurns: 0,
      reaction: 0,
      home: { ...E_POS[encounter.enemies.length][i] },
      turns: 0,
      bossPhase: 1,
      charging: false,
    };
  ...
  const b = {
    encounter: { ...encounter },
    biome: encounter.biome || state.region,
    heroes,
    enemies,
    readyQueue: [],
    selectedHero: null,
    lastSelectedHero: heroes[0]?.id || null,
    phase: 'active',
    mode: 'waiting',
    cursor: 0,
    target: 0,
  ...
  log(
    b,
    encounter.boss
      ? 'A signal answers from the dark.'
      : 'Weapons drawn. Keep the signal alive.',
  );
  checkEnd(b, state);
  return b;
}
...
function rate(actor) {
  return (
    (12 + actor.spd * (actor.side === 'hero' ? 1.36 : 1.02)) *
    (actor.slowTurns ? 0.57 : 1)
  );
}
...
function listTechs(b, state) {
  ...
  return Object.values(TECHS)
    .filter((t) => {
      ...
      return (
        t.heroes.length === 1 ||
        t.heroes.every((hid) => b.heroes.some((h) => h.id === hid))
      );
    })
...
function techReason(b, state, tech) {
  ...
  if (!tech.heroes.some((id) => learned(sourceHero(state, id) || {}, tech.id)))
    return 'Learn this technique in Skills.';
  ...
  const dry = members.find(
    (h) =>
      h.mp < (typeof tech.mp === 'object' ? tech.mp[h.id] || 0 : tech.mp || 0),
  );
...
function listItems(state) {
  return Object.entries(state.inventory || {})
    .filter(
      ([id, count]) =>
        count > 0 &&
        ['heal', 'restoreMp', 'revive'].includes(ITEMS[id]?.effect),
    )
    .map(([id, count]) => ({ ...ITEMS[id], id, count }));
}

function validTargets(b, command) {
  if (!command) return [];
  if (command.target === 'self')
    return [actorById(b, command.participants[0])].filter(Boolean);
  const side =
    command.target === 'ally' || command.target === 'allAllies'
      ? b.heroes
      : b.enemies;
  return side.filter((actor) =>
    command.effect === 'revive' ? !alive(actor) : alive(actor),
  );
}
...
function commandFromTech(b, tech) {
  return {
    ...tech,
    kind: 'tech',
    participants: [...tech.heroes],
    returnMode: 'tech',
    returnCursor: b.cursor,
  };
}
...
function rootConfirm(b, state) {
  ...
  if (cmd === 'Attack')
    chooseCommand(b, state, {
      id: 'attack',
      name: 'Attack',
      kind: 'attack',
      effect: 'damage',
      stat: h.id === 'vex' ? 'int' : 'str',
      power: 1,
      target: 'enemy',
      participants: [h.id],
      returnMode: 'command',
  ...
  if (cmd === 'Item') {
    if (!listItems(state).length) {
      notice(b, 'No field supplies. Vendors stock healing items.');
      return;
    }
    b.mode = 'item';
    b.cursor = 0;
  ...
  if (cmd === 'Retreat') {
    if (b.encounter.boss || b.enemies.some((e) => e.id === 'void_architect')) {
      notice(b, 'This confrontation must be faced.');
      return;
    }
    chooseCommand(b, state, {
      id: 'retreat',
...
function execute(b, state, command, targets, confirmation) {
  ...
  if (!members.length || members.some((h) => !h || !alive(h) || h.atb < 100)) {
    cleanQueue(b);
    b.mode = 'waiting';
    notice(b, 'The formation changed. Choose a ready hero.');
    return false;
  }
  if (command.kind === 'tech') {
    const unavailable = techReason(b, state, command);
    if (unavailable) {
      notice(b, unavailable);
      return false;
    }
  }
  ...
  if (command.kind === 'item') {
    const target = targets[0],
      use = assessItemUse(state, command.id, target, target, { battle: true });
    if (!use.ok) {
      notice(b, use.message);
      return false;
    }
  ...
  b.action = {
    ...
    targets: targets.map(actorId),
    elapsed: 0,
    contact,
    duration: combo ? 1.92 : 1.54,
    windowStart: contact - (assisted ? 0.36 : 0.28),
    windowEnd: contact - (assisted ? 0.055 : 0.095),
    timingEligible: harmful || command.kind === 'defend',
    timingAttempted: false,
    timingSuccess: false,
    timingPressedAt: null,
    resolved: false,
    critical: false,
    executeInput: b.inputSerial,
    stage: 'anticipation',
    itemSpendAtContact: command.kind === 'item',
    itemConfirmation: confirmation ?? null,
  };
  b.phase = 'action';
  b.mode = 'action';
  b.pending = null;
  ...
  log(
    b,
    `${members.map((h) => h.name).join(' + ')} · ${command.name}`,
    'action',
  );
  sync(b, state);
  return true;
...
function finishItemConfirmation(b, state, confirmed) {
  ...
  if (confirmed) {
    const target = b.heroes.find((h) => h.id === pending.targetId);
    execute(
      b,
      state,
      pending.command,
      target ? [target] : [],
      pending.use.confirmationKey,
    );
  }
  return true;
}
...
function commitTarget(b, state) {
  ...
  const group =
    b.pending.target === 'allEnemies' || b.pending.target === 'allAllies';
  const chosen = group
    ? targets
    : [targets[clamp(b.target, 0, targets.length - 1)]].filter(Boolean);
...
function nextReady(b, direction = 1) {
  ...
  b.selectedHero =
    b.readyQueue[
      (index + direction + b.readyQueue.length) % b.readyQueue.length
    ];
  b.pending = null;
  b.cursor = 0;
...
export function battleKey(b, state, input) {
  ...
  if (b.itemConfirmation) {
    if (['Escape', 'Backspace', 'ArrowLeft'].includes(key))
      finishItemConfirmation(b, state, false);
  ...
  if (key === 'Backspace' || key === 'ArrowLeft') {
    if (b.mode === 'target') {
      b.mode = b.pending?.returnMode || 'command';
      b.cursor = b.pending?.returnCursor || 0;
      b.pending = null;
    } else if (b.mode === 'tech' || b.mode === 'item') {
      const wasItem = b.mode === 'item';
      b.mode = 'command';
      b.cursor = wasItem ? 3 : 1;
    } else if (b.mode === 'command') b.mode = 'waiting';
    return true;
  }
  if (b.mode === 'waiting') {
    if (key === 'ArrowUp' || key === 'ArrowDown')
      return nextReady(b, key === 'ArrowUp' ? -1 : 1);
    if ((confirmKey(key) || key === 'ArrowRight') && b.selectedHero) {
      b.mode = 'command';
      b.cursor = 0;
      return true;
    }
  ...
  if (direction) {
    const count =
      b.mode === 'command'
        ? ROOT_COMMANDS.length
        : b.mode === 'tech'
          ? listTechs(b, state).length
          : b.mode === 'item'
            ? listItems(state).length
            : validTargets(b, b.pending).length;
    if (count) {
      if (b.mode === 'target')
        b.target = (b.target + direction + count) % count;
...
function timingPress(b) {
  ...
  if (a.side === 'enemy') {
    log(
      b,
      a.timingSuccess
        ? 'Critical guard · 75% less damage from this attack.'
        : 'Guard timing missed · existing defenses still apply.',
      a.timingSuccess ? 'timing' : 'miss',
    );
    return;
  }
  ...
  log(
    ...
    a.timingSuccess
      ? defending
        ? 'Critical guard · damage reduced by 75% until your next action.'
        : 'Signal caught · critical chance raised.'
      : defending
        ? 'Normal guard · damage reduced by 65% until your next action.'
        : 'Signal missed · the strike continues.',
    a.timingSuccess ? 'timing' : 'miss',
  );
}
...
function enemyTurn(b, state, e) {
  ...
  e.turns++;
  const wantsCharge =
    ['charge', 'fortress', 'architect'].includes(mechanic) &&
    !e.charging &&
    (e.turns % 3 === 1 || (mechanic === 'architect' && e.bossPhase > 1));
  e.atb = 0;
  let command, chosen;
  if (wantsCharge) {
    command = {
    ...
    const group =
      charged &&
      (mechanic === 'architect' || mechanic === 'charge' || e.tier >= 5);
    command = {
      id: charged ? 'surge' : 'strike',
      name: charged
        ? mechanic === 'architect'
          ? e.bossPhase === 3
            ? 'Last Equation'
      ...
      power: charged
        ? mechanic === 'architect'
          ? 1.7 + e.bossPhase * 0.13
          : 1.95
        : mechanic === 'architect'
          ? 1 + e.bossPhase * 0.08
          : 1,
      stat: 'str',
      target: group ? 'allAllies' : 'ally',
      charged,
    };
    chosen = group
      ? targets
      : [targets[Math.floor(roll(state) * targets.length)]];
    e.charging = false;
  }
  const contact = command.effect === 'telegraph' ? 0.46 : 0.72;
  const assisted = Boolean(state.settings?.timingAssist);
  b.action = {
    id: ++b.actionSerial,
    side: 'enemy',
    command,
    participants: [e.uid],
    targets: chosen.map(actorId),
    elapsed: 0,
    contact,
    duration: command.effect === 'telegraph' ? 1.04 : 1.4,
    windowStart: contact - (assisted ? 0.36 : 0.28),
    windowEnd: contact - (assisted ? 0.055 : 0.095),
    timingEligible: command.effect !== 'telegraph',
    timingAttempted: false,
    timingSuccess: false,
    timingPressedAt: null,
    resolved: false,
    ...
    returnSelection: {
      mode: b.mode,
      selectedHero: b.selectedHero,
      lastSelectedHero: b.lastSelectedHero,
      cursor: b.cursor,
      target: b.target,
      pending: b.pending,
    },
...
function floating(b, actor, text, kind) {
  const bounds = actorBounds(actor.id, {
    ...
    facing: actor.side === 'hero' ? 'right' : 'left',
  });
  const visual = actorVisual(b, actor, {});
  b.floaters.push({
    x: visual.x,
    y: Math.max(65, visual.y + bounds.top / DISPLAY_SCALE - 7),
    text,
    kind,
    life: 1.18,
    maxLife: 1.18,
...
function takeDamage(b, target, amount, critical, timedGuard = false) {
  if (timedGuard || target.guarding)
    amount = Math.max(
      1,
      Math.round(amount * (timedGuard || target.criticalGuard ? 0.25 : 0.35)),
    );
  ...
  target.reaction = timedGuard ? 0 : 0.32;
  if (amount === 0) floating(b, target, 'WARD', 'shield');
  else
    floating(
      b,
      target,
      `${timedGuard ? 'GUARD · ' : critical ? '✦ ' : ''}${amount}`,
      timedGuard || critical ? 'crit' : 'damage',
    );
  if (!alive(target)) {
    target.atb = 0;
    target.charging = false;
    target.guarding = false;
    target.criticalGuard = false;
    target.shield = 0;
    target.defeatedAt =
      b.clock -
      Math.max(0, (b.action?.elapsed || 0) - (b.action?.contact || 0));
...
function phaseTransition(b, e) {
  ...
  const mechanic = mechanicFor(e);
  let phase = 1;
  if (mechanic === 'architect')
    phase = e.hp / e.maxHp <= 0.33 ? 3 : e.hp / e.maxHp <= 0.67 ? 2 : 1;
  else if (['fortress', 'charge'].includes(mechanic))
    phase = e.hp / e.maxHp <= (e.phaseAt || 0.5) ? 2 : 1;
  if (phase > e.bossPhase) {
    e.bossPhase = phase;
    if (mechanic === 'architect' || mechanic === 'fortress')
      e.shield += Math.ceil(e.maxHp * (mechanic === 'architect' ? 0.06 : 0.12));
    const msg =
      mechanic === 'architect'
        ? phase === 2
          ? 'The Architect unfolds a second sky. Watch its signal.'
          : 'The final equation fractures. Stand together.'
        : `${e.name} exposes its core and gathers force.`;
    log(b, msg, 'phase');
    notice(b, msg);
    e.charging = true;
  }
...
function resolveAction(b, state, a) {
  ...
  if (command.kind === 'item' && !a.itemSpendAtContact) {
    state.inventory[command.id] = (state.inventory[command.id] || 0) + 1;
    a.itemSpendAtContact = true;
  }
  const participants = a.participants
    .map((id) => actorById(b, id))
    .filter((h) => h && alive(h));
  if (!participants.length) {
    log(b, 'The action breaks before contact.', 'cancel');
    return;
  }
  const targets = a.targets
    .map((id) => actorById(b, id))
    .filter((t) => t && (command.effect === 'revive' ? !alive(t) : alive(t)));
  const actor = participants[0];
  if (command.kind === 'item') {
    const target = targets[0],
      result = consumeItem(state, command.id, target, target, {
        battle: true,
        confirmation: a.itemConfirmation,
      });
    if (!result.ok) {
      log(
        b,
        result.needsConfirmation
          ? 'The target changed. Supply kept.'
          : result.message,
        'cancel',
      );
      return;
    ...
    floating(
      ...
      result.effect === 'revive'
        ? 'RISE'
        : `${result.unit === 'MP' ? 'MP ' : ''}+${result.amount}`,
      'heal',
    );
    log(b, result.message, 'heal');
    cleanQueue(b);
    sync(b, state);
    return;
  }
  if (command.effect === 'retreat') {
    b.retreatPending = true;
    log(b, 'The crew withdraws along the path.', 'retreat');
    return;
  }
  if (command.effect === 'telegraph') {
    ...
    log(
      b,
      actor.telegraph ||
        `${actor.name} is preparing ${command.name}. Defend or raise a ward.`,
      'telegraph',
    );
    floating(b, actor, 'CHARGING', 'warning');
    return;
  }
  if (command.effect === 'guard') {
    actor.guarding = true;
    actor.criticalGuard = a.timingSuccess;
    a.critical = a.timingSuccess;
    floating(
      b,
      actor,
      a.timingSuccess ? 'CRITICAL GUARD' : 'GUARD',
      a.timingSuccess ? 'crit' : 'shield',
    );
    return;
  }
  ...
  if (harm) {
    const baseline =
      participants.reduce((sum, h) => sum + (h.crit || 5), 0) /
      participants.length /
      100;
    a.criticalChance = clamp(
      baseline + (a.side === 'hero' && a.timingSuccess ? 0.46 : 0),
      0.02,
      0.88,
    );
  ...
  const combined = participants.reduce(
    (sum, h) => sum + (h[stat] || h.str || 10),
    0,
  );
  let drained = 0;
  for (const target of targets) {
    if (harm) {
      const variance = 0.91 + roll(state) * 0.18;
      const base =
        (combined * (command.power || 1) * 1.6 + 8 - (target.def || 0) * 0.66) *
        variance;
      ...
      const criticalMultiplier =
        a.side === 'hero' && a.timingSuccess ? 1.44 : 1.6;
      const damage = Math.max(
        3,
        Math.round(base * (a.critical ? criticalMultiplier : 1)),
      );
      const applied = takeDamage(
        b,
        target,
        damage,
        a.critical,
        a.side === 'enemy' && a.timingSuccess,
      );
      drained += applied;
      log(
        b,
        `${target.name} · ${applied ? `${applied} damage` : 'ward holds'}`,
        'damage',
      );
      if (command.effect === 'slow' && alive(target)) {
        target.slowTurns = Math.max(target.slowTurns, 2);
        target.atb = Math.max(0, target.atb - 16);
      }
      ...
      const amount =
        command.kind === 'item'
          ? command.power
          : Math.round(30 + combined * (command.power || 1.8));
      const gain = Math.max(0, Math.min(target.maxHp - target.hp, amount));
      target.hp += gain;
      floating(b, target, `+${gain}`, 'heal');
      log(b, `${target.name} recovers ${gain} HP.`, 'heal');
      ...
      floating(b, target, `WARD ${amount}`, 'shield');
    } else if (command.effect === 'restoreMp') {
      const gain = Math.min(target.maxMp - target.mp, command.power || 30);
      target.mp += gain;
      floating(b, target, `MP +${gain}`, 'heal');
      log(b, `${target.name} recovers ${gain} MP.`, 'heal');
    } else if (command.effect === 'revive') {
      target.hp = Math.max(
        1,
        Math.round(
          target.maxHp *
            (command.power > 1 ? command.power / 100 : command.power || 0.35),
        ),
      );
      target.atb = 28;
      floating(b, target, 'RISE', 'heal');
    }
  }
  if (command.effect === 'drain' && drained) {
    const gain = Math.min(actor.maxHp - actor.hp, Math.round(drained * 0.4));
    actor.hp += gain;
    floating(b, actor, `+${gain}`, 'heal');
  }
  if (harm && a.critical)
    log(b, 'Critical strike · the signal breaks through.', 'crit');
  cleanQueue(b);
  sync(b, state);
}
...
function checkEnd(b, state) {
  ...
  log(
    b,
    b.result === 'victory'
      ? 'The signal holds. Victory.'
      : b.result === 'defeat'
        ? 'The expedition falls silent.'
        : 'A path remains. The crew retreats.',
    b.result,
  );
  return true;
}
...
export function updateBattle(b, state, dt) {
  ...
  for (const actor of [...b.heroes, ...b.enemies])
  ...
  if (b.action) {
    ...
    a.stage =
      a.elapsed < (a.windowStart ?? 0.3)
        ? 'anticipation'
        : a.elapsed < a.contact
          ? 'approach'
          : a.elapsed < a.contact + 0.15
            ? 'contact'
            : 'recovery';
    ...
    if (a.elapsed >= a.duration) {
      ...
      if (a.side === 'enemy' && a.returnSelection) {
        const previous = a.returnSelection,
          hero = actorById(b, previous.selectedHero);
        if (
          previous.mode === 'waiting' ||
          (hero && alive(hero) && hero.atb >= 100)
        )
  ...
  for (const h of b.heroes) {
    ...
    if (h.atb >= 100)
      arriving.push({
        id: h.id,
        when: (100 - before) / rate(h),
        index: h.index,
      });
  }
  arriving.sort((a, z) => a.when - z.when || a.index - z.index);
  for (const h of arriving)
    if (!b.readyQueue.includes(h.id)) b.readyQueue.push(h.id);
  cleanQueue(b);
  for (const e of b.enemies)
    if (alive(e)) e.atb = Math.min(100, e.atb + rate(e) * dt);
...
export function battleView(b, state) {
  ...
  return {
    ...
    clock: b.clock,
    selectedHero: b.selectedHero,
    focusHero: b.selectedHero || b.lastSelectedHero || b.heroes.find(alive)?.id,
    readyQueue: [...b.readyQueue],
    cursor: b.cursor,
    target: b.target,
    ...
    status:
      b.result ||
      (a
        ? `${a.command.name} · ${a.stage}`
        : b.mode === 'waiting'
          ? b.selectedHero
            ? `${selected(b).name} ready`
            : 'Gauges charging'
          : 'Wait · choose an action'),
    commands: ROOT_COMMANDS.map((name, i) => ({
      name,
      selected: b.mode === 'command' && b.cursor === i,
      unavailable:
        name === 'Retreat' &&
        (b.encounter.boss || b.enemies.some((e) => e.id === 'void_architect'))
          ? 'This confrontation must be faced.'
          : name === 'Item' && !listItems(state).length
            ? 'No field supplies.'
            : '',
    })),
    pending: b.pending && {
      ...
      name: b.pending.name,
      kind: b.pending.kind,
      effect: b.pending.effect,
      target: b.pending.target,
      participants: [...b.pending.participants],
    },
    techs: techs.map((t) => ({
      id: t.id,
      name: t.name,
      mp: t.mp,
      participants: t.heroes,
      effect: t.effect,
      target: t.target,
      description: t.description,
      unavailable: t.unavailable,
    })),
    items: listItems(state).map((item) => ({
      id: item.id,
      name: item.name,
      count: item.count,
    ...
    targets: targets.map((t) => ({
      ...
      name: t.name,
      side: t.side,
      level: t.side === 'enemy' ? enemyLevel(t) : null,
      hp: t.hp,
      maxHp: t.maxHp,
      selected: targets[b.target] === t,
    })),
    heroes: b.heroes.map((h) => ({
      ...
      shield: h.shield,
      guarding: h.guarding,
      criticalGuard: Boolean(h.criticalGuard),
      slowTurns: h.slowTurns,
      visual: actorVisual(b, h, state),
    })),
    enemies: b.enemies.map((e) => ({
      ...
      atb: e.atb,
      shield: e.shield,
      slowTurns: e.slowTurns,
      charging: e.charging,
      bossPhase: e.bossPhase,
      visual: actorVisual(b, e, state),
    })),
    action: a && {
      id: a.id,
      side: a.side,
      kind: a.command.kind,
      effect: a.command.effect,
      name: a.command.name,
      ...
      timingEligible: a.timingEligible,
      timingAttempted: a.timingAttempted,
      timingSuccess: a.timingSuccess,
      timingPressedAt: a.timingPressedAt ?? null,
      resolved: a.resolved,
      critical: a.critical,
    ...
    hints:
      b.mode === 'target'
        ? '↑ ↓ Target · Space / Enter Execute · ← Back'
        : b.mode === 'waiting'
          ? '↑ ↓ Ready hero · → / Enter Commands'
...
function bar(ctx, x, y, width, height, value, color) {
  ...
  ctx.fillStyle = color;
  ctx.fillRect(x, y, Math.round(width * clamp(value, 0, 1)), height);
}
...
function button(b, x, y, w, h, kind, data = {}) {
  b.hitAreas.push({ x, y, w, h, kind, ...data });
}
...
function defeatOpacity(b, actor) {
  ...
  if (actor.side === 'hero') return 0.6;
  if (!Number.isFinite(actor.defeatedAt)) return 0;
  return clamp(1 - (b.clock - actor.defeatedAt - 0.68) / 0.42, 0, 1);
}

function harborPlant(b, a) {
  const rune = b.heroes.find((h) => h.id === 'rune');
  const compression =
    a.elapsed >= 0.42 && a.elapsed < 0.54
      ? Math.sin(((a.elapsed - 0.42) / 0.12) * Math.PI) * 3
      : 0;
  return {
    x: (rune?.home.x || 247) + 26,
    y: (rune?.home.y || 278) - 49 + compression,
    compression,
  };
}
...
function enemyExchange(b, a) {
  if (
    ...
    return null;
  const defender = actorById(b, a.targets[0]),
    attacker = actorById(b, a.participants[0]);
  if (!defender || defender.side !== 'hero' || !attacker) return null;
  const formationRight = Math.max(
    ...b.heroes.map(
      (h) =>
        h.home.x +
        actorBounds(h.id, {
  ...
  const body = actorBounds(defender.id, {
    pose: 'guard',
    side: 'hero',
    scale: artScale(b, defender),
    facing: 'right',
  });
  const weapon = actorBounds(attacker.id, {
    pose: 'attack',
    side: 'enemy',
    scale: artScale(b, attacker),
    facing: 'left',
  });
  ...
  const contactHeight =
    {
      frost_revenant: 0.277,
      ember_lord: 0.475,
      ember_golem: 0.528,
      glacier_wolf: 0.632,
      mire_hulk: 0.737,
      wraith_core: 0.615,
      architect_herald: 0.394,
      frost_colossus: 0.292,
      magma_behemoth: 0.541,
      mire_warden: 0.314,
      void_architect: 0.547,
    }[attacker.id] ?? 0.54;
  const weaponY = weapon.top + (weapon.bottom - weapon.top) * contactHeight;
  const offsetY = (body.top * 0.47 - weaponY) / DISPLAY_SCALE;
  const target = {
  ...
  return {
    ...
    attacker: {
      x:
        target.x +
        (body.right * 0.35) / DISPLAY_SCALE -
        (weapon.left + (weapon.right - weapon.left) * 0.025) / DISPLAY_SCALE,
      y: target.y + offsetY,
    },
  };
}
...
function actorVisual(b, actor, state) {
  ...
  let x = home.x,
    y = home.y,
    pose = actor.hp <= 0 ? 'down' : actor.guarding ? 'guard' : 'idle',
    moving = false,
    progress = 0;
  let facing = actor.side === 'hero' ? 'right' : 'left',
    animationTime = b.clock;
  const a = b.action;
  const timedGuard =
    a?.side === 'enemy' &&
    a.timingSuccess &&
    a.targets.includes(actorId(actor));
  if (timedGuard && alive(actor)) pose = 'guard';
  if (actor.reaction > 0 && alive(actor)) {
    pose = 'hurt';
    x +=
      Math.round(Math.sin(actor.reaction * 34) * 3) *
      (actor.side === 'hero' ? -1 : 1);
  }
  const exchange = enemyExchange(b, a);
  if (exchange?.defender === actor) {
    const t = a.elapsed,
      arrival = a.contact - 0.24;
    const travel =
      t < arrival
        ? easing((t - 0.12) / (arrival - 0.12))
        : t < a.contact + 0.18
          ? 1
          : 1 -
            easing((t - a.contact - 0.18) / (a.duration - a.contact - 0.22));
    ...
    pose =
      actor.hp <= 0
        ...
        : actor.reaction > 0
          ...
          : moving
            ? 'move'
            : travel === 1 || timedGuard || actor.guarding
              ? 'guard'
              : 'idle';
  ...
  if (a?.participants.includes(actorId(actor)) && alive(actor)) {
    ...
    const supportive = ['heal', 'shield', 'restoreMp', 'revive'].includes(
      a.command.effect,
    );
    const cast =
      harborAnchor ||
      a.command.stat === 'int' ||
      ['heal', 'shield', 'restoreMp', 'revive', 'telegraph'].includes(
        a.command.effect,
      ) ||
      actor.id === 'vex' ||
      a.command.target === 'allEnemies';
    if (a.command.kind === 'defend') pose = 'guard';
    else if (harborAnchor) {
      pose = 'guard';
      y += harborPlant(b, a).compression;
    } else if (supportive) {
      pose = actor.id === 'rune' ? 'guard' : 'cast';
    } else if (a.command.id === 'harbor_break' && actor.id === 'kaida') {
      const target = a.targets.map((id) => actorById(b, id)).find(Boolean);
      const plant = harborPlant(b, a),
        runway = { x: plant.x - 39, y: plant.y + 43 };
      const stop = {
        x: (target?.home.x || 568) - 77,
        y: target?.home.y || 241,
      };
      const cruiseY = state.settings?.reducedMotion
        ? Math.min(plant.y, stop.y) - 20
        : 112;
      if (t < 0.14) {
        pose = 'anticipate';
      } else if (t < 0.32) {
        const p = easing((t - 0.14) / 0.18);
        x += (runway.x - x) * p;
        y += (runway.y - y) * p;
        pose = 'move';
        moving = true;
      } else if (t < 0.46) {
        const p = easing((t - 0.32) / 0.14);
        x = runway.x + (plant.x - runway.x) * p;
        y = runway.y + (plant.y - runway.y) * p;
        pose = 'guard';
      } else if (t < 0.54) {
        x = plant.x;
        y = plant.y;
        ...
        const p = easing((t - 0.54) / 0.14);
        x = plant.x + 40 * p;
        y = plant.y + (cruiseY - plant.y) * p;
        pose = 'guard';
      } else if (t < 0.82) {
        const p = easing((t - 0.68) / 0.14);
        x = plant.x + 40 + (stop.x - plant.x - 40) * p;
        y = cruiseY;
        pose = 'guard';
      } else if (t < a.contact) {
        const p = easing((t - 0.82) / (a.contact - 0.82));
        x = stop.x;
        y = cruiseY + (stop.y - cruiseY) * p;
        pose = 'attack';
      } else if (t < a.contact + 0.18) {
        x = stop.x;
        y = stop.y;
        pose = 'attack';
      } else {
        const p = easing(
          (t - a.contact - 0.18) / (a.duration - a.contact - 0.26),
        );
        x = stop.x + (home.x - stop.x) * p;
        y = stop.y + (home.y - stop.y) * p;
        if (!state.settings?.reducedMotion) y -= Math.sin(p * Math.PI) * 24;
        moving = p < 0.98;
        pose = moving ? 'move' : 'idle';
        facing = moving ? 'left' : 'right';
      }
      animationTime = t * 1.3;
      ...
      pose = t < 0.22 ? 'anticipate' : 'cast';
    } else if (t < 0.22) {
      pose = 'anticipate';
      x += (actor.side === 'hero' ? -1 : 1) * Math.round(easing(t / 0.22) * 5);
    } else if (cast) {
      pose = t < a.contact ? 'cast' : t < a.contact + 0.18 ? 'attack' : 'cast';
    } else {
      const target = a.targets.map((id) => actorById(b, id)).find(Boolean);
      if (target) {
        const participantIndex = a.participants.indexOf(actorId(actor));
        const spread =
          a.command.id === 'harbor_break'
            ? 0
        ...
        const travel =
          t < a.contact - 0.16
            ? easing((t - 0.23) / Math.max(0.1, a.contact - 0.39))
            : t < a.contact + 0.16
              ? 1
              : 1 -
                easing(
                  (t - a.contact - 0.16) / (a.duration - a.contact - 0.25),
                );
        x += (stopX - x) * travel;
        y += (stopY - y) * travel;
        if (!state.settings?.reducedMotion)
          y -=
            ...
            (a.command.id === 'harbor_break' ? 42 : 11);
        moving = t < a.contact - 0.18 || t > a.contact + 0.2;
        pose =
          t < a.contact - 0.19
            ? 'move'
            : t < a.contact - 0.06
              ...
              : t < a.contact + 0.18
                ? 'attack'
                : 'move';
        if (t > a.contact + 0.2 && t < a.duration - 0.1)
          facing = actor.side === 'hero' ? 'left' : 'right';
        if (t >= a.duration - 0.1) {
          moving = false;
          pose = 'idle';
        }
  ...
  return {
    ...
    facing,
    animationTime,
    opacity: defeatOpacity(b, actor),
  };
}

function brackets(ctx, x, y, w, h, color) {
  const s = 7;
  line(ctx, x, y + s, x, y, color);
  line(ctx, x, y, x + s, y, color);
  line(ctx, x + w - s, y, x + w, y, color);
  line(ctx, x + w, y, x + w, y + s, color);
  line(ctx, x, y + h - s, x, y + h, color);
  line(ctx, x, y + h, x + s, y + h, color);
  line(ctx, x + w - s, y + h, x + w, y + h, color);
  line(ctx, x + w, y + h, x + w, y + h - s, color);
}
...
function groundRing(ctx, x, y, rx, color, dashed = false) {
  ...
  ctx.beginPath();
  ctx.ellipse(Math.round(x), Math.round(y) + 1, rx, 8, 0, 0, Math.PI * 2);
  ctx.stroke();
  ctx.setLineDash([]);
  line(ctx, x - rx - 6, y, x - rx + 2, y, color);
  line(ctx, x + rx - 2, y, x + rx + 6, y, color);
}
...
function spellArc(
  ...
  for (let i = 0; i <= steps; i++) {
    ...
    for (let band = 0; band < width; band++) {
      ...
      ctx.fillRect(
        Math.round((cx + Math.cos(angle) * r) / 2) * 2,
        Math.round((cy + Math.sin(angle) * r * squash) / 2) * 2,
        2,
        2,
      );
    }
...
function spellShard(ctx, x, y, length, color) {
  ...
  ctx.fillRect(
    x - 1,
    y - Math.round(length * 0.6),
    2,
    Math.max(2, Math.round(length * 0.5)),
  );
}
...
function spellRibbon(ctx, from, to, phase, color) {
  ...
  for (let i = 0; i <= steps; i++) {
    if (i % 17 === 0) continue;
    const p = i / steps,
      wave = Math.sin(p * Math.PI) * Math.sin(p * Math.PI * 3 - phase * 9) * 3;
    const x = Math.round(from.x + dx * p - uy * wave),
      y = Math.round(from.y + dy * p + ux * wave);
...
function drawEffect(ctx, b, state) {
  ...
  const support = ['heal', 'shield', 'restoreMp', 'revive', 'guard'].includes(
    a.command.effect,
  );
  const spell = a.command.stat === 'int' || active[0]?.id === 'vex';
  const prism = a.command.id === 'prism_cut',
    dawn = a.command.id === 'concord_dawn';
  ...
  if (!state.settings?.reducedMotion) {
    for (const actor of active) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        ctx.fillRect(
          Math.round(p.x + direction * (12 + i * 7)),
          Math.round(p.y - 2 - stride * 0.35),
          6 - i,
          2,
        );
      }
  ...
  if (
    ...
    !prism &&
    !dawn &&
    t < (a.command.id === 'harbor_break' ? 0.54 : a.contact + 0.1)
  ) {
    ...
    for (let i = 0; i < positions.length; i++) {
      ...
      if (support) {
        spellRibbon(
          ctx,
          { x: from.x, y: from.y - 24 },
          { x: to.x, y: to.y - 24 },
          t,
          C.pale,
        );
        for (let petal = 0; petal < 3; petal++) {
          const flight = (t * 0.8 + petal / 3) % 1;
          spellShard(
            ctx,
  ...
  if (a.command.id === 'harbor_break' && t >= 0.36 && t < 0.68) {
    const p = harborPlant(b, a),
      power = clamp((t - 0.36) / 0.18, 0, 1);
    ctx.globalAlpha = t > 0.54 ? 1 - (t - 0.54) / 0.14 : power;
  ...
  if (prism && t >= 0.2 && t < a.contact + 0.35) {
    const kaida = active.find((h) => h.id === 'kaida'),
      vex = active.find((h) => h.id === 'vex');
    const focus = {
      x: (kaida?.home.x || 192) + 54,
      y: (kaida?.home.y || 200) - 44,
    };
    const origin = vex && actorVisual(b, vex, state);
    const fade = t > a.contact ? 1 - (t - a.contact) / 0.35 : 1;
    ctx.globalAlpha = fade;
    if (origin)
      spellRibbon(
        ctx,
        { x: origin.x + 35, y: origin.y - 55 },
        focus,
        t,
        C.amber,
      );
    for (const [i, stroke] of [C.rose, C.paper, C.teal].entries()) {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(focus.x, focus.y - 13 - i * 2);
      ctx.lineTo(focus.x + 9 + i * 3, focus.y);
      ctx.lineTo(focus.x, focus.y + 13 + i * 2);
      ctx.lineTo(focus.x - 9 - i * 3, focus.y);
      ctx.closePath();
      ctx.stroke();
    }
    ...
    if (t >= a.contact - 0.19) {
      const travel = easing((t - a.contact + 0.19) / 0.19);
      for (const [i, target] of targets.entries()) {
        const end = { x: target.home.x, y: target.home.y - 37 };
        const dx = end.x - focus.x,
          dy = end.y - focus.y,
        ...
        ctx.lineTo(tip.x - ux * 17 - uy * 3, tip.y - uy * 17 + ux * 3);
        ctx.lineTo(tip.x - ux * 30, tip.y - uy * 30);
        ctx.lineTo(tip.x - ux * 17 + uy * 3, tip.y - uy * 17 - ux * 3);
        ctx.closePath();
        ctx.fill();
        line(
        ...
        for (let mote = 0; mote < 3; mote++) {
          const back = 36 + mote * 11,
            side = (mote % 2 ? -1 : 1) * (4 + mote * 2);
          ctx.fillStyle = mote === 0 ? C.paper : C.rose;
          ctx.fillRect(
            Math.round(tip.x - ux * back - uy * side),
            Math.round(tip.y - uy * back + ux * side),
            3 - (mote % 2),
            2,
          );
        }
  ...
  if (dawn && t >= 0.18 && t < a.contact + 0.46) {
    ...
    for (const [i, actor] of active.entries()) {
      ...
      spellArc(
        ...
        r + i * 4,
        -Math.PI * 0.92 + i * 1.6,
        -Math.PI * 0.92 + i * 1.6 + Math.PI * 1.1,
        [colors[i], C.paper],
        2,
      );
    }
    ...
    if (t >= a.contact) {
      ...
      for (const target of targets)
        for (let i = 0; i < 3; i++)
          spellShard(
            ctx,
            target.home.x - 12 + i * 12,
            target.home.y - 27 - p * 25,
            6 + i * 3,
            i % 2 ? C.amber : C.pale,
          );
    }
  ...
  if (
    ...
    !prism &&
    !dawn &&
    (spell || support || a.command.effect === 'telegraph')
  ) {
    for (const actor of active) {
      const p = actorVisual(b, actor, state),
        r = 13 + Math.floor(t * 12);
      ctx.strokeStyle = color;
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(p.x, p.y - 48 - r);
      ctx.lineTo(p.x + r, p.y - 48);
      ctx.lineTo(p.x, p.y - 48 + r);
      ctx.lineTo(p.x - r, p.y - 48);
      ctx.closePath();
      ctx.stroke();
      for (let i = 0; i < 5; i++) {
        ...
        ctx.fillStyle = color;
        ctx.fillRect(
          Math.round(p.x + Math.cos(angle) * (r + 6)),
          Math.round(p.y - 48 + Math.sin(angle) * (r + 6)),
          2,
          2,
        );
      }
  ...
  if (
    ...
    for (const target of targets) {
      const start = actorVisual(b, active[0], state),
        p = easing((t - a.contact + 0.12) / 0.12);
      const x = start.x + (target.home.x - start.x) * p,
        y = start.y - 47 + (target.home.y - start.y + 20) * p;
      line(ctx, start.x, start.y - 47, x, y, color);
      ctx.fillStyle = C.paper;
      ctx.fillRect(Math.round(x) - 3, Math.round(y) - 3, 6, 6);
    }
  }
  if (drone && t >= a.contact - 0.045 && t < a.contact + 0.15) {
    const p = actorVisual(b, drone, state),
      bounds = actorBounds(drone.id, {
        pose: 'attack',
        side: 'enemy',
        scale: artScale(b, drone),
        facing: 'left',
      });
    const from = {
      x: p.x + bounds.left / DISPLAY_SCALE + 3,
      y:
        p.y +
        (bounds.top + (bounds.bottom - bounds.top) * 0.58) / DISPLAY_SCALE,
    };
    for (const target of targets) {
      const hit = actorVisual(b, target, state),
        body = actorBounds(target.id, {
          pose: hit.pose,
          side: target.side,
          scale: artScale(b, target),
          facing: hit.facing,
        });
      const to = {
        x: hit.x + (body.right / DISPLAY_SCALE) * 0.8,
        y: hit.y + (body.top / DISPLAY_SCALE) * 0.5,
      };
      ctx.globalAlpha = clamp(1 - (t - a.contact) / 0.15, 0, 1);
      ctx.strokeStyle = '#6cc5b6';
      ctx.lineWidth = 3;
      ctx.beginPath();
  ...
  if (enemyWave && t >= a.contact - 0.2 && t < a.contact + 0.38) {
    ...
    for (const target of targets) {
      const hit = actorVisual(b, target, state),
        radius = t < a.contact ? 18 * p : 18 + after * 21;
      spellArc(
        ctx,
        hit.x,
        hit.y,
  ...
  if (t >= a.contact && t < a.contact + 0.38) {
    const p = (t - a.contact) / 0.38;
    for (const target of targets) {
      const hit = actorVisual(b, target, state),
        x = hit.x,
        y = hit.y - 32;
      ...
      if (support) {
        ...
        for (let i = 0; i < 5; i++) {
          const px = x - 25 + i * 12,
            py = y + 25 - p * 52 + Math.sin(i * 9) * 7;
          line(ctx, px - 3, py, px + 3, py, color);
          line(ctx, px, py - 3, px, py + 3, color);
        }
      } else if (
        a.command.effect !== 'telegraph' &&
        !prism &&
        !dawn &&
        !drone &&
        !enemyWave
        ...
        for (let i = 0; i < 7; i++) {
          const angle = (i * Math.PI * 2) / 7 + 0.4;
          const px = x + Math.cos(angle) * radius,
            py = y + Math.sin(angle) * radius * 0.65;
          ctx.fillStyle = i % 2 ? color : C.paper;
          ctx.fillRect(Math.round(px), Math.round(py), i % 2 ? 4 : 2, 2);
        }
        ctx.strokeStyle = a.critical ? C.amber : C.paper;
        ctx.lineWidth = a.critical ? 3 : 2;
        ctx.beginPath();
        ctx.moveTo(x - 24 + p * 10, y + 25);
        ctx.lineTo(x + 26 - p * 8, y - 25);
        ctx.stroke();
        if (a.critical) {
          ctx.lineWidth = 1;
          ctx.beginPath();
...
export function drawBattle(ctx, b, state) {
  ...
  const contactPriority = (actor) =>
    ...
    b.action.elapsed <= b.action.contact + 0.24 &&
    b.action.participants.includes(actorId(actor))
      ? 1000
      : 0;
  ...
  const actors = [...b.heroes, ...b.enemies].sort(
    (a, z) =>
      actorVisual(b, a, state).y +
      contactPriority(a) -
      actorVisual(b, z, state).y -
      contactPriority(z),
  );
  for (const actor of actors) {
    ...
    const bounds = actorBounds(actor.id, {
      ...
      side: actor.side,
      scale,
      facing: hero ? 'right' : 'left',
    });
    const boundsLeft = bounds.left / DISPLAY_SCALE - 5,
      boundsTop = bounds.top / DISPLAY_SCALE - 5;
    const boundsWidth = (bounds.right - bounds.left) / DISPLAY_SCALE + 10,
      boundsHeight = (bounds.bottom - bounds.top) / DISPLAY_SCALE + 10;
    ...
    if (hero && b.selectedHero === actor.id && alive(actor))
      groundRing(ctx, actor.home.x, actor.home.y, 25, C.amber, true);
    ...
    const artOptions = {
      ...
      facing: p.facing,
      pose: b.result === 'victory' && hero && alive(actor) ? 'victory' : p.pose,
      poseProgress: p.progress,
      actionElapsed: b.action?.elapsed || 0,
      strikeProgress: b.action
        ? clamp((b.action.elapsed - b.action.contact + 0.19) / 0.37, 0, 1)
        : 0,
      moving: p.moving,
      scale,
    };
    ctx.save();
    ctx.scale(1 / DISPLAY_SCALE, 1 / DISPLAY_SCALE);
    if (hero)
      drawHero(
        ctx,
        actor.id,
        Math.round(p.x * DISPLAY_SCALE),
        Math.round(p.y * DISPLAY_SCALE),
        artOptions,
      );
    else
      drawEnemy(
    ...
    if (highlighted.includes(actor)) {
      ...
      if (hero)
        text(
          ...
          group ? 'ALL' : 'TARGET',
          actor.home.x,
          Math.max(62, actor.home.y + boundsTop - 6),
          C.paper,
          8,
          'center',
        );
  ...
  for (const f of b.floaters) {
    ...
    const color =
      f.kind === 'crit' || f.kind === 'warning'
        ? C.amber
        : f.kind === 'heal' || f.kind === 'shield'
          ? C.pale
          : C.paper;
  ...
  if (b.result) {
    ...
    line(ctx, 241, 188, 527, 188, C.teal);
    text(
      ctx,
      b.result === 'victory'
        ? 'The signal holds'
...
export function battleIntent(b, state, area) {
  ...
  if (b.itemConfirmation) {
    if (area.kind === 'item-confirm')
      return finishItemConfirmation(b, state, true);
    if (area.kind === 'item-cancel' || area.kind === 'back')
      return finishItemConfirmation(b, state, false);
    return true;
  }
  ...
  if (area.kind === 'breadcrumb') {
    ...
    if (area.stage === 1 && b.selectedHero) {
      ...
      b.cursor = previous?.returnCursor || 0;
      b.pending = null;
    }
    return true;
  }
  if (
    (area.kind === 'hero' ||
      (area.kind === 'actor' && actorById(b, area.id)?.side === 'hero')) &&
    b.mode !== 'target'
  ) {
    if (!b.readyQueue.includes(area.id)) {
      notice(b, 'That companion is not ready yet.');
      return true;
    }
    b.selectedHero = area.id;
    b.lastSelectedHero = area.id;
    b.mode = 'command';
    b.cursor = 0;
    b.pending = null;
    return true;
  ...
  if (
    (area.kind === 'actor' || area.kind === 'hero' || area.kind === 'target') &&
    b.mode === 'target'
  ) {
    const index = validTargets(b, b.pending).findIndex(
      (t) => actorId(t) === area.id,
    );
  ...
  if (area.kind === 'back') {
    battleKey(b, state, 'ArrowLeft');
    return true;
  }
  if (area.kind === 'execute') {
    commitTarget(b, state);
    return true;
  }
  if (area.kind === 'open' && b.selectedHero) {
    b.mode = 'command';
    b.cursor = 0;
    return true;
  }
...
export function battleClick(b, state, x, y) {
  ...
  const area = [...b.hitAreas]
    .reverse()
    .find((a) => x >= a.x && y >= a.y && x <= a.x + a.w && y <= a.y + a.h);
