import { ITEMS, itemRarityName } from './content.js';
import {
  communityRegion,
  communityStatus,
      ...
      const next =
        status.reforge.toTier > weapon.tier
          ...
          : weapon.nextLevel
            ...
            : 'Fully reforged.';
      message = `${kind === 'restore' ? `${status.name} restoration complete · Added to inventory` : 'Reforge complete'} · ${weapon.name} — ${itemRarityName(weapon.item)} · ${weapon.heroName}. ${next}`;
    }
    ...
    return `<div class="community-view" data-community="${status.id}">
      ...
      <section class="community-reward" ${itemAttributes(weapon.item)} aria-label="Community reward">
        ...
        <div class="community-reward-status"><label class="community-receipt"><input type="checkbox" disabled ${received ? 'checked' : ''} aria-label="Reward received"><small>${received ? 'Received' : `Complete “${esc(finalProject.name)}” to receive`}</small></label>${received ? button('View in inventory', 'community-inventory') : ''}${
          status.complete
            ? `<div class="community-reforge">${reforge.toTier > weapon.tier ? `<div class="community-reforge-target">Improve ${itemBadges(ITEMS[reforge.itemId])}<span>Spend ${esc(costText(reforge.cost))}</span></div>${statsMarkup(ITEMS[reforge.itemId], weapon.item)}` : ''}<p class="community-requirement">${esc(reforge.reason)}</p>${this.controls('reforge', '', reforge.eligible, 'Reforge')}</div>`
            : ''
        }</div>
      </section>
    </div>`;
