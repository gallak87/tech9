...
const ready = (stage) => {
  const s = createState();
  s.tier = stage.tier;
  s.buildings = { ...stage.buildings };
  s.flags = Object.fromEntries(stage.flags.map((id) => [id, true]));
  s.resources = { ...stage.cost };
  return s;
};

for (const stage of stages)
  test(`${stage.name}: each building, story and cost blocks advancement with its own instruction`, () => {
    const cases = [
      ...Object.keys(stage.buildings).map((id) => ({
        id,
        type: 'building',
        remove: (s) => s.buildings[id]--,
      })),
      ...stage.flags.map((id) => ({
        id,
        type: 'story',
        remove: (s) => delete s.flags[id],
      })),
      ...Object.keys(stage.cost).map((id) => ({
        id,
        type: 'resource',
        remove: (s) => s.resources[id]--,
      })),
    ];
    for (const missing of cases) {
      ...
      assert.equal(
        status.requirements.filter((r) => r.met).length,
        status.requirements.length - 1,
      );
      ...
      assert.equal(
        JSON.stringify(s),
        before,
        'A blocked attempt must not change resources, tier or flags',
      );
    }
    ...
    assert.equal(
      JSON.stringify(s),
      before,
      'Reading eligibility must not change a save',
    );
...
test('advancement lists every unmet requirement and precise resource shortfalls together', () => {
  const s = createState();
  s.resources = { food: 34.5, ore: 35, energy: 0, renown: 0 };
  const status = tierEligibility(s);
  assert.deepEqual(
    status.missing.map((r) => r.id),
    ['town_center', 'beacon_restored', 'food', 'energy', 'renown'],
  );
...
test('legitimate Town Center upgrade and beacon restoration immediately unlock Reclaimer', () => {
  ...
  for (const [id, n] of Object.entries(stages[0].cost))
    assert.equal(s.resources[id], before[id] - n);
