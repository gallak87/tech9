...
Community restoration cards show three ordered local projects, clear costs, permanent completion marks, and inline Confirm/Cancel without moving the card layout. Keep balances visible above the world scrim. The completion reward names the weapon, owner, Exotic identity, power tier, and next hometown reforge threshold. After the automatic award, keep a visible receipt showing whether the weapon is in inventory or equipped, with a direct inventory link. Present future reforging separately from receiving and equipping the reward. The Exotic badge is separate from green/blue/purple/gold power colors; one weapon illustration persists through all four bands.
