...
try{
 ...
 await page.evaluate(async state=>{const g=window.__ECHO__.game,{saveState}=await import('/src/persistence.js');saveState(state,1);g.load(1);window.hudAdvance=g.update.bind(g);g.update=()=>{};},earned.state);
 report.party=(await page.evaluate(()=>window.__ECHO__.snapshot())).battle.heroes;assert.equal(report.party.find(h=>h.id==='rune').maxHp,1132);
 await page.waitForTimeout(40);await page.screenshot({path:reviewRoot + 'hp-hud-earned-crown-idle.png'});
 const mode=await page.evaluate(()=>{const g=window.__ECHO__.game;let n=0;while((!g.battle.selectedHero||g.battle.action)&&n++<1800)window.hudAdvance(1/60);if(n>=1800)throw Error('No ready hero');return g.battle.mode;});
 if(mode==='waiting')await page.keyboard.press('Enter');await page.keyboard.press('Enter');await page.keyboard.press('Enter');
 await page.evaluate(()=>{const a=window.__ECHO__.game.battle.action;window.hudAdvance(a.contact+.025-a.elapsed);});
 report.text=await page.evaluate(()=>{
  const g=window.__ECHO__.game,c=new OffscreenCanvas(960,540).getContext('2d'),original=c.fillText,draws=[];
 ...
 report.contact=await page.evaluate(()=>window.__ECHO__.snapshot().battle);assert.equal(report.contact.action.resolved,true);
