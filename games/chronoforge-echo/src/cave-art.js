...
const f = (biome, part, x, y, w, h, anchorX, anchorY) => ({biome, part, x, y, w, h, anchorX, anchorY});
const kit = (id, file, frames, backgroundSeeds) => ({
  id, source: `assets/world/${file}-source.png`, kind: 'caveKit',
...
export const CAVE_KITS = [
  kit('caves_coast_desert', 'caves-coast-desert-v1', [
  ...
  ], [[914,153],[997,775],[937,219],[815,394],[930,809],[633,272],[844,135],[890,146],[1150,430]]),
  kit('caves_forest_mire', 'caves-forest-mire-v1', [
  ...
  ], [[980,785],[893,760],[1083,820],[1186,854],[1196,887],[1060,198],[981,177],[1156,240],[1018,187],[1098,234],[1040,863],[688,981],[928,822],[577,769],[103,851],[1124,176],[932,785],[1150,184]]),
  kit('caves_orbital_frost', 'caves-orbital-frost-v1', [
  ...
  ], [[922,426],[942,561],[984,1135],[984,577],[786,451]]),
  kit('caves_crater_crown', 'caves-crater-crown-v1', [
  ...
  ], [[964,814],[820,153],[464,733],[880,966],[985,1090],[1037,798],[1030,764],[943,231]]),
];

export const CAVE_BIOMES = ['coast','desert','forest','mire','volcanic','snow','ice','alien'];
export const CAVE_FLOOR_ASSET = {
  id: 'cave_floor_materials', source: 'assets/world/cave-floor-materials-v1-source.png',
  kind: 'caveFloor', columns: 4, rows: 2, required: true,
  metadata: {sourceWidth: 1774, sourceHeight: 887, preserveSourceAlpha: true,
    frames: CAVE_BIOMES.map((biome,i) => f(biome,'floor',Math.floor(i%4*443.5)+2,Math.floor(i/4)*443+2,439,439,0,0))},
};
export const CAVE_ASSETS = [...CAVE_KITS, CAVE_FLOOR_ASSET];

const frames = new Map(CAVE_KITS.flatMap(k => k.metadata.frames.map(f => [`${f.biome}:${f.part}`, f])));
export function caveArtFrame(biome, part) {return frames.get(`${biome}:${part}`) || null;}
