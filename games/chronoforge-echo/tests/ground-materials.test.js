...
import {balanceMaterialLighting,blendRepeatEdges,shoreDistance,lavaShoreMix,forestShoreMix} from '../src/ground-materials.js';
import {readPngPixels} from './png-pixels.js';

for(const name of ['crater-ember','forest-veil'])test(name+' materials repeat without border color jumps and retain their detailed centers',()=>{
 const source=readPngPixels(new URL('../public/assets/'+name+'-ground-source.png',import.meta.url));
...
test('forest riverbanks blend roots and damp soil into shallows on both banks',()=>{
 const bank=forestShoreMix(0,600,300,true),shallow=forestShoreMix(27,600,300,true);
 assert.ok(bank.roots>.8&&bank.shallows>.95);
 assert.ok(shallow.shallows>0&&shallow.roots===0);
 assert.ok(forestShoreMix(0,600,300,false).roots>.6);
 for(const water of [true,false])assert.deepEqual(forestShoreMix(60,600,300,water),{shallows:0,roots:0,shade:0});
});
...
test('forest lighting correction removes broad stripes without flattening local texture',()=>{
 ...
 for(let y=0;y<height;y++)for(let x=0;x<width;x++){
  const i=(y*width+x)*4,value=70+x*.7+y*.3+(x%2?8:-8);
  data.set([value,value,value,255],i);
 }
 const stripMean=(pixels,x)=>{let sum=0;for(let y=0;y<height;y++)for(let dx=0;dx<8;dx++)sum+=pixels[(y*width+x+dx)*4];return sum/(height*8);};
 const before=stripMean(data,48)-stripMean(data,8);
 balanceMaterialLighting(data,width,height);
 assert.ok(Math.abs(stripMean(data,48)-stripMean(data,8))<before*.2,'large lighting variation reduced');
 const center=(32*width+32)*4;
 assert.ok(data[center+4]-data[center]>12,'fine light/dark texture stays crisp');
