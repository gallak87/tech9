// ─────────────────────────────────────────────────────────────────────────────
// settlement — LANE STUB.
//
// Town centre, farms, mines, extractors, workers, resource ticks, four tech tiers.
//
// Phase 0 ships the seam, not the feature. `main.js` already installs this and
// hands it the shared `ctx`; the settlement lane fills it in without touching a
// single shared-core file. Do not widen this signature — add fields to the
// returned API instead.
//
// Owned by the `settlement` lane. See CONTRACT.md §1.
// ─────────────────────────────────────────────────────────────────────────────
...
export function installSettlement(ctx) {
  return {
     ...
     *  `?showcase=settlement` boots straight into it and the critic reviews the
