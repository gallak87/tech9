...
import {exportSave, parseSaveFile, importSave, saveMeta, MAX_SAVE_BYTES} from './persistence.js';
...
export class SaveTransfer {
  ...
  exportSlot(slot) {
    ...
    try {
      ...
      ui.notice = 'Exported ' + (slot === 'checkpoint' ? 'autosave' : 'field record ' + slot)
  ...
  requestImport(slot) {
    ...
    const restore = () => {
      ...
      if (ui.menu && ui.tab === 5) ui.restoreShopRow('import-save:' + slot, returnScroll);
    };
    ...
    input.addEventListener('change', async () => {
      ...
      try {
        ...
        if (sequence !== this.sequence || !ui.menu || ui.tab !== 5 || ui.panel?.type === 'confirm') return;
        this.confirmImport(record, slot);
      } catch (error) {
        if (sequence === this.sequence && ui.menu && ui.tab === 5) {
          ui.feedback({ok: false, message: 'Could not import: ' + error.message});
  ...
  confirmImport(record, slot) {
    ...
    const label = slot === 'checkpoint' ? 'Autosave' : 'Field record ' + String(slot).padStart(2, '0');
    ...
    const party = record.state.heroes.map(hero => `${HEROES[hero.id].name} LV ${hero.level}`).join(', ');
    ui.confirm(
      (occupied ? 'Replace ' : 'Import into ') + label + '?',
      `Import ${party} into this row?${occupied ? ' The existing record will be replaced.' : ''}`
        + (slot === 'checkpoint' ? ' Future automatic checkpoints will update this row again.' : ''),
      finish,
      {
        eyebrow: 'IMPORT SAVE',
    ...
    ui.root.querySelector('[data-do="confirm-no"]')?.focus({preventScroll: true});
  }
}
