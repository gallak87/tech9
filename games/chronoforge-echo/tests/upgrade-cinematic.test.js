  ...
  const calls = [],
    game = {
      ...
      upgradeTour: {
        ...
        plan: null,
        clock: null,
        openUpgrade(plan, done) {
          calls.push('reveal');
          this.open = true;
          this.plan = plan;
          this.done = done;
          this.clock = new UpgradeTour();
          this.clock.play();
        },
        close() {
          this.open = false;
        ...
        finish(skip = false) {
          if (skip) this.clock.skip();
          else this.clock.advance(20);
          assert.equal(this.clock.phase, 'complete');
          this.close();
          this.done();
        },
      },
    ...
    for (const level of [1, 2, 3])
      for (const skip of [false, true]) {
        ...
        for (const key of Object.keys(before.resources))
          assert.equal(
            game.state.resources[key],
            before.resources[key] - (cost[key] || 0),
          );
        assert.deepEqual(
          loadState('checkpoint', storage),
          game.state,
          'Reload during the reveal retains the paid upgrade',
        );
        ...
        assert.equal(
          ...
          undefined,
          'Repeated activation during the reveal is ignored',
        );
        game.upgradeTour.plan.before.resources.ore = 0;
        game.upgradeTour.plan.after.heroes[0].hp = 1;
        ...
        assert.equal(game.ui.focus, 'build:town_center');
        for (const key of [
          'region',
          'x',
          'y',
          ...
          assert.deepEqual(game.state[key], before[key], key);
      }
});
...
test('unaffordable, locked and maxed upgrades show their error without a reveal or checkpoint', () => {
  for (const change of [
    (s) => (s.resources.ore = 0),
    (s) => (s.flags.haventide_liberated = false),
    (s) => (s.buildings.town_center = 2),
    (s) => (s.buildings.town_center = 4),
  ]) {
    ...
    assert.equal(game.upgradeTour.open, false);
    assert.equal(game.ui.panel.type, 'build');
  }
});
...
test('other structures still build normally and preview/menu actions cannot spend resources', () => {
  ...
  assert.equal(game.state.buildings.farm, 1);
  assert.equal(game.upgradeTour.open, false);
  ...
  for (const mutate of [
    ...
    mutate(fixture.game);
    const before = structuredClone(fixture.game.state);
    assert.equal(performBuild(fixture.game, 'town_center'), undefined);
    assert.deepEqual(fixture.game.state, before);
...
test('a failed cinematic restores settlement works while preserving the successful saved upgrade', () => {
  ...
  game.upgradeTour.openUpgrade = function () {
    this.open = true;
...
test('all town reveals frame both exterior sizes and preserve the actual indoor camera and crew', () => {
  for (const { region } of TOWN_CENTERS)
    for (const level of [1, 2, 3]) {
      ...
      const view = {
        camera: { x: 137, y: 325 },
        actors: [{ id: 'kaida', x: 617, y: 770, facing: 'left' }],
      };
      const cameras = upgradeTourCameras(outside, inside, plan, view);
      assert.deepEqual(cameras.desk, view.camera);
      assert.deepEqual(cameras.actors, view.actors);
      assert.notEqual(cameras.desk, view.camera);
      assert.notEqual(cameras.actors, view.actors);
      const entrance = outside.objects.find(
        (o) => o.id === region + '_entrance',
      );
      for (const state of [plan.before, plan.after]) {
        const b = townCenterBounds(entrance, state),
          anchor = {
            x: entrance.x - cameras.outside.x,
            y: entrance.y - cameras.outside.y,
          };
...
test('the approved zoom stays outside, restrained motion disables it, and sparkles rise once', () => {
  for (const id of [
    ...
    const shot = { id, progress: 0.8 };
    for (const frame of ['deskBefore', 'deskAfter'])
      assert.equal(upgradeShotScale(shot, frame), 1);
    assert.equal(upgradeShotScale(shot, 'outsideAfter', true), 1);
  }
  assert.equal(
    upgradeShotScale({ id: 'before', progress: 0 }, 'outsideBefore'),
    1,
  );
  assert.equal(
    upgradeShotScale({ id: 'before', progress: 1 }, 'outsideBefore'),
    1.02,
  );
  assert.equal(
    upgradeShotScale({ id: 'upgrade', progress: 0.5 }, 'outsideAfter'),
    1.02,
  );
  const early = upgradeSparkles(0.2),
    late = upgradeSparkles(0.3);
  assert.equal(early.length, 36);
  assert.equal(late.length, 36);
  assert.ok(late.every((p, i) => p.y < early[i].y));
  assert.deepEqual(upgradeSparkles(0.85), []);
  const targets = [
    { x: 140, y: 190, width: 220, height: 150 },
    { x: 580, y: 150, width: 170, height: 230 },
  ];
  const indoorEarly = interiorUpgradeSparkles(0.1, targets),
    indoorLate = interiorUpgradeSparkles(0.18, targets);
  assert.equal(indoorEarly.length, 16);
  assert.equal(indoorLate.length, 16);
  assert.ok(
    indoorEarly.length < early.length,
    'The interior shimmer stays lighter than the exterior burst',
  );
  assert.ok(
    indoorLate.every((p, i) => p.y < indoorEarly[i].y && p.alpha <= 0.65),
  );
  assert.ok(
    indoorEarly.every((p) =>
      targets.some(
    ...
    'Sparkles start over furnishings',
  );
  assert.deepEqual(interiorUpgradeSparkles(0.5, targets), []);
  assert.deepEqual(interiorUpgradeSparkles(0.1, []), []);
});
