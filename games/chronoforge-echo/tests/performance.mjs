...
try{
  ...
  for(const id of (process.env.SCENES?.split(',')||['haventide','emberline','forest_veil','mire_bog','crater_ember','orbital_reach','frost_canyon','last_crown','haventide_town','hav_house','frost_cave','battle-four','final'])){
    const prep=await page.evaluate(async id=>{const t=performance.now();if(id==='battle-four'||id==='final')__ECHO__.preset(id);else{__ECHO__.preset('party');__ECHO__.goto(id);}await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));return performance.now()-t;},id);
    await page.waitForTimeout(300);await page.evaluate(()=>{__ECHO__.game.frameTimes.length=0;});
    await page.waitForTimeout(1400);
    const s=await page.evaluate(()=>__ECHO__.snapshot()),a=s.frameTimes.filter(t=>t>0).sort((x,y)=>x-y),p95=a[Math.floor(a.length*.95)],mean=a.reduce((n,x)=>n+x,0)/a.length;
