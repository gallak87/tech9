...
try{
 ...
 await page.evaluate(()=>{const g=__ECHO__.game;g.ui.showVendor({id:'review-rest',name:'Wayfarer’s rest',service:'rest'});});await page.screenshot({path:new URL('rest.png',out).pathname});await click('open-atlas');assert.ok(await page.locator('.expedition').count());await page.keyboard.press('Escape');assert.ok(await page.locator('.merchant-heading').count());await page.keyboard.press('Escape');assert.equal(await page.locator('.atlas').count(),0);
