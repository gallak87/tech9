...
export function initParty() {
  return {
    x: PLAYER_START.x,
    y: PLAYER_START.y,
    subX: 0, subY: 0, // for smoothed animation later
    moveCooldown: 0,
    facing: 'down',
  };
}
...
export function drawOverworld(ctx, game) {
  ...
  // camera centered on party
  const camX = p.x * TILE + TILE / 2 - w / 2;
  const camY = p.y * TILE + TILE / 2 - h / 2;
  ...
  // cities (landmark sprites, larger than a tile)
  ...
  // party marker
  const pcx = Math.round(p.x * TILE - camX);
  const pcy = Math.round(p.y * TILE - camY);
  // three heroes stacked visually — main hero then two trailing dots
  drawSprite(ctx, 'kaida_overworld', pcx, pcy, TILE, TILE);
  // soft glow ring underfoot
  ctx.fillStyle = 'rgba(34,227,255,0.18)';
  ctx.beginPath();
  ctx.ellipse(pcx + TILE / 2, pcy + TILE - 2, 14, 5, 0, 0, Math.PI * 2);
  ctx.fill();

  // fog-of-war on unexplored regions (render last, semi-opaque)
  ctx.fillStyle = 'rgba(7,6,13,0.82)';
  for (let ty = firstTy; ty < lastTy; ty++) {
    for (let tx = firstTx; tx < lastTx; tx++) {
      if (!game.explored.has(`${tx},${ty}`)) {
        const sx = Math.round(tx * TILE - camX);
        const sy = Math.round(ty * TILE - camY);
