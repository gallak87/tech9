...
Keep only live assets, source code, repeatable checks and current guidance in the repository. Delete superseded assets once no live atlas frame uses them. Do not retain rejected generations, prompt histories, screenshots, review reports, archived experiments or iteration logs. Preserve font licenses.

`node scripts/asset-inventory.mjs` prints the live source inventory to stdout without writing files. Browser and art inspection scripts write captures/reports to a checkout-specific directory in the OS temporary folder. `node scripts/review-output.mjs` prints its location; **`npm run clean:review`** deletes it after inspection. Temporary artifacts are development aids, never committed evidence.

`npm run verify` runs the browser harness against the dev server with isolated storage and `?test=1` hooks. Checks that read earned campaign snapshots require a fresh `node tests/campaign-browser.mjs` run first; those snapshots share the temporary directory. `npm run verify:pages` builds and checks root/subpath deployment in an isolated browser, then closes its server. Browser checks are explicit development tools; user visual review remains separate from automated checks.
