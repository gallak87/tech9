...
- Retain the actual setting and actors when the space supports battle. Here a local formation transform places the existing attack timeline on the elevated terrace, while a 0.95-second camera blend changes pitch/zoom without changing the outdoor movement axes. The owner requested twice the original transition speed for a snappier entry/exit. The pocket's coordinates are authored; arbitrary slopes, cramped rooms and larger parties need their own footprints and camera decisions.
