...
Vex and Rune use `assets/<hero>-idle.png` sheets generated in one call per hero with three poses together. Kaida uses the fresh eight-frame sheet described below. Playback is **1 → 2 → 3 → 2**, holding each beat for 300 ms. At load, canvas compositing removes the edge-connected dark matte once and caches the three frames. Sprite RGB is preserved. One shared scale and baseline per hero keeps body height consistent with the original battle atlas; no frames are warped. Rune uses fixed whole-cell offsets of [0,0], [26,0] and [43,2] source pixels to correct generated padding drift at his boots. Reduced motion selects the resting pose. The shared `drawHero` renderer selects these approved side-facing idles in battle, town, interiors and full-body menu portraits without a scene-specific opt-in. North/south resting views hold frame zero of their directional row until matching idle loops exist. Movement and combat actions retain their original atlas rows. Small head portraits retain their static atlas crop. Generation inputs and prompts are kept in each hero's `art-lab/<hero>-guided-idle/` directory.
...
The [party direction study](http://127.0.0.1:4179/art-lab/kaida-guided-idle/) repurposes the old breathing preview for Kaida, Vex and Rune. It compares the active game idle with front/back/right standing views generated together per hero, using a shared baseline, close comparison and A/B switching. Source backgrounds are preserved and their opaque-checkerboard limitation is shown explicitly. These studies do not switch game assets. The old page is archived at `art-lab/kaida-guided-idle/breathing-study.html`; [assets, exact prompts and provenance](art-lab/DIRECTION-STUDIES.md) are retained locally.

The game renders into a backing canvas sized for the displayed stage and device pixel ratio, capped at 2880×1800, while gameplay/input coordinates remain 960×600. All hero frames, enemies, NPCs, generated props, portraits and item icons use high-quality image filtering. Atlas crops are isolated at native resolution with transparent gutters and a bounded cache to prevent neighboring-frame bleed. UI sprite canvases also follow their actual displayed size. Scenery caches and the minimap keep nearest-neighbor rendering; sprite positions and on-screen sizes are unchanged. Pure checks: `node tests/display-systems.mjs` and `node tests/sprite-image-systems.mjs`. This pass has not been playtested.

Kaida's shared side-facing idle uses `assets/kaida-idle-fresh.png`: eight frames in a 4×2 grid at 6 fps, generated from a text description alone. Native transparency and original pixels are preserved. The previous guided sheet remains at `assets/kaida-idle.png` and in its standalone art-lab preview. `src/hero-idle.js` selects the active sheet and timing per hero; Vex and Rune retain their guided idles. The new prompt, provenance and rollback setting are in `art-lab/kaida-fresh-idle/`.

All visual assets are new. `assets/kaida.png`, `vex.png`, and `rune.png` are original transparent 6×8 animation atlases created with the built-in imagegen tool from textual identity descriptions. **assets/prompts.json** preserves the exact prompts and grid metadata. The atlases supply walk, run, attack, cast, hurt, north and south frames; their old side-facing idle rows are retained as source assets but are superseded in every scene by the approved idle sheets. Facing left mirrors directional frames; no static rotation replaces animation. Attacks use discrete windup/impact/recovery frame scheduling. Rune’s original atlas has uneven row spacing; the renderer samples measured transparent gutters instead of equal-height rows so neighboring boots do not appear above his head. Each source pixel keeps the same display scale, and the approved idle keeps its existing height. Idle registration offsets live in `HERO_IDLE_SHEETS` for both game and art-lab playback.
...
`node tests/hero-render-systems.mjs` checks the production hero renderer in Node with synthetic image/canvas inputs: all three heroes select approved side idles by default, left views mirror them, reduced/static motion holds pose zero, and directional/action frames retain their atlas mappings. It does not launch a browser or play the game.
