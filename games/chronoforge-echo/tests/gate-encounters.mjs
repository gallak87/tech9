...
try{
  ...
  for(const region of ['haventide','emberline','orbital_reach','last_crown']){
    const gate=await page.evaluate(async region=>{
      __ECHO__.preset('world');__ECHO__.goto(region);const g=__ECHO__.game;g.resetSession();g.encounterCooldown=0;
    ...
    const passed=await page.evaluate(gate=>__ECHO__.game.state.y<gate.y,gate);assert.ok(passed,'Keyboard path must actually pass the guard');
    ...
    assert.deepEqual(await page.evaluate(()=>__ECHO__.game.battle.encounter.enemies),gate.enemies);
    gate.battle=await shot(region+'-battle');
    if(region==='haventide'){
      for(let action=0;action<12;action++){
        const ready=await page.evaluate(()=>{const g=__ECHO__.game;for(let i=0;i<3600;i++){if(g.ui.blocked||g.mode!=='battle'||g.battle.result||g.battle.selectedHero)return !!g.battle?.selectedHero;g.update(1/60);}throw Error('No battle progress');});
        if(!ready)break;
        await page.keyboard.press('Enter');await page.keyboard.press('Enter');await page.keyboard.press('Enter');
        await page.evaluate(()=>{const g=__ECHO__.game;for(let i=0;i<600&&g.battle?.mode==='action';i++)g.update(1/60);});
      }
      await page.evaluate(()=>{const g=__ECHO__.game;for(let i=0;i<180&&g.mode==='battle';i++)g.update(1/60);});
      await closeDialogue();
      const victory=await page.evaluate(()=>({clear:__ECHO__.game.state.cleared.hav_guard,liberated:__ECHO__.game.state.flags.haventide_liberated,roadCrabClear:!!__ECHO__.game.state.cleared.hav_first}));
      ...
      await page.waitForFunction(()=>__ECHO__.game.scene.id==='haventide_town'&&!__ECHO__.game.transition);
  ...
  await page.evaluate(()=>{__ECHO__.preset('world');const g=__ECHO__.game;g.resetSession();g.encounterCooldown=0;g.state.flags.battle_taught=true;const o=g.scene.objects.find(o=>o.id==='hav_first');Object.assign(g.state,{x:o.x,y:o.y+60});g.updateCamera(true);});
