...
export function createContentRegistry(namespace, groups) {
  if (!/^[a-z][a-z0-9_]*$/.test(namespace))
    throw new Error('Invalid content namespace');
  ...
  return Object.freeze({
    ...
    get(kind, id) {
      const key = `${namespace}:${kind}:${id}`;
      if (!entries.has(key))
        throw new Error(`Required content is missing: ${key}`);
...
export function validateWorld(registry) {
  for (const key of registry.keys.filter((k) => k.includes(':scenes:'))) {
    ...
    for (const object of [...scene.objects, ...scene.portals]) {
      ...
      if (
        object.item &&
        !['food', 'ore', 'energy', 'renown'].includes(object.item)
      )
        registry.get('items', object.item);
    }
  }
}
