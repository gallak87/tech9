...
// audio — LANE STUB.
//
// Web Audio synthesis: synthwave beds per biome, combo stingers, resource chimes.
// No samples — the concept forbids binary assets, and that includes audio.
//
// Phase 0 ships the seam, not the feature. `main.js` already installs this and
// hands it the shared `ctx`; the audio lane fills it in without touching a
// single shared-core file. Do not widen this signature — add fields to the
// returned API instead.
//
// Owned by the `audio` lane. See CONTRACT.md §1.
// ─────────────────────────────────────────────────────────────────────────────
...
export function installAudio(ctx) {
  return {
     ...
     *  `?showcase=audio` boots straight into it and the critic reviews the
