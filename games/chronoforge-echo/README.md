...
`npm run verify` runs the browser harness against the dev server with isolated storage and `?test=1` hooks. Checks that read earned campaign snapshots require a fresh `node tests/campaign-browser.mjs` run first; those disposable snapshots live in `.experiments/output/`. `npm run verify:pages` builds and checks root/subpath deployment in an isolated browser, then closes its server. Browser checks are explicit development tools; user visual review remains separate from automated checks.
