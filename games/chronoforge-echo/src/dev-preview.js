...
export class ArtPreview {
  ...
  resetSelection() {this.townCenterLevel=null;this.townCenterRegion='haventide';}

  selectTown(region) {
    if(!['haventide','emberline','orbital_reach','last_crown'].includes(region))throw Error('Unknown preview town.');
    if(this.open)this.townCenterRegion=region;
  }

  selectTownCenter(level) {
    if(level!==null&&(!Number.isInteger(level)||level<1||level>4))throw Error('Preview level must be 1–4 or null.');
