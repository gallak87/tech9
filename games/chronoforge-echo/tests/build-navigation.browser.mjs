...
try {
  ...
  await page.evaluate(() => {
    window.__ECHO__.preset('settlement');
    window.__ECHO__.interact('haventide_board');
  });
  const expectFocus = async (action) =>
    assert.equal(
      await page.evaluate(() => document.activeElement?.dataset.do),
      action,
    );
  const press = async (key, action) => {
    await page.keyboard.press(key);
    await expectFocus(action);
  };
  ...
  await press('ArrowDown', 'build:research_lab');
  assert.ok(
    await page.locator('[data-do="build:research_lab"]').evaluate((el) => {
      const r = el.getBoundingClientRect(),
        body = el.closest('.atlas-body').getBoundingClientRect();
      return r.top >= body.top - 1 && r.bottom <= body.bottom + 1;
    }),
    'Focused row should scroll into view',
  );
  ...
  console.log(
    'PASS: settlement columns, row boundaries, disabled entries, sequential Tab, header return, and scrolling.',
  );
} finally {
  await browser.close();
}
