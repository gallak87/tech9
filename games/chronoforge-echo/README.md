...
On phones, use the movement stick and contextual Interact, Run/Walk, Map and Menu buttons. Double-tap the stick to toggle running; tap-to-walk, handedness, control size and portrait zoom are configurable. Battle uses one active command card with Back, Execute and fresh Strike/Guard presses. Portrait and landscape adapt without changing the expedition. Phones and small windows offer **Full atlas** or **On demand** at first boot and remember the choice. Full atlas downloads all artwork before play; On demand starts with the current region and loads other regions when needed. Change modes in Settings with Save & reload; larger desktops load the full atlas immediately. See the [Roadmap](ROADMAP.md) for the current mobile handoff and remaining device checks.
...
Enemy badges show the strongest visible fighter as **LVL · Name** and compare their level with Kaida: muted at least two levels below, neutral within one, amber two–three above, red four–six above, and deep red with a skull seven or more above. A faint red ground oval has roughly 15 native pixels of contact padding; approaching it starts battle, including sentries. Cleared repeat fights offer Revisit. Loading beside an uncleared patrol temporarily protects against automatic contact; Interact still offers Engage/Confront if its prerequisites are met. Cleared enemies and story-locked encounters have no contact oval. Post-battle protection hides the ring until contact becomes active again.
