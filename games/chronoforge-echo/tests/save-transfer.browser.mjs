...
try{
 ...
 await upload(pageB,'3',portable.path);await waitNotice(pageB,'Imported into Field record 03');
 ...
 await upload(pageB,'1',manual.path);await button(pageB,'confirm-no').waitFor();assert.equal(await active(pageB),'confirm-no');assert.deepEqual(await snapshot(pageB),beforeOverwrite);
 ...
 stage='autosave import has an explicit overwrite warning';const beforeAuto=await snapshot(pageB);await upload(pageB,'checkpoint',portable.path);await button(pageB,'confirm-no').waitFor();assert.match(await pageB.locator('.modal').innerText(),/Future automatic checkpoints/);await pageB.keyboard.press('Escape');assert.deepEqual(await snapshot(pageB),beforeAuto);await upload(pageB,'checkpoint',portable.path);await button(pageB,'confirm-yes').waitFor();await confirm(pageB);await waitNotice(pageB,'Imported into Autosave');onlyTargetChanged(beforeAuto,await snapshot(pageB),'checkpoint');assert.deepEqual((await stored(pageB,'checkpoint')).state,rich);mark(stage);
