...
export function mountDevTools(game) {
  ...
  const preview = new ArtPreview(),
    worlds = new WorldTravelPreview(game);
  let previousFocus = null,
    previousCamera = null,
    previousScene = null,
    availability = '',
    mapExplored = false,
    autoOpenPending = localDevPreviewRequested(window.location);
  const root = document.createElement('div');
  root.id = 'dev-tools';
  root.hidden = true;
  root.setAttribute('role', 'dialog');
  root.setAttribute('aria-label', 'Local development tools');
  root.innerHTML = `
    ...
    <section class="dev-world-controls" aria-label="Temporary world exploration">
      ...
      <div class="dev-world-jumps" role="group" aria-label="Jump to a world">${Object.values(
        REGIONS,
      )
        .map(
          (region) =>
            `<button type="button" data-world="${region.id}">${region.name}</button>`,
        )
        .join('')}</div>
  ...
  worldBadge.id = 'dev-world-preview';
  worldBadge.type = 'button';
  worldBadge.hidden = true;
  worldBadge.textContent = 'World preview · never saved · return to expedition';
  document.querySelector('#game').append(worldBadge);
  const upgradeTour = mountUpgradeTour(game, {
    onReturnToPlay: () => render(),
  });
  const worldView = game.worldView;
  const actual = () =>
    Math.max(1, Math.min(4, game.state.buildings.town_center || 1));
  const current = () => preview.townCenterLevel ?? actual();
  const indoors = () =>
    game.mode === 'world' && game.scene.id === 'haventide_town';
  const busy = () =>
    game.ui.blocked ||
    !!game.transition ||
    upgradeTour.open ||
  ...
  const canArt = () =>
    !busy() &&
    game.mode === 'world' &&
    ['haventide', 'haventide_town'].includes(game.scene.id);
  function render() {
    const blocked = busy(),
      art = canArt(),
      selected = current(),
      town = TOWN_CENTERS.find((t) => t.region === preview.townCenterRegion),
      frame = town.metadata.frames[selected - 1];
    ...
    root.querySelector('[data-preview="world-view"]').disabled =
      blocked || !devPreviewReady(game) || !!game.battle;
    for (const button of root.querySelectorAll('[data-world]'))
      button.disabled = blocked || !worlds.canJump;
    const returnButton = root.querySelector('[data-preview="return-world"]');
    returnButton.hidden = !worlds.active;
    returnButton.disabled = blocked || game.mode !== 'world' || !!game.battle;
    root.querySelector('.dev-world-status').textContent = worlds.active
      ? 'Temporary trip · saves keep your real expedition.'
      : 'Click a world to jump · story gates bypassed.';
    const select = root.querySelector('[data-town-select]');
    select.value = preview.townCenterRegion;
    select.disabled = !art;
    root.querySelector('output').textContent =
      selected + ' · ' + TIERS[selected - 1];
    root.querySelector('.dev-size').textContent = indoors()
      ? 'Interior'
      : frame.nativeWidth +
        ' × ' +
        Math.round((frame.h * frame.nativeWidth) / frame.w);
    for (const button of root.querySelectorAll('[data-tier]')) {
      button.disabled = !art;
      button.setAttribute(
        'aria-pressed',
        String(Number(button.dataset.tier) === selected),
      );
    }
    root.querySelector('[data-preview="restore"]').disabled =
      !art ||
      (preview.townCenterLevel === null &&
        preview.townCenterRegion === 'haventide');
    const from = Math.min(3, selected),
      upgrade = root.querySelector('[data-preview="upgrade"]');
    upgrade.textContent = 'Preview upgrade ' + from + ' → ' + (from + 1);
    upgrade.disabled = !art;
    root.querySelector('.dev-status').textContent = art
      ? 'Inside → exterior → restored hall · actual level ' + actual()
      : blocked
        ? 'Controls paused during menus and reveals.'
        : 'Town art previews are available in Haventide.';
    root.querySelector('.dev-pause-status').textContent = game.transition
      ? 'Traveling'
      : blocked
        ? 'Controls paused'
        : 'Play paused';
  ...
  function focusTownArtwork() {
    const entrance = game.scene.objects.find(
      (o) => o.id === 'haventide_entrance',
    );
    if (!entrance) return;
    if (!previousCamera || previousScene !== game.scene.id) {
      previousCamera = { ...game.camera };
      previousScene = game.scene.id;
    }
    const bounds = townCenterPreviewBounds(entrance);
    game.camera.x = Math.max(
      0,
      Math.min(
        game.scene.width - VIEW_WIDTH,
        bounds.left + bounds.width / 2 - VIEW_WIDTH * 0.61,
      ),
    );
    game.camera.y = Math.max(
      0,
      Math.min(game.scene.height - VIEW_HEIGHT, bounds.top - 60),
    );
  }
  function setLevel(value) {
    if (canArt()) {
      preview.selectTownCenter(value);
      focusTownArtwork();
      render();
    }
  }
  function cycle(delta) {
    setLevel(((current() - 1 + delta + 4) % 4) + 1);
  }
  function setOpen(value) {
    ...
    preview.setOpen(value);
    root.hidden = !preview.open;
    game.keys.clear();
    game.movePath = [];
    game.moving = false;
    if (preview.open) {
      ...
      if (previousCamera && previousScene === game.scene.id)
        Object.assign(game.camera, previousCamera);
      previousCamera = null;
      previousScene = null;
      game.ui.positionInteraction();
  ...
  worldBadge.addEventListener('pointerdown', (event) =>
    event.stopPropagation(),
  );
  worldBadge.addEventListener('click', (event) => {
    event.stopPropagation();
    restoreWorld();
  });
  root.addEventListener('pointerdown', (event) => event.stopPropagation());
  root.addEventListener('change', (event) => {
    if (event.target.matches('[data-town-select]') && canArt()) {
      preview.selectTown(event.target.value);
      focusTownArtwork();
      render();
    }
  });
  root.addEventListener('click', (event) => {
    ...
    if (button.dataset.tier) {
      setLevel(Number(button.dataset.tier));
      return;
    }
    if (button.dataset.world) {
      if (!busy()) jumpWorld(button.dataset.world);
      return;
    }
    switch (button.dataset.preview) {
      ...
      case 'map':
        ...
        break;
      case 'return-world':
        if (!busy()) restoreWorld();
        break;
      case 'world-view':
        if (!busy()) {
          worldView.openView({ revealAll: true });
          render();
        }
        break;
      case 'upgrade':
        if (canArt()) {
          upgradeTour.openPreview(
            Math.min(3, current()),
  ...
  const api = {
    ...
    get paused() {
      return preview.open && !game.transition;
    },
    get mapExplored() {
      return mapExplored;
    },
    get worldPreviewActive() {
      return worlds.active;
    },
    saveSource() {
      return worlds.saveSource();
    },
    jumpWorld,
    visualState(state) {
      return preview.visualState(state);
    },
    update() {
    ...
    handleKey(event) {
      ...
      if (event.code === 'Backquote' || event.key === '`') {
        ...
        if (!event.repeat) setOpen(!preview.open);
        return true;
      }
      if (!preview.open) return false;
      if (worldView.handleKey(event) || upgradeTour.handleKey(event))
        return true;
      ...
      if (/^[1-4]$/.test(event.key)) {
        event.preventDefault();
        if (!event.repeat) setLevel(Number(event.key));
        return true;
      }
      if (['ArrowLeft', 'ArrowRight'].includes(event.key)) {
        event.preventDefault();
        if (!event.repeat) cycle(event.key === 'ArrowLeft' ? -1 : 1);
        return true;
      }
      if (event.key === 'Tab') {
        event.preventDefault();
      ...
      if (event.key === ' ' && !root.contains(document.activeElement))
        event.preventDefault();
      return true;
    },
    dispose() {
